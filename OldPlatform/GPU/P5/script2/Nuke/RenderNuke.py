...
class Nuke(RenderBase):
    ...
    def RBrender(self):#7
        ...
        if os.path.isfile(_NK):
            ...
            User_vers = RVLIB.Check_vers(_NK,_CGV)
            ...
            _NUKE_RUN_PATH = RVLIB._RV_App_Nuke_new_Setup(_SERVER_Plugins,User_vers,True)
            print "_NUKE_RUN_PATH=" + _NUKE_RUN_PATH

            if _CID == '1876444':
                RVNUKE.copy_plugins(_NUKE_RUN_PATH)
            if _CID == '1860670':
                RVNUKE.copy_plugins(_NUKE_RUN_PATH)
    ...
    def RBrenderConfig(self):#5
        ...
        ## edit by shen 
        custom_p = r'B:\plugins\nuke\custom'
        custom_id_p = os.path.join(custom_p,self.G_USERID,'CustConfig.py')
        if os.path.exists(custom_id_p):
            print('CustConfig , id: %s'%self.G_USERID)
            sys.path.append(os.path.join(custom_p,self.G_USERID))
            import CustConfig as cusccg
            cusccg.main(self.G_USERID)
