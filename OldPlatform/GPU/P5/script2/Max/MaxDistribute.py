...
class DistributedRender():
    ...
    #节点：net use Z: "\\主控IP\d$\work\render\任务号"
    def netDisk(self):
        self.G_PROCESS_LOG.info('[Max.netDisk start]')
        
        netBDisk='try3 net use B: "'+self.PLUGIN_B_PATH+'"'
        if os.path.exists(self.PLUGIN_B_PATH):
            self.runcmd(netBDisk)
        else:
            self.G_PROCESS_LOG.info('The path is not exist:%s' % (self.PLUGIN_B_PATH))
            
        master_path = '%s\\%s' % (self.R_MASTERIP,self.R_RENDER_WORK_TASK.replace(':','$'))
        net_use_cmd = 'net use Z: "%s"' % (master_path)
        if os.path.exists(master_path):
            self.runcmd(net_use_cmd)
        else:
            self.G_PROCESS_LOG.info('The path is not exist:%s' % (master_path))
            
        self.G_PROCESS_LOG.info('[Max.netDisk end]')
