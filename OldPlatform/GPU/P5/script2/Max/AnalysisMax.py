...
class Max(AnalysisBase):
    ...
    def RBhanFile(self):#3copy max file
        ...
        self.RBcmd(copyCmdStr,myLog=True)
    ...
    def maxfind(self):
        ...
        self.RBcmd(maxFindCmdStr.encode(sys.getfilesystemencoding()),myLog=True)
    ...
    def multiscatterAndVray(self):
        ...
        if self.CONFLICT_PLUGIN.maxPluginConflict(self.G_CG_VERSION.replace('3ds Max ',''),self.G_MAXFIND_RENDERER.replace('vray',''),multiscatterStr):
    ...
    def configMaxPlugin(self):#config 3ds Max ,vray,plugin...and so on
        ...
        self.G_ANALYSE_LOG.info('pluginJson------'+self.G_PLUGINS)
        self.G_ANALYSE_LOG.info('pluginCfg------'+pluginCfgFile)
        maxPlugin=MaxPlugin(pluginCfgFile,self.G_ANALYSE_LOG)
        maxPlugin.config()
        #sys.exit(-1)
    ...
    def RBconfig(self):#5
        ...
        DogUtil.killMaxVray(self.G_ANALYSE_LOG)
        ...
        if not os.path.exists(r'B:\plugins'):
            cmd='try3 net use B: '+self.G_PLUGIN_PATH.replace('/','\\').replace("10.60.100.151","10.60.100.152")
    ...
    def parseAnalyseTxt(self):
        ...
        self.ANALYSE_TXT_PARSER = ConfigParser.RawConfigParser()
        try:
    ...
    def getPointCacheList(self,assetFile,resultList):
        ...
        if wildPath.endswith('.xml') and wildPath2!=None and os.path.exists(wildPath2):
            ...
            # try:
                # cacheStart = int(time.split('-')[1]) / int(cacheTimePerFrame)
                # cacheEnd  = int(time.split('-')[2]) / int(cacheTimePerFrame)
            # except IndexError:
                # cacheStart = int(time.split('-')[0]) / int(cacheTimePerFrame)
                # cacheEnd  = int(time.split('-')[1]) / int(cacheTimePerFrame)
            patt = '(-?\d+)-(-?\d+)'
            ...
            if m != None:
                cacheStart = int(m.group(1)) / int(cacheTimePerFrame)
                cacheEnd  = int(m.group(2)) / int(cacheTimePerFrame)
            else:
        ...
        def handler(key):
            ...
            if key == 'fumefx':
               ...
               for asset in assetList:
                   ...
                   fileName =  format.replace('#', '*').replace('name', prefix).replace('ext', 'bin')  
    ...
    def copyTipTxt(self):
        if os.path.exists(self.G_TIPS_TXT_NODE):
            ...
            print tipTxtCmd
            self.RBcmd(tipTxtCmd,myLog=True)
    ...
    #-----------------Write rendertype to analyse_net.txt(e.g. rendertype=maxcmd)--------------------
    def writeRenderType(self):
        self.G_ANALYSE_LOG.info('[max.writeRenderType.start.....]')
            ...
            if self.PLUGIN_DICT.has_key('plugins'):
                ...
                for pluginsKey in pluginDict.keys():
                    ...
                    if pluginStr in pl_list:
                        cfgCommon = 'common'
                        if not self.ANALYSE_TXT_PARSER.has_section(cfgCommon):
                            self.ANALYSE_TXT_PARSER.add_section(cfgCommon)
                        self.ANALYSE_TXT_PARSER.set(cfgCommon, 'rendertype', 'maxcmd')
                        self.ANALYSE_TXT_PARSER.set(cfgCommon, 'prenodetype', 'gpu')
                        break
            else:
                print 'There is no key--plugins in the plugins.cfg'
        self.G_ANALYSE_LOG.info('[max.writeRenderType.end.....]')
    ...
    def RBhanResult(self):#8
        ...
        print analyseTxtCmd
        self.RBcmd(analyseTxtCmd,myLog=True)
