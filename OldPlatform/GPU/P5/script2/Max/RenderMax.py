...
class MaxClient(RenderBase):
    ...
    def RBrenderConfig(self):#5
        ...
        maxPlugin=MaxPlugin(pluginCfg,self.G_PROCESS_LOG,vray_license=self.G_VRAY_LICENSE)
        maxPlugin.config()
        #self.vrayDistribute()
        if int(self.G_CG_TILECOUNT)>1 and self.G_CG_TILECOUNT==self.G_CG_TILE:#merge Pic
            pass
    ...
    def startGrab(self):
        ...
        # 如果是分条 重新计算并覆盖startSocket
        if int(self.G_CG_TILECOUNT) > 1:
            if self.G_CG_TILECOUNT != self.G_CG_TILE:
                row = int(self.G_CG_TILECOUNT)
                index = int(self.G_CG_TILE)

                if self.argsmap.has_key('from') and self.argsmap['from'] == 'web':
                    if self.RENDER_CFG_PARSER.has_option('common', 'width'):
                        width = self.RENDER_CFG_PARSER.get('common', 'width')
                    if self.RENDER_CFG_PARSER.has_option('common', 'height'):
                        height = self.RENDER_CFG_PARSER.get('common', 'height')
                else:
                    if self.RENDER_CFG_PARSER.has_option('renderSettings', 'width'):
                        width = self.RENDER_CFG_PARSER.get('renderSettings', 'width')
                    if self.RENDER_CFG_PARSER.has_option('renderSettings', 'height'):
                        height = self.RENDER_CFG_PARSER.get('renderSettings', 'height')

                if self.argsmap.has_key("picplugin"):
                    if self.argsmap.has_key("coronarender"):
                        renderer = "coronarender"
                    else:
                        renderer = "vray"
                    # if self.argsmap.has_key("vray"):
                    #     renderer = "vray"
                else:
                    renderer = "vray"
                ...
                height = int(height)
                if index == row:
                    tile_height = height / row + height % row
                else:
                    tile_height = height / row

                startSocket = "rdf1grab|{rate}|{folder_local}|{folder_server}|{munu_id}_{job_id}|{node_name}|{width}|{height}|{version}".format(
                    rate=grabRate,
                    folder_local=grabFolderLocal,
                    folder_server=grabFolderServer,
                    munu_id=self.G_MUNU_ID,
                    job_id=self.G_JOB_ID,
                    node_name=self.G_NODE_NAME,
                    width=width,
                    height=tile_height,
                    version=renderer,
                )
    ...
    def restore(self, input_path, output_path):
        ...
        self.RBcmd(cmd, myLog=True)
        try:
            os.remove(os.path.join(output_path, "name.json"))
        except Exception as e:
            self.G_PROCESS_LOG.info("[restore], {}".format(e))
    ...
    def RBrenderMerge(self):
        ...
        for tile in tiles:
            ...
            if ext in [".bmp", ".rla", ".rpf", '.vrimg', ".fxr", ".hdr", ".pic", ".jpe", ".jpeg", ".vda", ".icb", ".vst", ".tiff"]:
                ...
                self.RBcmd(copy_cmd, myLog=True, continueOnErr=True)
            else:
                run_nuke = True
        if run_nuke is True:
            ...
            if os.path.isfile(nuke_path1):
                ...
                raise NotImplementedError("[merge picture--nuke] nuke does not exist")
            if not os.path.isfile(py_path):
                raise NotImplementedError("[merge picture--nuke] nuke_merge_script.py does not exist")
            ## 临时补丁(可能长期) 2018.04.26
            try:
                os.environ['foundry_LICENSE'] = "4101@127.0.0.1;4101@10.60.1.108;4101@10.60.5.248"
            except Exception as e:
                self.G_FEE_LOG.info("set environ for nuke error:\n{}".format(e))
            ########
    ...
    def RBrenderPic(self):#7
        ...
        #cmdStr=cmdFool
        self.G_PROCESS_LOG.info("[MAX_CMD_RENDER] =={}===\nself.G_JOB_NAME_STR={}".format(self.MAX_CMD_RENDER, self.G_JOB_NAME_STR))
        if self.G_JOB_NAME_STR == "channel":
            maxfile_channel = self.MAX_FILE[:-4] + '_channel' + self.MAX_FILE[-4:]
            if os.path.isfile(maxfile_channel):
                self.MAX_CMD_RENDER = True
            else:
                self.MAX_CMD_RENDER = False
        ...
        if  self.MAX_CMD_RENDER==True:
            ...
            cmdRenderOutput = self.filename_add_underline(cmdRenderOutput)
            ...
            if self.G_JOB_NAME_STR == "channel":
                root, ext = os.path.splitext(cmdRenderOutput)
                cmdRenderOutput = "{}{}_{}".format(root, "color", ext)
            #-----------------------------------------frames-----------------------------------------------
            frame_start = None
            frame_end = None
            frame_nth = None
            ...
            elif self.G_JOB_NAME_STR == "channel":
                maxfile_channel = self.MAX_FILE[:-4] + '_channel' + self.MAX_FILE[-4:]
                cmdStr=cmdStr+' "'+maxfile_channel.replace('\\','/')+'"'
            else:
...
#        sys.exit(-1)
        ...
        if int(self.G_CG_TILECOUNT) > 1 and self.argsmap.get('from', None) in ['3dsmax', 'client']:
            ...
            for f in files:
                ...
                if os.path.isfile(filename):
                    self.G_PROCESS_LOG.info("[move file] {}\n{}\n{}\n{}".format(filename, renderOutput, type(filename), type(renderOutput)))
                    cmd= 'c:\\fcopy\\FastCopy.exe /cmd=move /speed=full /force_close  /no_confirm_stop /force_start "{}" /to="{}"'.format(
                        filename.replace('/','\\'),
            ...
            self.G_PROCESS_LOG.info("[list dir] {}\n{}".format(renderOutput, type(renderOutput)))
            try:
                files = os.listdir(renderOutput)
            except WindowsError as e:
                files = os.listdir(renderOutput.decode("utf-8"))
    ...
    def remove_ending_dot(self, path):
        """
        如果路径的最后一个字符是 . 则去掉
        """
        if path.endswith("."):
            length = len(path)
            path = path[:length - 1]
            return self.remove_ending_dot(path)
        return path

    def filename_add_underline(self, path):
        """
        Add a underline at the end of the path(except ext) if the path((except ext)) does not end with the following line.
            hello_.jpg --> hello_.jpg  (remain unchanged)
            hello.jpg --> hello_.jpg
            c:\world\hello.jpg --> c:\world\hello_.jpg
            hello --> hello_
            c:\world\hello --> c:\world\hello_
        :param str path:  path
        :return: path have underline ending
        :rtype: str
        """
    ...
    def RBresultAction(self):
        ...
        self.RBlog('节点机输出路径:'+self.G_RENDER_WORK_OUTPUT.replace('\\','/'))
        self.RBlog('节点机输出备份路径:'+self.G_RENDER_WORK_OUTPUTBAK.replace('\\','/'))
        userOutput = self.remove_ending_dot(userOutput)
