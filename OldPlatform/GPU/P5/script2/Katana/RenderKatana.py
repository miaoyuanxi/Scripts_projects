...
class Katana(RenderBase):
	def __init__(self,**paramDict):
		...
		#-------------------add renderLog-------------------------------
		newRenderLogDir=os.path.join(self.G_LOG_WORK,self.G_TASKID)
		if not os.path.exists(newRenderLogDir):
			os.makedirs(newRenderLogDir)
		
		self.G_NEW_RENDER_LOG=logging.getLogger('newrenderlog')
		fm=logging.Formatter("%(asctime)s  %(levelname)s - %(message)s","%Y-%m-%d %H:%M:%S")
		newRenderLogPath=os.path.join(newRenderLogDir,(self.G_JOB_NAME+'_render.log'))
		self.G_NEW_RENDER_LOG.setLevel(logging.DEBUG)
		newRenderLogHandler=logging.FileHandler(newRenderLogPath)
		newRenderLogHandler.setFormatter(fm)
		self.G_NEW_RENDER_LOG.addHandler(newRenderLogHandler)
		console = logging.StreamHandler()  
		console.setLevel(logging.INFO)  
		self.G_NEW_RENDER_LOG.addHandler(console)
	...
	def getFinalList(self,renderNode):
		...
		if os.path.exists(self.analyseFile_path):
			...
			print '=============='+str(analyseJson[renderNode][0])
			if isinstance(analyseJson[renderNode][0],dict):
				# self.finalList = analyseJson[renderNode]["Final"]
				self.finalList = analyseJson[renderNode][0].values()
	...
	def RBrendercmd(self,cmdStr,continueOnErr=False,myShell=False,myLog=False):#continueOnErr=true-->not exit ; continueOnErr=false--> exit 
		print str(continueOnErr)+'--->>>'+str(myShell)
		if myLog:
			self.G_PROCESS_LOG.info('cmd...'+cmdStr)
			self.G_NEW_RENDER_LOG.info('cmd...'+cmdStr)
		...
		while cmdp.poll()==None:
			...
			if resultLine!='':
				if myLog:
					...
					self.G_NEW_RENDER_LOG.info(resultLine)
		...
		if myLog:
			...
			self.G_NEW_RENDER_LOG.info('resultStr...'+resultStr)
			self.G_PROCESS_LOG.info('resultCode...'+str(resultCode))
			self.G_NEW_RENDER_LOG.info('resultCode...'+str(resultCode))
	...
	def RBrender(self):#7
		...
		if 'denoise' in self.G_CG_OPTION:
			...
			# print "copy katana batch"
			# renderCmd="/opt/katana/bin/Katana-batch %s %s %s-%s %s %s  %s" % (self.CGFILE,self.G_CG_LAYER_NAME,self.G_CG_START_FRAME,self.G_CG_END_FRAME,"32","32",os.path.dirname(self.CGFILE))
			# print "renderCmd inof ...%s " % renderCmd
				...
				if len(self.finalList)>0:
					...
					for finalPath in self.finalList:
						...
						if os.path.exists(finalPath):
							output_path = self.outputPath+os.path.dirname(finalPath)
							self.pythonCopy(finalPath,output_path)
	...
	def outputMnt(self):
		...
		# mnt_output = self.G_PATH_USER_OUTPUT.replace(self.outputFolder,'').replace('\\','/')
		spPath = "outputdata5"
		self.outputFolder=self.G_PATH_USER_OUTPUT[self.G_PATH_USER_OUTPUT.rfind(spPath)+len(spPath):len(self.G_PATH_USER_OUTPUT)]
		outputMntpath =  self.G_PATH_USER_OUTPUT.replace(self.outputFolder,'').replace('\\','/')
		...
		outputmnt='mount -t cifs -o username=enfuzion,password=ruiyun2016,codepage=936,iocharset=gb2312 '+outputMntpath+' '+self.outputPath
		feemnt='mount -t cifs -o username=enfuzion,password=ruiyun2016,codepage=936,iocharset=gb2312 '+self.G_PATH_COST.replace('\\','/')+' '+self.feePath
		if not os.path.exists(self.outputPath):
			os.makedirs(self.outputPath)
							...
							if int(self.G_CG_START_FRAME) <= int(self.G_CG_END_FRAME):
								for frame in xrange(int(self.G_CG_START_FRAME),int(self.G_CG_END_FRAME)+1):
									...
									if os.path.exists(fromPath):
										copy_to_dir = '{}{}'.format(self.G_RENDER_WORK_OUTPUT.replace('\\','/'), os.path.dirname(fromPath.replace('\\','/')))
										# self.pythonCopy(fromPath.replace('\\','/'),self.G_RENDER_WORK_OUTPUT.replace('\\','/'))
										self.pythonCopy(fromPath.replace('\\','/'), copy_to_dir)
									else:
