...
class Softimage(RenderBase):
    ...
    def RBrender(self):#7
        ...
        renderCmd=softimageRenderbat+' "'+self.G_USERID+'" "'+self.G_TASKID+'" "'+myCgVersion+'" "'+self.analyseTxt.replace("mnt","/10.50.244.116").replace("/","\\")+'" "'+self.G_CG_START_FRAME+'" "'+self.projectPath+'" "'+output+'" "'+self.cgfile.replace("/","\\")+'" "'+self.G_CG_LAYER_NAME+'"' + ' >>' + outputrenderlogFile
