...
class Maya(RenderBase):
    ...
    def RBexecute(self):#Render
        ...
        if self.G_RENDEROS=='Linux':
            ...
            self.copyBlack()
        try:
            cust_set_path = r"\\10.90.96.51\td1\custom_config\%s"%self.G_USERID
            if os.path.exists(r"%s\post.py"%cust_set_path):
                print("Copy post")
                os.system("copy /y %s %s"%(r"%s\post.py"%cust_set_path,"C:\work\render\%s\cfg\post.py"%self.G_TASKID))
