...
class Maya(RenderBase):
    ...
    def RBrender(self):#7
        ...
        new_log_dir = os.path.join(self.G_LOG_WORK, self.G_TASKID)
        if not os.path.exists(new_log_dir):
            os.makedirs(new_log_dir)
        new_log_file = os.path.normpath(os.path.join(new_log_dir, (self.G_JOB_NAME + '_newlog.txt')))
        ...
        renderCmd=renderBat +' "'+self.G_USERID+'"  "'+self.G_TASKID+'"  "'+mayaExePath+'" "'+renderTxtFile+'" "'+self.G_CG_START_FRAME+'" "' + self.G_CG_END_FRAME +'" "'+self.G_CG_BY_FRAME +'" "'+ self.G_PATH_INPUTPROJECT +'" "'+ self.G_PATH_INPUTFILE+'" "'+ self.G_CONFIG+'" "'+ self.G_PLUGINS+'" "'+ self.G_CG_TILE+'" "'+ self.G_CG_TILECOUNT+'" ' + ' > ' + new_log_file
        
        #os.system(renderCmd)
        self.RBcmd_test20180330(renderCmd,False,1,myLog=True)
        endTime = time.time()
    ...
    def RBcmd_test20180330(self,cmdStr,continueOnErr=False,myShell=False,myLog=False):#continueOnErr=true-->not exit ; continueOnErr=false--> exit 
        print str(continueOnErr)+'--->>>'+str(myShell)
        if myLog:
            self.G_PROCESS_LOG.info('cmd...'+cmdStr)
        cmdp=subprocess.call(cmdStr, shell = myShell)
        exitcode = 0
        return exitcode
...
class MayaClient(Maya):
    ...
    def RBrender(self):#7
        ...
        self.RENDER_RESULT_CODE=0
        if self.G_CG_START_FRAME==self.G_CG_END_FRAME:#one node with one frame
            self.G_PROCESS_LOG.info('One node with one Frame')
            subprocess.call(renderCmd)
            #self.RENDER_RESULT_CODE,resultStr=self.mayaCmd(renderCmd,False,False)
            #self.G_PROCESS_LOG.info('RENDER_RESULT_CODE='+str(self.RENDER_RESULT_CODE))
        else:#one node with multi frame
            self.G_PROCESS_LOG.info('One node with multiple frames')
            self.RENDER_RESULT_CODE,resultStr=self.mayaCmd(renderCmd,True,False)
            self.G_PROCESS_LOG.info('RENDER_RESULT_CODE='+str(self.RENDER_RESULT_CODE))
    ...
    def mayaCmd(self,cmdStr,continueOnErr=False,myShell=False):#continueOnErr=true-->not exit ; continueOnErr=false--> exit 
        print str(continueOnErr)+'--->>>'+str(myShell)
        self.G_PROCESS_LOG.info('cmd...'+cmdStr)
        cmdp=subprocess.Popen(cmdStr,stdin = subprocess.PIPE,stdout = subprocess.PIPE, stderr = subprocess.STDOUT, shell = myShell)
        cmdp.stdin.write('3/n')
    ...
    def RBpostPy(self):#pre custom
        ...
        if self.G_CG_START_FRAME!=self.G_CG_END_FRAME and self.RENDER_RESULT_CODE!=0:
            self.G_PROCESS_LOG.info('One node with multiple frames Exit -1')
            sys.exit(-1)
        self.RBlog('done','end')
