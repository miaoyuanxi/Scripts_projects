...
class Merge(RenderBase):
    ...
    def RBrender(self):
        ...
        if os.path.exists(photonUserPath):
            ...
            copyVrmapCmd=r'c:\fcopy\FastCopy.exe /speed=full /force_close /no_confirm_stop /force_start "'+photonUserPath+'\\*.vr*map" /to="'+photonPath.replace('/','\\')+'"'
            self.RBlog(copyVrmapCmd)
            self.RBcmd(copyVrmapCmd)
        ...
        self.mergePhoton(photonPath,'vrmap')
