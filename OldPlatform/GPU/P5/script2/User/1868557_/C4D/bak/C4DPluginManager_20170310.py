...
class C4DPluginManagerCenter(object):
    ...
    def DeleteFiles(self,path):
        if plugin.plugin_name == 'c4dtoa':
            ...
            print ai_path('11111111111')
            # os.remove(ai_path, c4d_installed_path)
            # os.remove(lic_path, c4d_installed_path)
        elif plugin.plugin_name == 'vray':
            # del VrayBridge.key
            vray_installed_path = os.path.join("C:/Program Files/MAXON", \
                                            self.plugin.soft_version) 
                                            
            vray_key_path = os.path.join(self.vray_installed_path, "VrayBridge.key")
            print vray_key_path('11111111111')
            # os.remove(vray_key_path)
    '''
    ...
    def set_custom_env(self, plugin):
        ...
        if plugin.plugin_name == 'c4dtoa':
            ...
            ai_path  = os.path.join(self.plugin_path, "ai.dll")
            lic_path = os.path.join(self.plugin_path, "solidangle.lic")
            os.system('xcopy /f /y "%s" "%s"' % (ai_path, c4d_installed_path))
            os.system('xcopy /f /y "%s" "%s"' % (lic_path, c4d_installed_path))
        elif plugin.plugin_name == 'vray':
            # copy VrayBridge.key
            vray_installed_path = os.path.join("C:/Program Files/MAXON", \
                                            self.plugin.soft_version) 
            ...
            os.system('xcopy /f /y "%s" "%s"' % (vray_key_path,  vray_key_dest_path))
...
if __name__ == "__main__":
    ...
    node           = sys.argv[4]
    print plugin_name,plugin_version,plugin_version,node
...
if __name__ == "__main__":
    ...
    plugin = C4DPlugin("c4dtoa", "1.0.14.1", soft_ver,node)
    plugin_mgr.set_custom_env(plugin)
