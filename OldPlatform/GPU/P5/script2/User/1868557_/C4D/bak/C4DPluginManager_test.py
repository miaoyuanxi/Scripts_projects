    ...
    def set_custom_env(self, plugin):
        ...
        if plugin.plugin_name == 'c4dtoa':
            if self.plugin.soft_version == "CINEMA 4D R19":
                print( '!!! C4dtoa for R19 special set !!! \n')
                R19_root = os.path.join(r'C:/Program Files/MAXON/' + self.plugin.soft_version)
                if os.path.exists(R19_root):
        ...
        # Some plugin like .pyv .pye set should be by copy to node then load.
