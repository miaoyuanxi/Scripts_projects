...
class C4Dparser():
    ...
    def __del__(self):
        ...
        import kill
        os.remove('%appdata%\MAXON\CINEMA 4D R13_05DFD2A0\plugins\Analys.pyp')
        os.remove('%appdata%\MAXON\CINEMA 4D R14_4A9E4467\plugins\Analys.pyp')
        os.remove('%appdata%\MAXON\CINEMA 4D R15_53857526\plugins\Analys.pyp')
        os.remove('%appdata%\MAXON\CINEMA 4D R16_14AF56B1\plugins\Analys.pyp')
        os.remove('%appdata%\MAXON\CINEMA 4D R17_8DE13DAD\plugins\Analys.pyp')
        os.remove('%appdata%\MAXON\CINEMA 4D R18_62A5E681\plugins\Analys.pyp')
        os.remove('%appdata%\MAXON\CINEMA 4D R19_BFE04C39\plugins\Analys.pyp')
    ...
    def Analyze(self, *args):
        ...
        with open(analystxt,'w+') as f:
            ...
            Created_Version = data[c4d.DOCUMENT_INFO_PRGCREATOR_NAME]
            Saved_Version = data[c4d.DOCUMENT_INFO_PRGWRITER_NAME]
            f.write("Created_Version:"+str(Created_Version)+"\n\n")
            f.write("Saved_Version:"+str(Saved_Version)+"\n\n")'''
            
            f.write("\n------------Renderer-----------------\n")
            doc = documents.GetActiveDocument()
            MyRenderSetting = doc.GetActiveRenderData()
            Rnderer_ID = {0:'Standard',1023342:'Physical',1:'Software_OpenGL',300001061:'Hardware_OpenGL',1016630:'CineMan',1019782:'V_Ray',1029988:'Arnold(c4dtoa)',1029525:'Octane_GPURender',1036219:'Redshift_GPURender'}
            Renderer = Rnderer_ID[MyRenderSetting[c4d.RDATA_RENDERENGINE]]
            f.write("Renderer:"+Renderer+"\n\n")
            
            f.write("\n------------OPENGL_RENDERER_NAME-----------------\n")
            bc = c4d.GetMachineFeatures()
            print("OPENGL_RENDERER_NAME:"+bc[c4d.OPENGL_RENDERER_NAME])
            f.write("OPENGL_RENDERER_NAME:"+bc[c4d.OPENGL_RENDERER_NAME])
            ...
            if Renderer == "Physical":
                ...
                f.write("Phy_S:"+Phy_S[int(vp[c4d.VP_XMB_RAYTRACING_SAMPLER])]+"\n\n")
    ...
    def extract_texture(self, items):
        ...
        if str(fps) != str(Render_Settings_fps):
            self.FPS_unified.append(fps)
            self.FPS_unified.append(Render_Settings_fps)
        else:
            pass
        ...
        doc = documents.GetActiveDocument()
        MyRenderSetting = doc.GetActiveRenderData()
        Rnderer_ID = {0:'Standard',1023342:'Physical',1:'Software_OpenGL',1016630:'CineMan',1019782:'V_Ray',1029988:'Arnold(c4dtoa)',1029525:'Octane_GPURender',1036219:'Redshift_GPURender'}
        Renderer = Rnderer_ID[MyRenderSetting[c4d.RDATA_RENDERENGINE]]
        if Renderer == "Physical":
            doc = documents.GetActiveDocument()
            rd = doc.GetActiveRenderData()
            ...
            if Phy_S == "Progressive":
                if Phy_S_Pro_Mode == "Infinite":
                    self.PhySampler_Mode.append(Phy_S_Pro_Mode)
        else: 
            pass'''
        ...
        self.missings['35002'] = self.FPS_unified
