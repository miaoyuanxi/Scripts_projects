...
class Maya(RenderBase):
    def __init__(self,**paramDict):
        ...
        self.G_VRAY_LICENSE = '1'  # 1:old vray     2:copyrighted vray
        if paramDict.has_key('G_VRAY_LICENSE'):
            self.G_VRAY_LICENSE=paramDict['G_VRAY_LICENSE']
        os.environ["G_VRAY_LICENSE"] = self.G_VRAY_LICENSE
        ...
        if os.path.exists(self.G_CONFIG):
            ...
            if isinstance(configJson["mntMap"],dict):
                for key in configJson["mntMap"]:
                    ...
                    cmd='mount -t cifs -o username=enfuzion,password=ruiyun2016 '+mnt_value+' '+key
                    # self.G_RENDER_LOG.info(cmd)
                    self.G_PROCESS_LOG.info(cmd.replace('enfuzion', '').replace('ruiyun2016', ''))
                    self.RBcmd(cmd,False,1)
        
        # linux mount self.G_PATH_TILES        
        self.G_PATH_INPUTPROJECT_LINUX = '/input_project'
        if self.G_CG_TILECOUNT !='1' and self.G_CG_TILECOUNT == self.G_CG_TILE:
            if not os.path.exists(self.G_PATH_INPUTPROJECT_LINUX):
                os.makedirs(self.G_PATH_INPUTPROJECT_LINUX)
            input_project_mnt_path = self.G_PATH_INPUTPROJECT.replace('\\','/')
            # input_project_mnt = 'mount -t cifs -o username=enfuzion,password=ruiyun2016,codepage=936,iocharset=gb2312 %s %s' % (input_project_mnt_path, self.G_PATH_INPUTPROJECT_LINUX)
            input_project_mnt = 'mount -t cifs -o username=enfuzion,password=ruiyun2016 %s %s' % (input_project_mnt_path, self.G_PATH_INPUTPROJECT_LINUX)
            self.G_PROCESS_LOG.info(input_project_mnt.replace('enfuzion', '').replace('ruiyun2016', ''))
            self.RBcmd(input_project_mnt, False, 1)
            ...
            if isinstance(configJson["mntMap"],dict):
                ...
                for key in configJson["mntMap"]:
                    ...
                    if os.path.exists(mnt_value):
                        self.RBTry3cmd('net use "'+key+'" "'+mnt_value+'"',isContinue)
                    else:
    ...
    def RBrender(self):#7
        ...
        project_path = self.G_PATH_INPUTPROJECT
        if self.G_RENDEROS=='Linux':
            project_path = self.G_PATH_INPUTPROJECT_LINUX
        ...
        renderCmd=renderBat +' "'+self.G_USERID+'"  "'+self.G_TASKID+'"  "'+mayaExePath+'" "'+renderTxtFile+'" "'+self.G_CG_START_FRAME+'" "' + self.G_CG_END_FRAME +'" "'+self.G_CG_BY_FRAME +'" "'+ project_path +'" "'+ self.G_PATH_INPUTFILE+'" "'+ self.G_CONFIG+'" "'+ self.G_PLUGINS+'" "'+ self.G_CG_TILE+'" "'+ self.G_CG_TILECOUNT+'" '
        
        #os.system(renderCmd)
        self.RBRenderCmd(renderCmd,False,1,myLog=True)
        endTime = time.time()
        ...
        for parent,dirnames,filenames in os.walk(rootdir):
            for filename in filenames:
                if 'filtered' in filename:
                    print ""
                elif "variance" in filename : 
                    filePath=os.path.join(parent,filename)
                    
                    p = re.compile(r"[0-9]+(?=[^0-9]*$)", re.I)
                    nums= p.findall(filePath)
                    if nums:
                        num=nums[0]
                        ## key = filePath.replace(num,"###")
                        key = os.path.join(parent,filename.replace(num,"###"))
                        #print filePath
                        #print key
                        if fileobj.has_key(key):
                            fileobj[key]=fileobj[key]+','+num
                        else:
                            fileobj[key]=num
    ...
    def RBRenderCmd(self, cmdStr, continueOnErr=False, myShell=False, myLog=False):
        """
        Run render cmd.
        :param str cmdStr:
        :param bool continueOnErr: True:not exit / False:exit if run cmd error
        :param bool myShell:  True:use shell run cmd / False:not use shell run cmd
        :param bool myLog:  True:print info / False:not print info
        :return:
        """
        self.G_PROCESS_LOG.info('continueOnErr={}--->>>myShell={}--->>>myLog={}'.format(continueOnErr, myShell, myLog))
        if myLog:
            self.G_PROCESS_LOG.info('cmd...{}'.format(cmdStr))
        ...
        while True:
            ...
            if resultLine:  # not EOF
                resultLine = resultLine.strip()
                if resultLine:  # not empty line
                    if myLog:
                        self.G_PROCESS_LOG.info(resultLine)
                        
                    if "Exit Success" in resultLine:
                        break
                    elif "Exit Fail" in resultLine or \
                            "License error" in resultLine or \
                            "[mtoa] Failed batch render" in resultLine or \
                            "error checking out license for arnold" in resultLine or \
                            "No license for product (-1)" in resultLine:
                        sys.exit(-1)
            else:
                break
    ...
    def RBresultAction(self):
        ...
        if self.G_RENDEROS=='Linux':
            ...
            if self.G_CG_TILECOUNT !='1' and self.G_CG_TILECOUNT!=self.G_CG_TILE:
                ...
                outputPath = self.G_PATH_TILES.replace('\\','/').replace(self.G_PATH_INPUTPROJECT.replace('\\','/'), self.G_PATH_INPUTPROJECT_LINUX.replace('\\','/'))
            else:
                spPath = "outputdata5"
                ...
                outputmnt='mount -t cifs -o username=enfuzion,password=ruiyun2016 '+outputMntpath+' '+outputPath
                if not os.path.exists(outputPath):
                    os.makedirs(outputPath)
                self.G_PROCESS_LOG.info(outputmnt.replace('enfuzion', '').replace('ruiyun2016', ''))
            ...
            feemnt='mount -t cifs -o username=enfuzion,password=ruiyun2016 '+self.G_PATH_COST.replace('\\','/')+' '+feePath
            if not os.path.exists(feePath):
                os.makedirs(feePath)
            self.G_PROCESS_LOG.info(feemnt.replace('enfuzion', '').replace('ruiyun2016', ''))
            ...
            FrameChecker.main(self.G_RENDER_WORK_OUTPUT, outputPath, my_log=self.G_PROCESS_LOG)
    ...
    def copy_log(self):
        """copy log to user output"""
        self.G_PROCESS_LOG.info('[Maya.copy_log.start.....]')
        if self.G_RENDEROS=='Linux':
            user_output_log_dir = r'/output/%s/%s/render_log/%s' % (self.G_USERID_PARENT, self.G_USERID, self.G_TASKID)
            local_log_dir = os.path.join(self.G_LOG_WORK,self.G_TASKID)
            local_log_file = os.path.join(local_log_dir, self.G_JOB_NAME+'_render.log')

            if not os.path.exists(user_output_log_dir):
                try:
                    os.makedirs(user_output_log_dir)
            ...
            if os.path.exists(local_log_file):
                self.pythonCopy(local_log_file.replace('\\','/'), user_output_log_dir.replace('\\','/'), continueOnErr=True)
            else:
                self.G_PROCESS_LOG.info('[warn]%s is not exists.' % local_log_file)
        self.G_PROCESS_LOG.info('[Maya.copy_log.end.....]')
...
class MayaExpress(Maya):
    def RBrender(self):#7
        ...
        #os.system(renderCmd)
        self.RBRenderCmd(renderCmd,False,False,myLog=True)
        endTime = time.time()
        self.G_FEE_LOG.info('endTime='+str(int(endTime)))
        self.G_RENDER_LOG.info('[Maya.RBrender.end.....]')
