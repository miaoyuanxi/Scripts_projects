...
class MaxClient(RenderBase):
    ...
    def RBrenderConfig(self):#5
        ...
        #--------redshift test------------
        pluginsCfgDict = Common.GetPluginDict(pluginCfg)
        if pluginsCfgDict.has_key('plugins') and pluginsCfgDict['plugins'].has_key('redshift'):
            pluginName = u'redshift'
            pluginVersion = pluginsCfgDict['plugins']['redshift']
            localAutodesk=ur'C:/Program Files/Autodesk'
            ...
            cmdStrNotWait='call "'+pluginBatPathNotWait+'" "'+self.G_CG_VERSION+'" "'+pluginName+'" "'+pluginName+pluginVersion+'" "'+self.G_MAX_B+'"'
            print '********************************chen test*****************************'
            print cmdStr
            print cmdStrNotWait
            if os.path.exists(pluginBatPath):
                ...
                dstDir=os.path.join(localAutodesk,self.G_CG_VERSION).replace('\\','/')
        ...
        if self.MAX_VRAY_DISTRIBUTE:
            ...
            def check_vrayspawner_cmd(thread_name,node_ip):
                ...
                if self.G_PLATFORM == '1008':
                    ...
                    password = 'ruiyun2016'
                check_cmd = 'tasklist /S %s /U %s /P %s /FI "IMAGENAME eq vrayspawner*"' % (node_ip,username,password)
...
#        sys.exit(-1)
    ...
    def vrayDistributeNode(self):
        def send_cmd(thread_name,localIP,nodeIP):
            ...
            # self.G_PROCESS_LOG.info('*****************************************************') 
            # self.G_PROCESS_LOG.info(from_addr_user)
            # self.G_PROCESS_LOG.info(from_addr2_user)
            # self.G_PROCESS_LOG.info('*****************************************************')  
...
""" % (nodeIP_num)
                elif self.PLUGIN_DICT['plugins']['vray'].startswith('0'):
                    print 'vray0000\n'
                    content_bottom = """restart_slaves 1
list_in_scene 1
max_servers %s
