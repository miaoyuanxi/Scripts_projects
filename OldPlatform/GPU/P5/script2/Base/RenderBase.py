...
class RenderBase():
    ...
    def __init__(self,**paramDict):
        ...
        print paramDict
        if 'G_CG_TILE' in paramDict and paramDict['G_CG_TILE'] != '':
            self.G_CG_TILE=paramDict['G_CG_TILE']

        if 'G_CG_TILECOUNT' in paramDict and paramDict['G_CG_TILECOUNT'] != '':
            self.G_CG_TILECOUNT=paramDict['G_CG_TILECOUNT']

        self.G_SYS_ARGVS=paramDict['G_SYS_ARGVS']#taskid,jobindex,jobid,nodeid,nodename
        self.G_JOB_ID=self.G_SYS_ARGVS[2].replace("\"","")
        self.G_JOB_NAME=self.G_SYS_ARGVS[3].replace("\"","")
        self.G_NODE_NAME=self.G_SYS_ARGVS[5].replace("\"","")
        
        self.G_SCRIPT_START_TIME = paramDict.get('G_SCRIPT_START_TIME', str(int(time.time())))
...
#           os.system('xcopy /y /v /f  "\\\\10.50.1.3\\p\\script\\py\\cleanNode.py" "c:/script/py/"')
        ...
        self.killSomeProcess()


    def killSomeProcess(self):
        """Kill some process before rendering."""
        kill_list = []
        if self.G_RENDEROS=='Linux':
            kill_list.append('hython')
        else:
            kill_list.append('hython.exe')
            kill_list.append('vrayspawner.exe')

        for process_name in kill_list:
            if self.G_RENDEROS=='Linux':
                kill_cmd = r"ps -ef | grep -i %s | grep -v grep | awk '{print $2}' | xargs kill -9" % process_name
            else:
                kill_cmd = r'taskkill /F /IM %s /T' % process_name
            print kill_cmd
            os.system(kill_cmd)
    ...
    def RBinitLog(self):#2
        ...
        feeLogHandler=logging.FileHandler(feeTxt, encoding='utf-8')
    ...
    def RBwriteConsumeTxt(self):#6
        ...
        self.G_FEE_LOG.info('scriptStartTime={}'.format(self.G_SCRIPT_START_TIME))  # added in 20180723

        #get numbers of cpu cores
        self.G_PROCESS_LOG.info(self.G_RENDEROS)
        if self.G_RENDEROS == 'Windows':
            cpu_cores = 0
            get_cpu_cores_cmd = ur'wmic cpu get NumberOfCores'
            cmdp = subprocess.Popen(get_cpu_cores_cmd,stdin = subprocess.PIPE,stdout = subprocess.PIPE, stderr = subprocess.STDOUT)
            result_line = cmdp.stdout.readlines()
            self.G_PROCESS_LOG.info(result_line)
            for result in result_line:
                result = result.strip()
                self.G_PROCESS_LOG.info(result)
                if result != '' and result != 'NumberOfCores':
                    cpu_cores += int(result)
            # while cmdp.poll() == None:
                # result_line = cmdp.stdout.readline().strip()
                # self.G_PROCESS_LOG.info(result_line)
                # if result_line != '' and result_line != 'NumberOfCores':
                    # cpu_cores += int(result_line)
            self.G_FEE_LOG.info('cpu='+str(cpu_cores))
        elif self.G_RENDEROS == 'Linux':
            cpu_cores = 0
            get_cpu_cores_cmd = ur'cat /proc/cpuinfo | egrep "core id|physical id" | tr -d "\n" | sed s/physical/\\nphysical/g | grep -v ^$ | sort | uniq | wc -l'
            cmdp = subprocess.Popen(get_cpu_cores_cmd,stdin = subprocess.PIPE,stdout = subprocess.PIPE, stderr = subprocess.STDOUT,shell=True)
            while cmdp.poll() == None:
                result_line = cmdp.stdout.readline().strip()
                self.G_PROCESS_LOG.info(result_line)
                if result_line != '':
    ...
    def RBconvertDir(self,dir,workSmallPath):
        if  os.path.exists(dir):
            ...
            if self.G_KG=='1':
                smallSize='40'
            if self.G_CG_TILECOUNT and int(self.G_CG_TILECOUNT) > 1 and hasattr(self, "argsmap") and self.argsmap.get("from", None) in ["3dsmax"]:
                smallSize = '960'
            ...
            for root, dirs, files in list_dirs:
                for name in files:
                    ...
                    try:
                        ...
                        pass
                    self.G_PROCESS_LOG.info("workBigPicTmp={}\nworkBigPic={}\nworkSmallTmpPic={}\nworkSmallPic={}".format(workBigPicTmp, workBigPic, workSmallTmpPic, workSmallPic))
                    if not workBigPicTmp==workBigPic:
                        ...
                        if os.path.exists(workSmallTmpPic):
                            try:
                                ...
                                self.G_PROCESS_LOG.info("rename error")
                    ...
                    self.G_FEE_LOG.info('smallPic='+smallName.decode(sys.getfilesystemencoding()))
    ...
    def RBresultAction(self):
        ...
        if self.G_RENDEROS=='Linux':
            ...
            self.G_FEE_LOG.info('scriptEndTime={}'.format(str(int(time.time()))))  # added in 20180723
    ...
    def RBnodeClean(self):
        from NodeCleaner import NodeCleaner
        
        log_stay_hours = 30*24
        work_stay_hours = 7*24
        if self.G_RENDEROS == 'Linux':
            work_stay_hours = 3*24
        
        NC1=NodeCleaner(self.G_LOG_WORK, log_stay_hours, self.G_PROCESS_LOG)
        NC1.main()

        NC2=NodeCleaner(self.G_RENDER_WORK, work_stay_hours, self.G_PROCESS_LOG)
        NC2.main()

        NC3=NodeCleaner(self.G_HELPER_WORK, work_stay_hours, self.G_PROCESS_LOG)
        NC3.main()

        if self.G_RENDEROS == 'Windows':
            if "gpuid" not in os.environ:
                self.G_PROCESS_LOG.info(u"[delete Textures] 'gpuid' key is not found ")
                return

            log_path = r'D:\Temp\REDSHIFT'
            texture_dirs = [
                r'D:\Temp\REDSHIFT\CACHE\Redshift\Cache\Textures',
                r'D:\Temp\REDSHIFT\CACHE\G0\Redshift\Cache\Textures',
                r'D:\Temp\REDSHIFT\CACHE\G1\Redshift\Cache\Textures',
                # r'D:\Temp\REDSHIFT\CACHE\G0\G0\Redshift\Cache\Textures'
            ]

            NC4 = NodeCleaner(log_path, 7 * 24, self.G_PROCESS_LOG)
            NC4.clean_texture_cache_dirs(texture_dirs)

    def monitorCmd(self, command, resource_monitor):
        self.G_PROCESS_LOG.info(command)
        # register(self.monitorExit, resource_monitor)
        os.system(command)

    def monitorExit(self, resource_monitor):
        self.G_PROCESS_LOG.info('[monitorExit]start...')
        software_name = os.path.basename(resource_monitor)

        kill_cmd = ur'taskkill /F /IM %s /T' % software_name
        if self.G_RENDEROS == 'Linux':
            kill_cmd = ur"ps -ef | grep -i %s | grep -v grep | awk '{print $2}' | xargs kill -9" % software_name

        os.system(kill_cmd)
        self.G_PROCESS_LOG.info('[monitorExit]end...')

    def resourceMonitor(self):
        self.G_PROCESS_LOG.info('[BASE.resourceMonitor start]')
        #parse B:\config\cg_process.json --> <G_POOL>\tools\resource_monitor\config\cg_process.json 
        cg_process_dir = os.path.normpath(os.path.join(self.G_POOL, 'tools', 'resource_monitor', 'config'))
        cg_process_path = os.path.join(cg_process_dir, 'cg_process.json')
        resource_monitor = ur'C:\work\munu_client\resource_monitor\resource_monitor.exe'  # resource_monitor  #1
        if self.G_RENDEROS == 'Linux':
            resource_monitor = ur'/home/tools/munu_client/resource_monitor/resource_monitor'
            cg_process_path = os.path.join(cg_process_dir, 'cg_process_linux.json')

        if not os.path.exists(cg_process_path):
            self.G_PROCESS_LOG.info('[warn]%s is not exists!!!' % cg_process_path)
        else:
            cg_process_dict = eval(open(cg_process_path, 'r').read())

            #xxx.exe "3dsmax.exe|other.exe" "1-5|19126418|1868556|3dsmax|vray,multiscatter"
            if not os.path.exists(resource_monitor):
                self.G_PROCESS_LOG.info('[warn]%s is not exists!!!' % resource_monitor)
            else:
                if self.G_CG_NAME in cg_process_dict:
                    process_name_list = cg_process_dict[self.G_CG_NAME]
                    if len(process_name_list) <= 0:
                        self.G_PROCESS_LOG.info('[warn]The list of key %s in the %s is empty!!' % (self.G_CG_NAME, cg_process_path))
                    else:
                        ...
                        #self.G_TASKID  #4
                        #self.G_USERID  #5
                        cg_flag = os.path.splitext(process_name)[0]  #6
                        plugin_string = ur''

                        plugin_path = ur''
                        try:
                            plugin_path = self.G_PLUGINS  #plugins.json/plugins.cfg
                        except:
                            pass
                        if not plugin_path:
                            plugin_path = self.G_RAYVISION_PLUGINSCFG  #plugins.cfg

                        if os.path.exists(plugin_path):
                            config_dict = eval(open(plugin_path, 'r').read())
                            cg_flag = config_dict['renderSoftware'] + ' ' + config_dict['softwareVer']  #3ds Max 2014
                            if isinstance(config_dict['plugins'],dict):
                                for key,value in config_dict['plugins'].items():
                                    plugin_string += key+value+','
                                plugin_string = plugin_string.strip(',')

                        if not plugin_string:
                            plugin_string = ur'None'

                        # command = resource_monitor + ' "' + process_name + '" "' + render_frame + '|' + self.G_TASKID + '|' + self.G_USERID + '|' + cg_flag + '|' + plugin_string + '"'
                        command = ur'%s "%s" "jobId=%s|taskId=%s|userId=%s|software=%s|plugin=%s"' % (resource_monitor, process_name, self.G_JOB_ID, self.G_TASKID, self.G_USERID, cg_flag, plugin_string)
                        # self.G_PROCESS_LOG.info(command)
                        # os.system(command)
                        t = threading.Thread(target=self.monitorCmd,args=(command, resource_monitor,))
                        t.daemon = True
                        t.start()

        self.G_PROCESS_LOG.info('[BASE.resourceMonitor end]')
