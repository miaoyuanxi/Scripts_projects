...
if os.path.exists(r'c:\script\pySitePackages'):
...
class MaxClient(RenderBase):
    ...
    def readPyCfg(self):
        ...
        self.RBlog('user_pool='+self.G_POOL.replace('/','\\'))
        self.RBlog('plugin_path='+self.argsmap['pluginPath'].replace('/','\\'))
    ...
    def RBrender(self):#7
        ...
        myT= ThreadHelper(5,self.G_TASKID,self.G_JOB_NAME,self.G_NODE_NAME,self.G_RENDER_WORK_TASK,self.G_PATH_GRAB,self.G_PROCESS_LOG)
        myT.start()
...
class ThreadHelper(threading.Thread):
    def __init__(self,interval,taskId,jobName,nodeName,workTask,grabOutput,logObj):  
        ...
        self.grabImgName='grabImg1.2.exe'
    ...
    def grabPic(self,grapImg,workAreaArr):

        if self.checkProcess('3dsmax.exe'):
            if workAreaArr!=None and len(workAreaArr)==4:
                print 'grab  workarea'
                im=ImageGrab.grab(workAreaArr)
                im.save(grapImg)
            else:
                print 'grab global desk'
                im=ImageGrab.grab()
                im.save(grapImg)
    ...
    def grabRun(self,nodeName,output):
        try:
            workAreaArr=self.getWorkArea()
            self.RENDER_LOG.info(str(workAreaArr))
            if workAreaArr[2]>1023 :
                ...
                self.grabPic(grapImg,workAreaArr)
        except Exception, e:
