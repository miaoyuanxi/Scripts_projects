...
class Preprocess(RenderBase):
    ...
    def __init__(self,**paramDict):
        ...
        self.LOCAL_FOLDER=os.path.join(self.G_HELPER,self.G_TASKID,'max')
        self.G_HELPER_WORK_TASK_CFG=os.path.join(self.G_HELPER,self.G_TASKID,'cfg')
        print self.LOCAL_FOLDER
        self.tempFileFolder=os.path.join(self.G_POOL,'temp',(self.G_TASKID+'_render'),'max')
        self.packFileName=os.path.join(self.LOCAL_FOLDER,'max.7z');
        self.fileCount=0
        pass
    ...
    def PackFile(self):
        ...
        if os.path.exists(self.LOCAL_FOLDER):
            cmd=r'B:/tools/7-Zip/7z.exe a -t7z "'+self.LOCAL_FOLDER+'.7z" "'+self.LOCAL_FOLDER+'\*"  -mmt -r'
        ...
        if fileList:
            ...
            if skipUpload=="1":
                ...
                if preFileTxt=="-1":
                    ...
                    cmd=u'c:\\fcopy\\FastCopy.exe  /speed=full /force_close  /no_confirm_stop /force_start /srcfile="'+preFileTxt+'" /to="'+self.LOCAL_FOLDER.replace('/','\\')+'"'
                ...
                for files in fileList:
                    ...
                    if os.path.exists(filePath):
                        if self.isScriptUpdateOk(20160301):
                            ...
                            cmd=u'c:\\fcopy\\FastCopy.exe  /speed=full /force_close  /no_confirm_stop /force_start "'+filePath+'" /to="'+self.LOCAL_FOLDER.replace('/','\\')+filedir+'"'
                            #self.G_PROCESS_LOG.info('[PreprocessTasK.copyFile.cmd.....]'+cmd)
                        ...
                        self.G_PROCESS_LOG.info('[PreprocessTasK.copyFile.filePath.....]'+filePath+"not exists")
                        sys.exit(-1)
                        #break
    ...
    def RBresultAction(self):
        ...
        cmd='c:\\fcopy\\FastCopy.exe  /speed=full /force_close  /no_confirm_stop /force_start "'+self.LOCAL_FOLDER+'.7z" /to="'+os.path.join(self.G_TEMP,self.G_TASKID).replace('/','\\')+'"'
