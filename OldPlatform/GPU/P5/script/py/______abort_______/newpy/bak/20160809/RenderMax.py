...
class MaxClient(RenderBase):
    ...
    def RBhanFile(self):#3 copy script,copy from pool,#unpack
        ...
        if int(self.G_CG_TILECOUNT)>1 and self.G_CG_TILECOUNT==self.G_CG_TILE:#merge Pic
            ...
            blockPath2=self.G_PATH_USER_OUTPUT.replace('/','\\')
            self.G_PROCESS_LOG.info(blockPath2)
            if os.path.exists(blockPath2):
                copyBlockCmd='c:\\fcopy\\FastCopy.exe   /speed=full /force_close  /no_confirm_stop /force_start "'+blockPath2+'\\*.*" /to="'+self.G_RENDER_WORK_TASK_BLOCK.replace('/','\\')+'"'
                copyBlockCmd=copyBlockCmd.encode(sys.getfilesystemencoding())
                self.RBcmd(copyBlockCmd)
    ...
    def RBloadPlugin(self,customBat):#abort
        ...
        maxPlugin=MaxPlugin(self.G_PROCESS_LOG,self.maxB,self.maxPluginIniPath,self.G_CG_VERSION,self.localMaxPath,self.RBgetPluginDict(),self.G_PROGRAMFILES,self.G_RENDER_LOG)
        maxPlugin.config()
    ...
    def RBrenderConfig(self):#5
        ...
        #----------load max plugin----------
        maxPlugin=MaxPlugin(self.G_PROCESS_LOG,self.G_MAX_B,self.G_CG_VERSION,self.RBgetPluginDict(),self.G_PROGRAMFILES,self.G_RENDER_LOG)
        maxPlugin.config()
