...
class Houdini(RenderBase):
	def __init__(self,**paramDict):
		...
		print "Houdini INIT"
		self.G_RENDERBAT_NAME='Erender.py'
		self.CGV="";
		self.CGFILE="";
	...
	def RBrender(self):#7
		...
		houdiniScript='B:\\plugins\\houdini\\'
		renderPy=os.path.join(houdiniScript,self.G_RENDERBAT_NAME)
		...
		renderCmd='c:\\Python27\\python.exe '+renderPy +' "'+self.G_USERID+'"  "'+self.G_TASKID+'" "'+self.G_CG_START_FRAME+'" "' + self.G_CG_END_FRAME +'" "'+self.G_CG_BY_FRAME +'" "'+ self.CGFILE+'" "' +self.G_RENDER_WORK_OUTPUT+'"  "'+self.G_CG_LAYER_NAME+'" "'+self.G_RENDER_WORK_OUTPUT.replace('/','\\')+'" "'+self.G_CG_OPTION+'"' 
