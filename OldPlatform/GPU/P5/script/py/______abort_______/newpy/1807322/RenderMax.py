...
class ThreadHelper(threading.Thread):
    def __init__(self,interval,taskId,jobName,nodeName,workTask,grabOutput,logObj):  
        ...
        self.G_MAXB='B:/plugins/max'
        self.G_MAXSCRIPTPATH=self.G_MAXB+'/script'
        self.MAXSCRIPTNAME='renderu.ms'#max2013,max204,max2015
        self.PLUGINBATNAME='plugin.bat'
        self.MAXBATNAME='max.bat'
        self.CUSTOMBATNAME='custom.bat'
        self.USERHOSTNAME='hosts.txt'
        
        #2016-07-08quanshiyin-test---------------------------------------------------QSY-------------------------------------------
...
class MaxClient(RenderBase):
    def __init__(self,**paramDict):
        ...
        self.PLUGINS_MAX_SCRIPT='B:/plugins/max2/script'
    ...
    def RBloadPlugin(self,customBat):
        loadplugin=RBConfigPlugin()
        loadplugin.delete()
        loadplugin.getIncludeIni()
        loadplugin.getDirectoryIni()
        loadplugin.writeIni()
        loadplugin.copyConfigPlugin()
    ...
    def RBrenderConfig(self):#5
        ...
        #2016-07-08quanshiyin-test---------------------------------------------------QSY-------------------------------------------
        maxPluginIniPath=maxB+'/ini'
        localMaxPath='C:/Program Files/Autodesk'
        '''
        '''
        if self.isScriptUpdateOk(20151125):
            pass
        ...
        if self.G_CG_VERSION=='3ds Max 2012' or self.G_CG_VERSION=='3ds Max 2011' or self.G_CG_VERSION=='3ds Max 2010' or self.G_CG_VERSION=='3ds Max 2009':
            self.MAXSCRIPTNAME='rendera.ms'
        customerMs=os.path.join(customerScriptPath,self.MAXSCRIPTNAME).replace('\\','/')
        customerPluginBat=os.path.join(customerScriptPath,self.PLUGINBATNAME).replace('\\','/')
        customerMaxBat=os.path.join(customerScriptPath,self.MAXBATNAME).replace('\\','/')
        customBat=os.path.join(customerScriptPath,self.CUSTOMBATNAME).replace('\\','/')
        self.G_PROCESS_LOG.info('customBat------'+customBat)
        self.G_PROCESS_LOG.info('maxscriptName------'+self.MAXSCRIPTNAME)
        self.G_RAYVISION_MAXMS=os.path.join(self.G_MAXSCRIPTPATH,self.MAXSCRIPTNAME).replace('\\','/')
        if os.path.exists(customerMs):
            self.G_RAYVISION_MAXMS=customerMs
            
        self.G_RAYVISION_PLUGINBAT=os.path.join(self.G_MAXSCRIPTPATH,self.PLUGINBATNAME).replace('\\','/')
        if os.path.exists(customerPluginBat):
            self.G_RAYVISION_PLUGINBAT=customerPluginBat
        
        self.G_RAYVISION_MAXBAT=os.path.join(self.G_MAXSCRIPTPATH,self.MAXBATNAME).replace('\\','/')
        ...
        if os.path.exists(myTempVrayLog):
            try:
                ...
                self.G_PROCESS_LOG.info(e)
        try:
            maxIni=os.path.join(self.G_MAXB,'ini/3dsmax',self.G_CG_VERSION,'3dsmax.ini').replace('\\','/')
            ...
            maxUserIni=os.path.join(self.G_MAXB,'ini/3dsmax',self.G_USERID,self.G_CG_VERSION,'3dsmax.ini').replace('\\','/')
        ...
        userHostFile=os.path.join(customerScriptPath,self.USERHOSTNAME).replace('\\','/')
...
class RBConfigPlugin(MaxClient):
    def __init__(self,**paramDict):
        RenderBase.__init__(self,**paramDict)
        self.MAXPLUGININIPATH=self.G_MAXB+'/ini'
        self.LOCALMAXPATH='C:/Program Files/Autodesk'
        
    self.G_PROCESS_LOG.info('[Max.RVloadPlugin start]')
    self.G_PROCESS_LOG.info('\n\n------------------------------------------------------------[Start Config plugin]--------------------------------------------------------\n\n')
    
    def delete(self,*args):
        defDelPath=self.G_MAXB+ '/script/delFile.bat' 
    ...
    self.G_PROCESS_LOG.info('\n\n--------------------------------[Start config ini ]--------------------------------\n\n')
    def getIncludeIni(self,*args):
        self.G_PROCESS_LOG.info('getIncludeIni...start------>>>>>\n')
        standIni=os.path.join(self.MAXPLUGININIPATH,'standard',self.G_CG_VERSION,'plugin_include.ini').replace('\\','/')
        includeIniList=[]
        ...
        if allPlugDict and allPlugDict.has_key('plugins'):
            ...
            for pluginName in pluginDict:  
                pluginVersion=pluginDict[pluginName]
                pluginPath=os.path.join(self.MAXPLUGININIPATH,pluginName,pluginName+pluginVersion,self.G_CG_VERSION,'plugin_include.ini').replace('\\','/')
        ...
        for lncludeIni in includeIniList:
            self.G_PROCESS_LOG.info(lncludeIni+'\n')
        self.G_PROCESS_LOG.info('getIncludeIni...end')
        
    def getDirectoryIni(self,*args):
        self.G_PROCESS_LOG.info('getDirectoryIni...start------>>>>>\n')
        standIni=os.path.join(self.MAXPLUGININIPATH,'standard',self.G_CG_VERSION,'plugin_directory.ini')
        directoryIniList=[]
        if os.path.exists(standIni):
            f=open(standIni)
        ...
        if allPlugDict and allPlugDict.has_key('plugins'):
            ...
            for pluginName in pluginDict:  
                pluginVersion=pluginDict[pluginName]
                pluginPath=os.path.join(self.MAXPLUGININIPATH,pluginName,pluginName+pluginVersion,self.G_CG_VERSION,'plugin_directory.ini').replace('\\','/')
                if os.path.exists(pluginPath):
                    f=open(pluginPath)
                    sourcedirectory=f.read()
                    f.close()
        ...
        for directoryIni in directoryIniList:
            self.G_PROCESS_LOG.info(directoryIni+'\n')
        self.G_PROCESS_LOG.info('getDirectoryIni...end\n')
        
        
    def writeIni(self,*args):
        self.G_PROCESS_LOG.info('writeIni...start------>>>>>\n')
        includeIniList=self.getIncludeIni()
        directoryIniList=self.getDirectoryIni()
        targetPath=os.path.join(self.G_LOCALMAXPATH,self.G_CG_VERSION,'en-US','plugin.ini').replace('\\','/')
        if self.G_CG_VERSION=='3ds Max 2010' or self.G_CG_VERSION=='3ds Max 2011' or self.G_CG_VERSION=='3ds Max 2012':
            targetPath=os.path.join(self.G_LOCALMAXPATH,self.G_CG_VERSION,'plugin.ini').replace('\\','/') 
    ...
    def copyConfigPlugin(self,*args):
        self.G_PROCESS_LOG.info('\n\n--------------------------------[start copy plugin]--------------------------------\n\n')
        allPlugDict=self.RBgetPluginDict()
        if allPlugDict and allPlugDict.has_key('plugins'):
            pluginDict=allPlugDict['plugins']
            for pluginName in pluginDict:
                if pluginName=="vray":
                    pluginVersion=pluginDict[pluginName]
                    pluginBatPath=os.path.join(self.G_MAXB,pluginName,'script',pluginName + '.bat').replace('\\','/')
                    cmdDir='"'+pluginBatPath+'" "'+self.G_CG_VERSION+'" "'+pluginName+'" "'+pluginName+pluginVersion+'" "'+self.G_MAXB+'"'
                    ...
                    if os.path.exists(pluginBatPath):
                        ...
                        srcDir=os.path.join(self.G_MAXB,pluginName,pluginName+pluginVersion,self.G_CG_VERSION).replace('\\','/')
                        dstDir=os.path.join(self.G_LOCALMAXPATH,self.G_CG_VERSION).replace('\\','/')
