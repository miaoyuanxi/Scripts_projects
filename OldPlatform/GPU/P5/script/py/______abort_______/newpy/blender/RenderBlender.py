...
class Blender(RenderBase):
	...
	def RBrender(self):#7
		...
		exePath=os.path.join(r'B:/plugins/blender/Blender Foundation',myCgVersion,r'blender.ex')
		output=self.G_RENDER_WORK_OUTPUT.replace("/","\\")
		
		renderCmd='"'+exePath+'" -b "'+self.cgfile+'" -o "'+output+'" -P '+checkPy+' -f "'+self.G_CG_START_FRAME+'" -- "'+self.analyseTxt.replace("mnt","/10.50.244.116").replace("/","\\")+'"'
