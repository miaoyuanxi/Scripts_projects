...
class MaxClient(RenderBase):
    ...
    def readPyCfg(self):
        ...
        self.argsmap={}
        try:
            line=self.readFileByCode(self.G_CFG_PYNAME,'UTF-8')
        except Exception, e:
            try:
                line=self.readFileByCode(self.G_CFG_PYNAME,'gbk')
            except Exception, e:
                line=self.readFileByCode(self.G_CFG_PYNAME,'UTF-16')
        for l in line:
            if "=" in l:
                params=l.split("=")
    ...
    def RBrender(self):#7
        ...
        #projectPath=os.path.join(inputDataPath,self.G_USERID_PARENT,self.G_USERID,self.RENDER_CFG_PARSER.get('common','projectSymbol'),'max')
        #projectPath=projectPath.replace('/','\\')
        #cgFile=os.path.join(projectPath,cgFileName).replace('\\','/')
        '''
        if self.isScriptUpdateOk(20151125):
            cgFile=os.path.join(self.G_RENDER_WORK_TASK_MAX,cgFileName).replace('\\','/')
        '''
