...
def get_platfom(platform):
    ...
    if platform == 1002:
        ...
        info["cfg_path"] = r"\\10.50.19.150\d\ninputdata5"
        info["home_path"] = r""
...
def get_json(**kwargs):
    ...
    if kwargs["task_id"]:
        ...
        options["common"]["plugin_file"] = os.path.join(cfg_path, "plugins.cfg")
...
#        return subprocess.call(command, shell=1)
    ...
    def clean_network(self):
        ...
        print "clean vcfs"
        print "net stop vcfsservice"
        sys.stdout.flush()
        self.run_command("net stop vcfsservice")
        sys.stdout.flush()
    ...
    def mapping_network(self):
        for i in self["mount"]:
            if i == "vcfs":
                print "net start vcfsservice /path=c:/vcfsclient " \
                    "/conf=vcfs.conf /letter=%s" % (self["mount"][i])
                sys.stdout.flush()
                if self.run_command("net start vcfsservice /path=c:/vcfsclient "
                    "/conf=vcfs.conf /letter=%s" % (self["mount"][i])):
                    sys.stdout.flush()
...
class MayaRender(MayaClass):
    ...
    def render(self):
        ...
        cmd = "\"%(render.exe)s\" -s %(start)s -e %(end)s -b %(by)s " \
            "-proj \"%(projectPath)s\" -rd \"%(output)s\" -mr:art -mr:aml" \
            % self["renderSettings"]
        ...
        if self["common"]["submitFrom"] == "client":
            if self["common"]["userId"] in [962539, 962796]:
