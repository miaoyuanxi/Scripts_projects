...
class Base():
	...
	def RBmapNetwork(self):
		self.G_RENDER_LOG.info('[BASE.RBmapNetwork.start.....]')
		self.G_RENDER_LOG.info('[BASE.RBmapNetwork.end.....]')
		
	#-----------------temp-----------------
	def RBcmd(self,renderCmd,continueOnErr=False,myShell=False):#continueOnErr=true-->not exit ; continueOnErr=false--> exit 
        cmdp=subprocess.Popen(renderCmd,shell = True,stdout = subprocess.PIPE, stderr = subprocess.STDOUT)
        while True:
            buff = cmdp.stdout.readline()
            if buff == '' and cmdp.poll() != None:
                break
            #self.G_PROCESS_LOGGER.info(buff)
            self.G_RENDER_LOG.info(buff)
        resultCode = cmdp.returncode
        resultStr = cmdp.stdout.read()
        self.G_RENDER_LOG.info('resultStr...'+str(resultStr))
...
class MaxClient(Base):
	...
	def RBrenderConfig(self):#5
		...
		maxBatName='max.bat'
		pluginPool=r'\\10.50.8.16\td\SOFT\B'
		#-----------------------------------
    ...
    def writeMsFile(self,sceneFile,renderFrame,mySonid):
		...
		msFile=os.path.join(self.G_RENDER_WORK_TASK_CFG,self.'render.ms')
		print msFile
		...
		mystr='rvRender "'+self.G_USERID+'" "'+self.G_TASKID+'" "'+mySonid+'" "'+renderFrame+'" "'+self.G_KG+'" "'+self.G_JOB_NAME+'" "'+sceneFile+'"\r\n'
	...
	def RBrender(self):#7
		...
		if self.G_CG_VERSION=='3ds Max 2010':
			...
			cmdStr = maxRenderExe+' -silent  -mxs "filein \\"'+self.G_RAYVISION_MAXMS+'\\";rvRender \\"'+self.G_USERID+'\\" \\"'+self.G_TASKID+'\\" \\"'+mySonid+'\\" \\"'+self.G_CG_START_FRAME+'\\" \\"'+self.G_KG+'\\" \\"'+self.G_JOB_NAME+'\\" \\"'+cgFile+'\\" "'
		
		self.RBcmd(cmdStr.decode('utf-8').encode('gbk'))
