...
class MaxClient(RenderBase):
    ...
    def RBhanFile(self):#3 copy script,copy max ,texture,cache,vrmap,#unpack
        ...
        pluginPath=self.argsmap['pluginPath'].replace("10.60.100.151","10.60.100.152")
        ...
        try:
            ...
            pass
        grab=os.path.join(pluginPath,'tools','grab')
        copyGrabCmd=r'c:\fcopy\FastCopy.exe /speed=full /force_close /no_confirm_stop /force_start "'+grab.replace('/','\\')+'" /to="c:\\"'
        self.RBlog(copyGrabCmd)
        self.RBcmd(copyGrabCmd)
        
        grabServer=r'start c:/grab/grab_service.exe'
        self.G_PROCESS_LOG.info(grabServer)
        os.system(grabServer)
    ...
    def writeMsFile2(self,msFile,sceneFile,renderOutput,renderFrame,notRender):
        ...
        clientArrayStr=clientArrayStr+self.G_JOB_NAME+'","'+renderOutput+'/","'+self.G_PLATFORM+'","'+self.G_CG_OPTION+'","'+self.CURRENT_TASK+'","client")\r\n'
        ...
        if self.argsmap.has_key('from') and self.argsmap['from']=='web':
            ...
            webArrayStr=webArrayStr+self.G_JOB_NAME+'","'+renderOutput+'/","'+self.G_PLATFORM+'","'+self.G_CG_OPTION+'","'+self.CURRENT_TASK+'","web","'+sceneFile+'") \r\n'
    ...
    def socketSend(self,finalData):
        try:
            ...
            self.G_PROCESS_LOG.info(self.HOST+'_____'+recvData.decode('utf8'))
    ...
    def startGrab(self):
        ...
        startSocket='rdf1grab|'+grabRate+'|'+grabFolderLocal+'|'+grabFolderServer+'|'+self.G_JOB_NAME+'|'+self.G_NODE_NAME
    ...
    def RBrender(self):#7
        ...
        monitorMaxT.start()
        monitorLogT= MonitorLog(180,self.G_TASKID,self.G_CG_VERSION,self.G_PROCESS_LOG,self.G_LOG_WORK,self.G_JOB_NAME)
