...
class Maya(AnalysisBase):
	...
	def webNetPath(self):
		if os.path.exists(self.G_CONFIG):
			...
			#ip="10.50.101."+str(random.choice([201,202,203,204,201,202,203,204,201,202,203,204,201,202,203,204,205,206,207]))
			print configJson
			if isinstance(configJson["mntMap"],dict):
				self.RBcmd("net use * /del /y")
				for key in configJson["mntMap"]:
					#self.RBcmd("net use "+key+" "+configJson["mntMap"][key])
					path = configJson["mntMap"][key]
					#path=path.replace("10.50.101.201",ip)
					self.RBcmd("net use "+key+" "+path)		
