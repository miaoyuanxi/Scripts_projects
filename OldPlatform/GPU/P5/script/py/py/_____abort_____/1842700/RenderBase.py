...
#           os.system('xcopy /y /v /f  "\\\\10.50.1.3\\p\\script\\py\\cleanNode.py" "c:/script/py/"')
            ...
            for root, dirs, files in list_dirs: 
                ...
                for name in files:
                    ...
                    if ext=='.exr' and os.path.exists(oiioPath):
                        ...
                        convertCmd=oiioPath+' "'+workBigPic+'" -resize 200x200 -o "'+workSmallPic+'"'
                    #print workBigPic
    ...
    def checkResult(self):
        ...
        try:
            ...
            renderResult=checkObj.checkFrames(nodeOutput,serverOutput,'checkcopy')
            renderWarn=checkObj.checkFrames(nodeOutput,serverOutput,'checkwarning')
            self.G_PROCESS_LOG.info("\nFinal chech result: "+str(renderResult))
            self.G_PROCESS_LOG.info("\nFinal chech warn: "+str(renderWarn))
            if renderWarn==1:
                self.G_FEE_LOG.info('jobStatus=picException')#jobStatus=picException|timeException
            else:
                self.G_FEE_LOG.info('jobStatus=')
        except Exception, e:
    ...
    def RBcmd(self,cmdStr,continueOnErr=False,myShell=False):#continueOnErr=true-->not exit ; continueOnErr=false--> exit 
        ...
        if not continueOnErr:
            if resultCode!=0:
                self.G_PROCESS_LOG.info('cmd...exit'+str(resultCode))
                sys.exit(resultCode)
        return resultStr
    ...
    def copyConsumeTxt(self):
        self.G_PROCESS_LOG.info('[BASE.consumTxt.copy.start.....]')
        feeLogFile=self.G_USERID+'-'+self.G_TASKID+'-'+self.G_JOB_NAME+'.txt'
        feeTxt=os.path.join(self.G_RENDER_WORK_TASK,feeLogFile)
        if self.G_RENDEROS=='Linux':
            feePath="/fee"
            ...
            cmd4='xcopy /y /f "'+feeTxt+'" "'+self.G_PATH_COST.replace('/','\\')+'/" '
            self.RBcmd(cmd4)
        self.G_PROCESS_LOG.info('[BASE.consumTxt.copy.end.....]')
