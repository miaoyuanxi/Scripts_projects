...
class Max(AnalysisBase):
    def __init__(self,**paramDict):
        ...
        self.CODE_PC_MISSING='111'
        self.CODE_GAMMA_ON='112'
    ...
    def maxfind(self):
        ...
        if self.G_MAXFIND_RENDERER==None or self.G_MAXFIND_MAX_VERSION_STR==None:
            ...
            tipStr='{'+str(self.ERRCODE_NO_INFO)+':[]}'
    ...
    def hanTips(self):
        ...
        if self.ANALYSE_TXT_PARSER.has_option('common','filegamma'):
            ...
            if gamma!=None and gamma.strip()=='gamma':
                gammaTip='"'+self.CODE_GAMMA_ON+'":['+'],\r\n'
                tipStr=tipStr+gammaTip
            
        #------------missing-----------
        pcMissing=self.getMissingCodeInfo(self.CODE_PC_MISSING,'pointcache_missing')
        
        tipStr=tipStr+pcMissing
        tipStr=tipStr+'\r\n}'
    ...
    def checkListFileExists(self, file,list,missingList):
    
        convertedFile=self.renderbusPathObj.convertPath(file)
        list.append(convertedFile)
        if not os.path.exists(convertedFile):
            missingList.append(file)
        
    def filterAsset2(self,allAssetsDict):
        resultDict={}
        
        for key in allAssetsDict:
            assetList = allAssetsDict[key]
            ...
            if key == 'fumefx':
                ...
                for asset in assetList:
                    ...
                    for i in range(start , end+1, -1 if start > end else 1):
                        ...
                        convertedFile=self.renderbusPathObj.convertPath(file)
                        fumefxList.append(convertedFile)
                        if not os.path.exists(convertedFile):
                            missingFumefxList.append(file)
                             
                resultDict[key]=fumefxList
                resultDict[key+'_missing']=missingFumefxList#self.checkListFileExists(fumefxList)
            elif key == 'realflow':
                realflowList=[]
                missingRealflowList=[]
                for asset in assetList:
                    ...
                    #realflowList.append(self.renderbusPathObj.convertPath(file))
                    convertedFile=self.renderbusPathObj.convertPath(file)
                    realflowList.append(convertedFile)
                    if not os.path.exists(convertedFile):
                        missingRealflowList.append(file)
                    
                        
                resultDict[key]=realflowList
                resultDict[key+'_missing']=missingRealflowList#self.checkListFileExists(realflowList)
            elif key == 'vrmesh':
                vrmeshList=[]
                missingVrmeshList=[]
                for asset in assetList:
                    ...
                    if wildPath.endswith('.vrmesh'):
                        ...
                        #vrmeshList.append(filePath)
                        self.checkListFileExists(filePath,vrmeshList,missingVrmeshList)                        
                    else:
                        print '#########################################################'
                        #self.G_ANALYSE_LOG.error('[Max vrmesh invalid: %s]' % wildPath)
                resultDict[key]=vrmeshList
                resultDict[key+'_missing']=missingVrmeshList
                
            elif key == 'pointcache':
                pointcacheList=[]
                missingPointcacheList=[]
                for asset in assetList:
                    
                    #pointcacheList.append(wildPath)
                    self.checkListFileExists(asset,pointcacheList,missingPointcacheList) 
                    ...
                    if wildPath.endswith('.xml') and os.path.exists(wildPath):
                        ...
                        if cacheType == 'OneFilePerFrame':                        
                            for i in range(1, cacheCount + 1):
                                ...
                                pointcacheList.append(cacheFile)
                                if not os.path.exists(cacheFile):
                                    missingPointcacheList.append(cacheFile)
                                #pointcacheList.append(cacheFile) 
                                
                        elif cacheType == 'OneFile':
                            cacheFile = wildPath.replace('.xml', suffix)
                            #pointcacheList.append(cacheFile)
                            #self.checkListFileExists(cacheFile,pointcacheList,missingPointcacheList) 
                            pointcacheList.append(cacheFile)
                            if not os.path.exists(cacheFile):
                                missingPointcacheList.append(cacheFile)
                resultDict[key]=pointcacheList
                resultDict[key+'_missing']=missingPointcacheList#self.checkListFileExists(pointcacheList)
            elif key == 'texture':
                tetureList=[]
                for asset in assetList:
                    tetureList.append(self.renderbusPathObj.convertPath(asset))   
                resultDict[key]=tetureList
                #resultDict[key+'_missing']=self.checkListFileExists(tetureList)
    ...
    def parserSetList(self,resultKey,list):
        count=1
        for item in list:
            self.ANALYSE_TXT_PARSER.set(resultKey, ('path' + str(count)), item)
            count=count+1
    def getAllAsset(self):
        ...
        resultDict=self.filterAsset2(assetsDict)
        
        assetInputList=[]
        assetInputMissingList=[]
        for resultKey in resultDict:
            if not self.ANALYSE_TXT_PARSER.has_section(resultKey):
                self.ANALYSE_TXT_PARSER.add_section(resultKey)
            if  resultKey.endswith('_missing'):
                assetInputMissingList.extend(resultDict[resultKey])
                self.parserSetList(resultKey,resultDict[resultKey])
            else:
                assetInputList.extend(resultDict[resultKey])
        if not self.ANALYSE_TXT_PARSER.has_section('asset_input'):
            self.ANALYSE_TXT_PARSER.add_section('asset_input')
        if not self.ANALYSE_TXT_PARSER.has_section('asset_input_missing'):
            self.ANALYSE_TXT_PARSER.add_section('asset_input_missing')
			
        self.parserSetList('asset_input',assetInputList)
        self.parserSetList('asset_input_missing',assetInputMissingList)
        ...
        if not self.ANALYSE_TXT_PARSER.has_section('texture_missing'):
            self.ANALYSE_TXT_PARSER.add_section('texture_missing')
        if not self.ANALYSE_TXT_PARSER.has_section('vrmesh_missing'):
            self.ANALYSE_TXT_PARSER.add_section('vrmesh_missing')
        if not self.ANALYSE_TXT_PARSER.has_section('alembic_missing'):
            self.ANALYSE_TXT_PARSER.add_section('alembic_missing')
        if not self.ANALYSE_TXT_PARSER.has_section('fumefx_missing'):
            self.ANALYSE_TXT_PARSER.add_section('fumefx_missing')
        if not self.ANALYSE_TXT_PARSER.has_section('pc2_missing'):
            self.ANALYSE_TXT_PARSER.add_section('pc2_missing')
        if not self.ANALYSE_TXT_PARSER.has_section('pointcache_missing'):
            self.ANALYSE_TXT_PARSER.add_section('pointcache_missing')
        if not self.ANALYSE_TXT_PARSER.has_section('realflow_missing'):
            self.ANALYSE_TXT_PARSER.add_section('realflow_missing')
        if not self.ANALYSE_TXT_PARSER.has_section('ies_missing'):
            self.ANALYSE_TXT_PARSER.add_section('ies_missing')
        if not self.ANALYSE_TXT_PARSER.has_section('xrefs_missing'):
            self.ANALYSE_TXT_PARSER.add_section('xrefs_missing')
        ...
        for item in self.inputMissingItems:
            if os.path.exists():
            self.ANALYSE_TXT_PARSER.set('texture_missing', ('path' + str(self.inputMissingCount)), item)
            self.inputMissingCount = self.inputMissingCount + 1
            
        
        for item in self.inputMissingItems:
            self.ANALYSE_TXT_PARSER.set('vrmesh_missing', ('path' + str(self.inputMissingCount)), item)
            self.inputMissingCount = self.inputMissingCount + 1
        for item in self.inputMissingItems:
            self.ANALYSE_TXT_PARSER.set('alembic_missing', ('path' + str(self.inputMissingCount)), item)
            self.inputMissingCount = self.inputMissingCount + 1
        for item in self.inputMissingItems:
            self.ANALYSE_TXT_PARSER.set('fumefx_missing', ('path' + str(self.inputMissingCount)), item)
            self.inputMissingCount = self.inputMissingCount + 1
        for item in self.inputMissingItems:
            self.ANALYSE_TXT_PARSER.set('pc2_missing', ('path' + str(self.inputMissingCount)), item)
            self.inputMissingCount = self.inputMissingCount + 1
        for item in self.inputMissingItems:
            self.ANALYSE_TXT_PARSER.set('pointcache_missing', ('path' + str(self.inputMissingCount)), item)
            self.inputMissingCount = self.inputMissingCount + 1
        for item in self.inputMissingItems:
            self.ANALYSE_TXT_PARSER.set('realflow_missing', ('path' + str(self.inputMissingCount)), item)
            self.inputMissingCount = self.inputMissingCount + 1
        for item in self.inputMissingItems:
            self.ANALYSE_TXT_PARSER.set('ies_missing', ('path' + str(self.inputMissingCount)), item)
            self.inputMissingCount = self.inputMissingCount + 1
        for item in self.inputMissingItems:
            self.ANALYSE_TXT_PARSER.set('xrefs_missing', ('path' + str(self.inputMissingCount)), item)
            self.inputMissingCount = self.inputMissingCount + 1
        '''
