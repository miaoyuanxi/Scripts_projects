...
class RenderbusPath():
    ...
    def convertPath(self,itemPath):
        ...
        if self.InterPath(src_max_lowercase):
            ...
            resultMaxFile= self.G_INPUT_USER + '/__'+ start.replace('//', '')+ rest.replace('\\', '/') 
        else:
            ...
            if key == 'fumefx':
                ...
                for asset in assetList:
                    ...
                    if type=='firesmokesim':
                        ...
                        for i in range(start , end+1, -1 if start > end else 1):
                            if i >= 0 :
                                file = inputFile[:pos] + '%04d' % i + '.aur'
                            else:
                                file = inputFile[:pos]+'_' + str(i) + '.aur'
                            resultList.append(self.renderbusPathObj.convertPath(file)) 
                    else:
                        for i in range(start , end+1, -1 if start > end else 1):
                        
                            if i >= 0 and inputFile.endswith('####.aur'):
                                file = inputFile[:pos]  + '%04d' % i + '.aur'
                            else:
