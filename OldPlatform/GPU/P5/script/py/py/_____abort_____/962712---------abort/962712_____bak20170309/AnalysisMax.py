...
class Max(AnalysisBase):
    def __init__(self,**paramDict):
        ...
        self.G_MAXFIND_FRAMES=None
        self.G_MAXFIND_FRAME_BY=None
    ...
    def maxfind(self):
        ...
        for maxInfoLine in propertyList:
            ...
            elif maxInfoLine.startswith('\tRender Camera '):#default is 0.00
                pos=maxInfoLine.find('=')+1
                renderCamera=maxInfoLine[pos:].replace('\r','').replace('\n','')
                self.MAXFIND_CAMERA_LIST.append(renderCamera)
            elif maxInfoLine.startswith('\tRender Width='):#default is 0.00
                renderWidth=maxInfoLine.replace('\tRender Width=','').replace('\r','').replace('\n','')
                self.G_MAXFIND_WIDTH=renderWidth
            elif maxInfoLine.startswith('\tRender Height='):#default is 0.00
                renderHight=maxInfoLine.replace('\tRender Height=','').replace('\r','').replace('\n','')
                self.G_MAXFIND_HEIGHT=renderHight
            ...
            elif maxInfoLine.startswith('\tAnimation Start='):#default is 0.00
                renderStart=maxInfoLine.replace('\tAnimation Start=','').replace('\r','').replace('\n','')
                self.G_MAXFIND_FRAME_START=renderStart.strip()
            elif maxInfoLine.startswith('\tAnimation End='):#default is 0.00
                renderEnd=maxInfoLine.replace('\tAnimation End=','').replace('\r','').replace('\n','')
                self.G_MAXFIND_FRAME_END=renderEnd.strip()
            elif maxInfoLine.startswith('\tFrames='):#default is 0.00
                renderBy=maxInfoLine.replace('\tNth Frame=','').replace('\r','').replace('\n','')
                self.G_MAXFIND_FRAME_BY=renderBy.strip()
            elif maxInfoLine.startswith('\tNon Sequential Frames='):#default is 0.00
                renderFrames=maxInfoLine.replace('\tNon Sequential Frames=','').replace('\r','').replace('\n','')
                self.G_MAXFIND_FRAMES=renderFrames.strip()
        
      
        
        print '------++++++++'
        print self.G_MAXFIND_RENDERER
        print self.G_MAXFIND_MAX_VERSION_STR
    ...
    def configMaxPlugin(self):#config 3ds Max ,vray,plugin...and so on
        ...
        self.G_ANALYSE_LOG.info('[max.configMax.start_____.....]')
        
        self.PLUGIN_DICT={}
        self.G_ANALYSE_LOG.info('[max.configMax.start2.....]')
        print '--------..plugin........'
    ...
    def RBconfig(self):#5
        ...
        analyseModeTxt=self.G_MAXSCRIPT+'/user/'+self.G_USERID+'/fastanalyse.txt'
        #if self.G_USERID=='962712' or self.G_USERID=='1301040':
    ...
    def writeTipsList(self):
        self.G_ANALYSE_LOG.info('------------------------------ write tips list---------------------------------') 
        tipStr='{\r\n'
    ...
    def getMissingCodeInfo(self,infoCode,infoType):
        self.G_ANALYSE_LOG.info('------------------------------ getMissingCodeInfo---------------------------------') 
        self.G_ANALYSE_LOG.info(infoCode)
        self.G_ANALYSE_LOG.info(infoType) 
    ...
    def hanTips(self):
        self.G_ANALYSE_LOG.info('------------------------------ hand ms  tips---------------------------------') 
        #------------ms tips-----------
        ...
        self.G_ANALYSE_LOG.info('------------------------------ hand tips gamma---------------------------------') 
        #------------gamma-----------
        ...
        self.G_ANALYSE_LOG.info('------------------------------ han tips vfb---------------------------------') 
        ...
        self.G_ANALYSE_LOG.info('------------------------------ han tips gi---------------------------------') 
        #--------------------check gi------------------- 
        ...
        #------------missing-----------
        self.G_ANALYSE_LOG.info('------------------------------ check missing---------------------------------') 
    ...
    def getFileFromInputByName(self,file):
        ...
        if fileBaseName in self.ASSET_INPUT_KEY_LIST:
            fileFromInput=self.ASSET_INPUT_DIR_DICT[fileBaseName]
        return fileFromInput
        
    def getPointCacheList(self,assetFile,resultList):
        print '-------get point cache ---------'
        wildPath=assetFile
        if self.ASSET_WEB_COOLECT_BY_PATH:
            wildPath2 = self.renderbusPathObj.convertPath(assetFile)
            resultList.append(wildPath2)
        else:
            resultList.append(wildPath)
            wildPath2=self.getFileFromInputByName(assetFile)
        print wildPath2
        if wildPath.endswith('.xml') and wildPath2!=None and os.path.exists(wildPath2):
            
            root = ET.parse(wildPath2).getroot()
            ...
            if cacheType == 'OneFilePerFrame':                        
                for i in range(cacheStart, cacheEnd + 1):
                    cacheFile = os.path.splitext(wildPath2)[0] + 'Frame' + str(i) + suffix
                    if self.ASSET_WEB_COOLECT_BY_PATH:
                        cacheFile2 = self.renderbusPathObj.convertPath(cacheFile)
                    else:
                        cacheBaseName=os.path.basename(cacheFile)
                        cacheFile2=os.path.join(os.path.dirname(wildPath),cacheBaseName)
                    resultList.append(cacheFile2) 
                    
            elif cacheType == 'OneFile':
                cacheFile = wildPath2.replace('.xml', suffix)
                if self.ASSET_WEB_COOLECT_BY_PATH:
                    cacheFile2 = self.renderbusPathObj.convertPath(cacheFile)
                else:
                    cacheFile2=wildPath.replace('.xml', suffix)
                resultList.append(cacheFile2)
    ...
    def gatherAssetByFilePath(self,assetType,assetList):
        self.G_ANALYSE_LOG.info('[-----------------max.RBanalyse.gatherAssetByFilePath]')
    ...
    def gatherAssetByFileName(self,assetType,assetList):
        self.G_ANALYSE_LOG.info('[-----------------max.RBanalyse.gatherAssetByFileName]')
        self.G_ANALYSE_LOG.info(assetType)
        missingCount = 1
        ...
        for asset in assetList:
            ...
            #assetInputList= self.ASSET_INPUT_DIR_DICT.keys()
            if  assetName  in self.ASSET_INPUT_KEY_LIST:
    ...
    def getAllAsset(self):
        ...
        self.G_ANALYSE_LOG.info('------------------------------ get all asset end---------------------------------')
    ...
    def loopInputAsset(self):
        ...
        self.ASSET_INPUT_KEY_LIST= self.ASSET_INPUT_DIR_DICT.keys()
        
    def RBhanResult(self):#8
        self.G_ANALYSE_LOG.info('------------------------------ RBhanResult--------------------------------')
    ...
    def RBhanFastResult(self):#8
        self.G_ANALYSE_LOG.info('------------------------------ RBhanFastResult--------------------------------')
        
        self.ANALYSE_TXT_PARSER=ConfigParser.ConfigParser()
        #self.ANALYSE_TXT_PARSER.readfp(codecs.open(self.G_ANALYSE_TXT_NODE, "r", "UTF-16"))
        if not self.ANALYSE_TXT_PARSER.has_section('common'):
            self.ANALYSE_TXT_PARSER.add_section('common')
        self.ANALYSE_TXT_PARSER.set('common', 'kg', '0')
        self.ANALYSE_TXT_PARSER.set('common', 'cgv', self.G_CG_VERSION)
        
        #self.ANALYSE_TXT_PARSER.set('common', 'rendtimetype', '4')
        
          
        
        
        if self.G_MAXFIND_FRAMES==None or  self.G_MAXFIND_FRAMES.strip()=='':
            if self.G_MAXFIND_FRAME_START==self.G_MAXFIND_FRAME_END:
                self.G_MAXFIND_FRAMES=self.G_MAXFIND_FRAME_START
            else:
                self.G_MAXFIND_FRAMES=self.G_MAXFIND_FRAME_START+'-'+self.G_MAXFIND_FRAME_END
            if self.G_MAXFIND_FRAME_BY!=None and self.G_MAXFIND_FRAME_BY.strip()!='':
                by=int(self.G_MAXFIND_FRAME_BY)
                if by>1:
                    self.G_MAXFIND_FRAMES=self.G_MAXFIND_FRAMES+'['+self.G_MAXFIND_FRAME_BY+']'
        
        self.ANALYSE_TXT_PARSER.set('common', 'frames', self.G_MAXFIND_FRAMES)
        #self.ANALYSE_TXT_PARSER.set('common', 'animationrange', '0-15')
        self.ANALYSE_TXT_PARSER.set('common', 'width', self.G_MAXFIND_WIDTH)
        self.ANALYSE_TXT_PARSER.set('common', 'height', self.G_MAXFIND_HEIGHT)
        self.ANALYSE_TXT_PARSER.set('common', 'rendoutputfilename', 'renderbus.tga')
        self.ANALYSE_TXT_PARSER.set('common', 'rendoutputfilebasename', 'renderbus')
        self.ANALYSE_TXT_PARSER.set('common', 'rendoutputfiletype', 'tga')
        self.ANALYSE_TXT_PARSER.set('common', 'analysemode', 'fast')
        camStr=''
        for cam in self.MAXFIND_CAMERA_LIST:
            camStr= camStr+cam+'[,]'
        camStr=camStr.strip('[,]')
        self.ANALYSE_TXT_PARSER.set('common', 'allcamera', camStr)
