...
class Max(AnalysisBase):
    ...
    def checkVrmesh(self, asset):
        if asset.endswith('.vrmesh'):
            #Missing vrmesh suffix
            assetList = []
            for vrmeshAsset in vrmeshFile:
                if not os.path.exists(asset):
                    print 'error code...'
                else:
                    vrmeshFile = self.renderbusPathObj.convertPath(asset)
                    print vrmeshFile
                return 0
    ...
    def copyTipTxt(self):
        if os.path.exists(self.G_TIPS_TXT_NODE):
            tipTxtCmd = 'xcopy /y /f "' + os.path.normpath(self.G_TIPS_TXT_NODE) + '"' + ' ' + '"' + os.path.normpath(self.G_POOL_CONFIG) + '\\"'
            print tipTxtCmd
    ...
    def RBhanResult(self):#8
        ...
        analyseTxtCmd = 'xcopy /y /f "' + os.path.normpath(self.G_ANALYSE_TXT_NODE) + '"' + ' ' + '"' + os.path.normpath(self.G_POOL_CONFIG) + '\\"'
        print analyseTxtCmd
        self.RBcmd(analyseTxtCmd)
        
        propertyTxtCmd = 'xcopy /y /f "' + os.path.normpath(self.G_PROPERTY_TXT_NODE) + '"' + ' ' + '"' + os.path.normpath(self.G_POOL_CONFIG) + '\\"'
        print propertyTxtCmd
        self.RBcmd(propertyTxtCmd)
        
        pluginCfgFile=os.path.join(self.G_HLEPER_TASK_CFG,'plugins.cfg')
        pluginCfgCmd = 'xcopy /y /f "' + os.path.normpath(pluginCfgFile) + '"' + ' ' + '"' + os.path.normpath(self.G_POOL_CONFIG) + '\\"'
        print pluginCfgCmd
