...
class MaxClient(RenderBase):
    ...
    def getMaxFileWeb(self,sourceMaxFile):
        ...
        if sourceMaxFile.startswith('net'):
    ...
    def writeMsFile(self,sceneFile,renderOutput,renderFrame,mySonid):
        ...
        if self.argsmap.has_key('from') and self.argsmap['from']=='web':
            mystr='fastRender "'+self.G_USERID+'" "'+self.G_TASKID+'"  "'+renderFrame+'" "'+self.G_CG_TILE+'" "'+self.G_CG_TILECOUNT+'" "'+self.G_KG+'" "'+self.G_JOB_NAME+'"  "'+renderOutput+'/" "'+self.G_PLATFORM+'" "'+self.CURRENT_TASK+'" "web" "'+sceneFile+'" \r\n'
        self.RBlog(mystr)
    ...
    def RBrender(self):#7
        ...
        renderMsFile=self.writeMsFile(self.MAX_FILE.replace('\\','/'),renderOutput,renderFrame,mySonid)
