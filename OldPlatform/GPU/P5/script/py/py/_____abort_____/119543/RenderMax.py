...
class MaxClient(RenderBase):
    ...
    def RBrenderConfig(self):#5
        ...
        #If got fumefx cache in render.cfg, analysis fumefx cache which needed
        if self.RENDER_CFG_PARSER.has_section('fumefx'):
            #Get fumefx information
            fumePathOptsList = self.RENDER_CFG_PARSER.options('fumefx')
            #Get current rendering frame
            curRenderFrame = self.G_CG_START_FRAME
            #List of target fumefx caches
            targetFumeCacheList = []
            #Dictionary of all fumefx caches
            fumePathAllDict = {}
            
            #Create list of the target fumefx caches
            for optName in fumePathOptsList:
                cachePath = self.RENDER_CFG_PARSER.get('fumefx', optName)
                
                cachePathSplitList = cachePath.split('|')
                offsetVal = None
                
                if len(cachePathSplitList) > 3:
                    offsetVal = cachePathSplitList[2]
                
                if offsetVal:
                    cacheNameOrigianl = os.path.basename(cachePath)
                    cacheNameSplit = os.path.splitext(cacheNameOrigianl)
                    
                    print 'curRenderFrame', curRenderFrame
                    print 'offsetVal', offsetVal
                    
                    targetCacheFrame = int(curRenderFrame) - int(offsetVal)
                    
                    if targetCacheFrame >= 0:
                        targetCacheFrame = string.zfill(targetCacheFrame, 4)
                    
                    cacheName = '%s%s%s' % (cacheNameSplit[0], str(targetCacheFrame), cacheNameSplit[1])
                
                    targetFumeCacheList.append(cacheName)
                    
                else:
                    print 'Fumefx Path information error! %s = %s' % (optName, cachePath)

            
            #Collect texture path with .fxd extension
            if self.RENDER_CFG_PARSER.has_section('texture'):
                texPathOptsList = self.RENDER_CFG_PARSER.options('texture')
                
                for opt in texPathOptsList:
                    texInfo = self.RENDER_CFG_PARSER.get('texture', opt)
                    texName = os.path.basename(texInfo)
                    cacheNameExt = os.path.splitext(texName)
                    
                    if cacheNameExt[1] == '.fxd':
                        fumePathAllDict.setdefault(texName, texInfo)
                
                print 'targetFumeCacheList'
                pprint(targetFumeCacheList)
                '''
                print 'fumePathAllDict'
                pprint(fumePathAllDict)
                '''
                #User data storage
                userInput = self.argsmap['inputDataPath'].replace('/', '\\')
                
                #Find is target fumefx cache exists
                for cache in targetFumeCacheList:
                    if cache in fumePathAllDict:
                        #Check texture path information
                        print 'fumePathAllDict[%s] : %s' % (cache, fumePathAllDict[cache])
                        if '>>' in fumePathAllDict[cache]:
                            fumeCacheInfo = fumePathAllDict[cache].split('>>')
                            #The path of fumefx cache at user input
                            fileAtInput = '%s\\%s' % (userInput, fumeCacheInfo[1])
                            #The path of fumefx cache at render node
                            fileDirectory = os.path.dirname(fumeCacheInfo[0])
                            FDInfo = os.path.splitdrive(fileDirectory)
                            onlyDirLetter = FDInfo[0][0]
                            pathAtNode = '%s\\%s\\%s' % (self.G_RENDER_WORK_TASK_MAX, onlyDirLetter, FDInfo[1])
                            
                            #C:\work\render\5229823\max
                            
                            #Copy cache file to render node
                            print '== Start Copy Fumefx Cache =='
                            print 'src : %s' % fileAtInput
                            print 'des : %s' % pathAtNode
                            cmd=u'c:\\fcopy\\FastCopy.exe  /speed=full /force_close  /no_confirm_stop /force_start "%s" /to="%s"' % (fileAtInput.replace('/', '\\'), pathAtNode.replace('/', '\\'))
                            print 'cmd : %s' % cmd
                            ...
                            print 'The texture path missing ">>"!'
            
                '''
                for k in fumePathAllDict.keys():
                    self.RENDER_CFG_PARSER.remove_option('texture', fumePathAllDict[k])
                 
                print 'RENDER_CFG_PARSER texture after del'
                pprint(self.RENDER_CFG_PARSER.options('texture'))
                '''
    ...
    def RBrender(self):#7
        ...
        #cmdStr=cmdFool
        customUsers=['1839967','1821122','1822553','964488','1833382','1818029','1818411','1839248','962712','1840568']
