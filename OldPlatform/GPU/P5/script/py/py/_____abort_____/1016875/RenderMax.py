...
class MaxClient(RenderBase):
    ...
    def getMaxFileClient(self,sourceMaxFile):
        self.G_RENDER_LOG.info('-----getMaxFileClient-----')
        self.G_RENDER_LOG.info(sourceMaxFile)
        netRenderTxt=os.path.join(self.G_MAXSCRIPT,'user',self.G_USERID,'netrender.txt').replace('\\','/')
        self.G_RENDER_LOG.info(netRenderTxt)
    ...
    def writeMsFile2(self,msFile,sceneFile,renderOutput,renderFrame,notRender):
        ...
        '''
        msFileObject.write('fn loadRenderbus = (\r\n')
        msFileObject.write('\tlocal MS_DIR=getFilenamePath (getSourceFileName())\r\n')
        msFileObject.write('\tlocal renderbusDll =  "c:/Renderbus.dll"\r\n')
        msFileObject.write('\tlocal assembly = dotNetClass "System.Reflection.Assembly"\r\n')
        msFileObject.write('\tlocal loadRenderbusDll = assembly.loadfrom renderbusDll\r\n')
        msFileObject.write('\tglobal G_RENDERBUS_DOT = dotnetobject "Renderbus.RenderUtil"\r\n')
        msFileObject.write(')\r\n')
        msFileObject.write('loadRenderbus()\r\n')
        '''
        
        msFileObject.write('filein @"'+self.G_RAYVISION_MAXMS+'"\r\n')
        msFileObject.write('fn renderRun = (\r\n')
        #mystr='rvRender "'+self.G_USERID+'" "'+self.G_TASKID+'" "'+notRender+'" "'+renderFrame+'" "'+self.G_CG_TILE+'" "'+self.G_CG_TILECOUNT+'" "'+self.G_KG+'" "'+self.G_JOB_NAME+'"  "'+renderOutput+'/" "'+self.G_PLATFORM+'" "'+self.CURRENT_TASK+'" "'+self.G_PATH_GRAB.replace('\\','/')+'"\r\n'
        ...
        if self.argsmap.has_key('from') and self.argsmap['from']=='web':
            mystr='fastRender "'+self.G_USERID+'" "'+self.G_TASKID+'" "'+notRender+'" "'+renderFrame+'" "'+self.G_CG_TILE+'" "'+self.G_CG_TILECOUNT+'" "'+self.G_KG+'" "'+self.G_JOB_NAME+'"  "'+renderOutput+'/" "'+self.G_PLATFORM+'" "'+self.CURRENT_TASK+'" "web" "'+sceneFile+'" "'+self.G_PATH_GRAB.replace('\\','/')+'" \r\n'
        self.RBlog('--------ms-------')
        self.RBlog(mystr)
    ...
    def writeFile(self,file,list):
        ...
        bolockTxtObject=codecs.open(file,'w',"utf-8")
        for line in list:
            bolockTxtObject.write(line)
        bolockTxtObject.close()
        
    def getNodeIP(self):
        hostname=socket.gethostname()
        localIp = socket.gethostbyname_ex(hostname)[2][0]
        return localIp


    def writeVrayDR(self,nodeList):
        list=[]
        for nodeIp in nodeList:
            thisNode=self.G_NODE_NAME.replace('A0','')
            if nodeIp.endswith(thisNode):
                pass
            else:
                list.append(nodeIp+' 1 20204\r\n')
            #list.append('10.50.7.100 1 20204')
        list.append('restart_slaves 0\r\n')
        list.append('list_in_scene 0\r\n')
        list.append('max_servers 5\r\n')
        list.append('use_local_machine 1\r\n')
        list.append('transfer_missing_assets 0\r\n')
        list.append('use_cached_assets 0\r\n')
        list.append('cache_limit_type 0\r\n')
        list.append('cache_limit 0.000000\r\n')
        
        vrayCfg=os.path.join(self.G_RENDER_WORK_TASK_CFG,'vray_dr.cfg')
        self.writeFile(vrayCfg,list)
        userprofile=os.environ["userprofile"]
        plugcfg=userprofile+'\\AppData\\Local\\Autodesk\\3dsMax\\'+self.G_CG_VERSION.replace('3ds Max ','')+' - 64bit\\enu\en-US\\plugcfg\\'
        cmd='xcopy /y /v /f "'+vrayCfg.replace('\\','/')+'" "'+plugcfg.replace('/','\\')+'"'
    ...
    def doBlock(self):
        ...
        if self.G_JOB_NAME.endswith('_0'):#client
            self.G_PROCESS_LOG.info('do block client...')
            tempPath=self.argsmap['tempPath']
            blockTempPath=os.path.join(tempPath,self.G_TASKID,'vrayblock')
            self.G_PROCESS_LOG.info(blockTempPath)
            
            nodeList=[]
            nodeList.append('10.50.11.57')
            nodeList.append('10.50.11.58')
            nodeList.append('10.50.11.59')
            nodeList.append('10.50.11.60')
            self.writeVrayDR(nodeList)
            
            vraySpawner=r'C:\Program Files\Autodesk\3ds Max 2014\vrayspawner2014.exe'
            #self.RBcmd('"'+ vraySpawner+'"')
            #os.system('start "" "'+ vraySpawner+'"')
            '''
            while True:
                list= os.listdir(blockTempPath)
                if len(list)==int(self.G_CG_TILECOUNT)-1:
                    nodeList=[]
                    for ipFile in list:
                        self.G_PROCESS_LOG.info(ipFile)
                        nodeList.append(ipFile)
                    self.writeVrayDR(nodeList)
                    break
                self.G_PROCESS_LOG.info('server wait 10...')
                time.sleep(10)
                
            '''
        else:#server
            #os.system()
            self.G_PROCESS_LOG.info('do block server...')
            self.blockServer()
    ...
    def blockServer(self):
        ...
        #todo start vrayspawner server
        vraySpawner=r'C:\Program Files\Autodesk\3ds Max 2014\vrayspawner2014.exe'
        os.system('start "" "'+ vraySpawner+'"')
        time.sleep(3)
        tempPath=self.argsmap['tempPath']
        blockTempPath=os.path.join(tempPath,self.G_TASKID,'vrayblock.txt').replace('/','\\')
        self.G_PROCESS_LOG.info(blockTempPath)
        
        while True:
            print 'wait...'
            if  os.path.exists(blockTempPath.replace('/','\\')):
                time.sleep(10)
            else:
                try:
                    self.RBcmd('taskkill /F  /IM vrayspawner2014.exe /T')
                except Exception, e:
                    print e
                time.sleep(3)
                try:
                    self.RBcmd('taskkill /F  /IM 3dsmax.exe /T')
                except Exception, e:
                    print e
                break
    ...
    def blockServer___BAK(self):
        ...
        time.sleep(3)
        ip=self.getNodeIP()
        blockTxt=os.path.join(self.G_RENDER_WORK_TASK_CFG,(ip))
        self.writeFile(blockTxt,[self.G_JOB_NAME])
        tempPath=self.argsmap['tempPath']
        blockTempPath=os.path.join(tempPath,self.G_TASKID,'vrayblock')
        self.G_PROCESS_LOG.info(blockTempPath)
        if not os.path.exists(blockTempPath):
            os.makedirs(blockTempPath)
        cmdStr='c:\\fcopy\\FastCopy.exe  /speed=full /force_close  /no_confirm_stop /force_start "' +blockTxt.replace('/','\\') +'" /to="'+blockTempPath.replace('/','\\')+'"'
        self.RBcmd(cmdStr)
        while True:
            print 'wait...'
            if os.path.exists(blockTempPath.replace('/','\\')):
    ...
    def RBrender(self):#7
        ...
        #cmdStr=cmdFool
        customUsers=['1839967','1821122','1822553','964488','1833382','1818029','964497','1818411','1839248']
        #if self.G_USERID in customUsers :
        if self.MAX_CMD_RENDER==True or self.G_USERID in customUsers:
            if self.argsmap.has_key('from') and self.argsmap['from']=='web':
                if self.RENDER_CFG_PARSER.has_option('common','rendoutputfilename'):
                    cmdRenderOutput = self.RENDER_CFG_PARSER.get('common','rendoutputfilename')
        ...
        if self.G_JOB_NAME.endswith('_0'):#client
            ...
            blockTempPath=os.path.join(tempPath,self.G_TASKID,'vrayblock.txt').replace('/','\\')
            self.G_PROCESS_LOG.info(blockTempPath)
            self.writeFile(blockTempPath,['endblock'])
            
            self.RBrenderCmd(cmdStr,True,True)
            
            os.remove(blockTempPath)
            
        
            userprofile=os.environ["userprofile"]
            plugcfg=userprofile+'\\AppData\\Local\\Autodesk\\3dsMax\\'+self.G_CG_VERSION.replace('3ds Max ','')+' - 64bit\\enu\en-US\\plugcfg\\vray_dr.cfg'
            os.remove(plugcfg)
