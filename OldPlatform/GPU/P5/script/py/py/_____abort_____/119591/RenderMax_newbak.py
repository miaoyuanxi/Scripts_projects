...
class MaxClient(RenderBase):
    ...
    def RBloadPlugin(self,customBat):#abort
        ...
        maxPlugin=MaxPlugin(self.G_PROCESS_LOG,self.maxB,self.maxPluginIniPath,self.G_CG_VERSION,self.localMaxPath,self.RBgetPluginDict(),self.G_PROGRAMFILES)
        maxPlugin.config()
    ...
    def RBrenderConfig(self):#5
        ...
        #----------load max plugin----------
        maxPlugin=MaxPlugin(self.G_PROCESS_LOG,self.G_MAX_B,self.G_CG_VERSION,self.RBgetPluginDict(),self.G_PROGRAMFILES)
        maxPlugin.config()
...
class MaxPlugin():
    def __init__(self,processLog,maxB,cgVersion,pluginDict,g_programfiles):
		print 'config lugin...'
        ...
        if allPlugDict and allPlugDict.has_key('plugins'):
            ...
            for pluginName in pluginDict:
				if pluginName=="vray":
					...
					pluginVersion=pluginDict[pluginName]
					self.G_PROCESS_LOG.info('[Load Plugin]----'+pluginName+","+pluginName+pluginVersion)
					pluginBatPath=os.path.join(self.G_MAX_B,pluginName,'script','pluginBat',pluginName + '.bat').replace('\\','/')
					cmdStr='"'+pluginBatPath+'" "'+self.G_CG_VERSION+'" "'+pluginName+'" "'+pluginName+pluginVersion+'" "'+self.G_MAX_B+'"'
					if os.path.exists(pluginBatPath):
						self.runcmd(cmdStr)
					else:
