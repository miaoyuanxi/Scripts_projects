...
class RenderMaxYCX():
    def __init__(self,**paramDict):
        self.G_SOURCE_FOLDER=paramDict['sourceFolder']#\\10.50.100.2\Model\Animal
        self.G_MAX_FOLDER_NAME=paramDict['maxFolderName']
        ...
        print 'self.G_MAX_FOLDER_NAME=',self.G_MAX_FOLDER_NAME
    ...
    def hanFile(self): 
        ...
        #-------------remove task folder----------
        taskPath=self.G_WORK_RENDER+"\\"+self.G_MAX_FOLDER_NAME
        ...
        #-------------maxscript file----------
        print 'maxscript'
        scriptSourceFolder=self.G_SOURCE_FOLDER+'\\script'
        if os.path.exists(scriptSourceFolder):
            ...
            scriptCmdStr = r'C:\fcopy\FastCopy.exe /speed=full /force_close /no_confirm_stop /force_start "' + scriptSourceFolder+'\\*.*" /to="'+ self.G_WORK_RENDER +'\\'+self.G_MAX_FOLDER_NAME+'\\script\\"'
            print scriptCmdStr
        ...
        #--------------env file----------
        print 'env'
        sourceEnv=self.G_SOURCE_FOLDER+'\\env'
        if os.path.exists(sourceEnv):
            envCmdStr = r'C:\fcopy\FastCopy.exe /speed=full /force_close /no_confirm_stop /force_start "' + sourceEnv.replace('/','\\') +'\\*.*" /to="' + self.G_WORK_RENDER +'\\'+self.G_MAX_FOLDER_NAME+'\\env\\"'
            print envCmdStr
            self.rbCmd(envCmdStr)
        else:
            exit(-9)
        '''
        doorTypePath=os.path.basename(self.G_SOURCE_FOLDER)
        print doorTypePath
        if doorTypePath.lower()=='indoor':
        ...
        #----------------scene file ----------------
        print 'scene'
        cmdStr = r'C:\fcopy\FastCopy.exe /speed=full /force_close /no_confirm_stop /force_start "' + self.G_SOURCE_FOLDER+'\\'+self.G_MAX_FOLDER_NAME +'" /to="' + self.G_WORK_RENDER +'\\"'
        print cmdStr
        self.rbCmd(cmdStr)
        pass
    ...
    def render(self):
        ...
        maxFile= self.G_WORK_RENDER +'\\'+ self.G_MAX_FOLDER_NAME+'\\' + self.G_MAX_FOLDER_NAME+ '.max'
        envPath=self.G_WORK_RENDER +'\\'+ self.G_MAX_FOLDER_NAME+'\\env\\'
        scriptFile=self.G_WORK_RENDER +'/'+self.G_MAX_FOLDER_NAME+'/script/ycxU.ms'
        renderCmd =  maxExe+" -silent -ma -mxs \"filein \\\""+scriptFile.replace('\\','/')+"\\\";renderMaxYCX \\\"" +maxFile.replace('\\','/') + "\\\" \\\""+envPath.replace('\\','/')+"\\\" \""
        #print "%s" % renderCmd
        print renderCmd
    ...
    def hanResult(self):
        ...
        jpgFile = self.G_WORK_RENDER + '\\' + self.G_MAX_FOLDER_NAME+ '\\' + self.G_MAX_FOLDER_NAME + '.jpg'
        print jpgFile
        if os.path.exists(jpgFile):
            renderOutput = r'c:\fcopy\FastCopy.exe /speed=full /force_close /no_confirm_stop /force_start "' + jpgFile + '" /to="' + self.G_SOURCE_FOLDER +'\\'+self.G_MAX_FOLDER_NAME+'\\"' 
            print renderOutput
    ...
    def checkResult(self):
        ...
        jpgFile = self.G_SOURCE_FOLDER + '\\' + self.G_MAX_FOLDER_NAME+'\\'+self.G_MAX_FOLDER_NAME +'.jpg'
        #print jpgFile
        if os.path.exists(jpgFile):
            print jpgFile
        else:
    ...
    def cleanThiTask(self):
        ...
        cleanCmd = 'rd /S /Q "' + self.G_WORK_RENDER+"\\"+self.G_MAX_FOLDER_NAME+'"'
        print cleanCmd
        try:
            print 'clean'
            self.rbCmd(cleanCmd)
