...
if os.path.exists(pySitePackagesNode):
    ...
    from MaxThread import MonitorMaxThread
...
class MaxClient(RenderBase):
    ...
    def writeMsFile2(self,msFile,sceneFile,renderOutput,renderFrame,notRender):
        ...
        mystr='rvRender "'+self.G_USERID+'" "'+self.G_TASKID+'" "'+notRender+'" "'+renderFrame+'" "'+self.G_CG_TILE+'" "'+self.G_CG_TILECOUNT+'" "'+self.G_KG+'" "'+self.G_JOB_NAME+'"  "'+renderOutput+'/" "'+self.G_PLATFORM+'" "'+self.CURRENT_TASK+'" "'+self.G_CG_OPTION+'"\r\n'
        print '\r\n\r\n\r\n\r\n\r\n+++++++++++++'
        print self.argsmap.has_key('from')
        if self.argsmap.has_key('from') and self.argsmap['from']=='web':
            mystr='fastRender "'+self.G_USERID+'" "'+self.G_TASKID+'" "'+notRender+'" "'+renderFrame+'" "'+self.G_CG_TILE+'" "'+self.G_CG_TILECOUNT+'" "'+self.G_KG+'" "'+self.G_JOB_NAME+'"  "'+renderOutput+'/" "'+self.G_PLATFORM+'" "'+self.CURRENT_TASK+'" "web" "'+sceneFile+'" "'+self.G_CG_OPTION+'" \r\n'
        self.RBlog('--------ms-------')
        self.RBlog(mystr)
    ...
    def RBrender(self):#7
        ...
        #cmdStr=cmdFool
        customUsers=['1821122','1822553','964488','1833382','1818029','1818411','1839248','1840568']
        ...
        monitorMaxT=None
        if os.path.exists(pySitePackagesNode):
            monitorMaxT= MonitorMaxThread(60,self.G_TASKID,self.G_JOB_NAME,self.G_NODE_NAME,self.G_PROCESS_LOG)
