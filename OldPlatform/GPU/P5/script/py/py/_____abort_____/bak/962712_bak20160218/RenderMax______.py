...
class MaxClient(RenderBase):
    ...
    def getFlythroughtFrame(self):
        ...
        return renderFrame
    def rendercmd(self):
        self.G_PROCESS_LOG.info('[Max.rendercmd.start....]')
        '''
        os.system('robocopy /e /ns /nc /nfl /ndl /np "B:/plugins/max/backburner" "C:/backburner"')
        os.system('msiexec /uninstall "C:/backburner/3ds Max 2016/Backburner.msi" /quiet')
        os.system('"C:/backburner/3ds Max 2016/Backburner.msi" /qn')
        '''
        renderFrame=self.G_CG_START_FRAME
        ...
        myoutput=self.RENDER_CFG_PARSER.get('renderSettings','output')
        maxRenderExe='C:/Program Files/Autodesk/'+ self.G_CG_VERSION+'/3dsmaxcmd.exe'
        cmdStr='"'+maxRenderExe+'" -o:'+self.G_RENDER_WORK_OUTPUT.replace('\\','/')+'/'+myoutput+'  -continueOnError -start:'+renderFrame+'  -end:'+renderFrame+'  "'+cgFile+'"'
        self.G_PROCESS_LOG.info(cmdStr)
        self.RBrenderCmd(cmdStr,True,True)
        self.G_PROCESS_LOG.info('[Max.rendercmd.end....]')
        
    def RBrender3333(self):
