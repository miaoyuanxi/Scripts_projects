...
class Max(AnalysisBase):
    ...
    def hanTips(self):
        ...
        if self.ANALYSE_TXT_PARSER.has_section('tips'):
            ...
            for itemKey in itemKeyList:
                ...
                dataDict[int(itemKey.strip())]=itemValList
            #data = {10001:["aaa","bbb"],10004:[],12345:["ccc"]}
            jsonStr = json.dumps(dataDict,sort_keys=True,indent=4)
            print self.G_TIPS_TXT_NODE
            fl=codecs.open(self.G_TIPS_TXT_NODE, 'w', "UTF-8")
            fl.write(str(dataDict))
            fl.close()
    ...
    def RBhanResult(self):#8
        ...
        dstDir = self.G_POOL + '/config/' + os.path.join(self.G_USERID_PARENT, self.G_USERID) + '/' + self.G_BIG_TASK_ID 
        analyseTxtCmd = 'xcopy /y /f "' + os.path.normpath(self.G_ANALYSE_TXT_NODE) + '"' + ' ' + '"' + os.path.normpath(dstDir) + '\\"'
        print analyseTxtCmd
        self.RBcmd(analyseTxtCmd)
        
        propertyTxtCmd = 'xcopy /y /f "' + os.path.normpath(self.G_PROPERTY_TXT_NODE) + '"' + ' ' + '"' + os.path.normpath(dstDir) + '\\"'
