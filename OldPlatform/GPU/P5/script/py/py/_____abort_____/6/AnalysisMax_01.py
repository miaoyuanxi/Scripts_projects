        ...
        if asset.endswith('.ifl'):
            if os.path.exists(asset):
                with codecs.open(asset, 'r', 'utf-8') as iflFile:
                    for linefFile in iflFile:
                        normalPath = self.renderbusPathObj.convertPath(linefFile.decode('utf-8')).strip()
                        print normalPath.encode('gbk', 'ignore')
                        if not os.path.exists(normalPath):
                            self.inputMissingItems.append(normalPath)
