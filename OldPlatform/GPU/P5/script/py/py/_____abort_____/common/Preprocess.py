...
class Preprocess(RenderBase):
    ...
    def readFile(self,path):
        if os.path.exists(path):
            ...
            return line
        pass
    def writeFile(self,filePath,list):
        fileObject=codecs.open(filePath,'w',"utf-8")
        for line in list :
            fileObject.write(self.convertStr2Unicode(line))
        fileObject.close()
        ...
        for ifl in self.iflList:
            ...
            if os.path.exists(ifl):
                ...
                for img in imgList:
                    ...
                    self.writeFile(ifl,imgBasenameList)
    ...
    def CopyFile2(self):
        ...
        if not self.RENDER_CFG_PARSER.has_section('vrmap') and not self.RENDER_CFG_PARSER.has_section('vrlmap') and self.RENDER_CFG_PARSER.has_section('customfile'):
            self.G_PROCESS_LOG.info('---customfile---')
