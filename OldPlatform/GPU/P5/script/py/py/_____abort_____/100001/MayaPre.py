...
class MayaPre(RenderBase):
	...
	def getPackFile(self):
		if self.RENDER_CFG_PARSER.has_option('common','cgFile'):
			self.packFile=self.RENDER_CFG_PARSER.get('common','render_file')
			self.cgFile=self.RENDER_CFG_PARSER.get('common','cgFile')	

	def RBrender(self):#7
		self.G_RENDER_LOG.info('[MayaPre.RBrender.start.....]')
		filePath = ""
		unpackCmd='7za x -y "'+self.packFile+'" -o "'+self.cgFile+'"'

		self.RBcmd(unpackCmd,True,False)

		self.G_RENDER_LOG.info('[MayaPre.RBrender.end.....]')
