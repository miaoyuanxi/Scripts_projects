        ...
        if  os.path.exists(dir):
            ...
            for root, dirs, files in list_dirs: 
                for name in files:
                    ...
                    convertCmd='c:/ImageMagick/nconvert.exe  -out jpeg -ratio -resize '+smallSize+' 0 -overwrite -o "'+workSmallTmpPic +'" "'+workBigPic+'"'
                    if ext=='.exr':
                        if os.path.exists(oiioPath):
                            self.G_PROCESS_LOG.info('exr parse----'+name)
                            convertCmd=oiioPath+' "'+workBigPic+'" -resize '+self.getConvertR(workBigPic,oiioPath)+' -o "'+workSmallTmpPic+'"'
                        else:
                            continue
                    #print workBigPic
                    self.RBcmd(convertCmd,True,False)
                    os.rename(workSmallTmpPic,workSmallPic)
                    self.G_PROCESS_LOG.info('workSmallTmpPic---'+workSmallTmpPic+'---workSmallPic---'+workSmallPic)
