...
class Vray(RenderBase):
		...
		if os.path.exists(self.G_CONFIG):
			...
			if isinstance(configJson["mntMap"],dict):
				...
				for key in configJson["mntMap"]:
					...
					netcmd = "net use "+key+" "+configJson["mntMap"][key]
					self.RBcmd(netcmd.encode('gbk'))
	...
	def RBrender(self):#7
		...
		renderCmd=bat+' "'+self.G_USERID+'" "'+self.G_TASKID+'" "'+self.pluginName.replace(" for x64","")+'" "'+self.pluginVersion+'" "'+self.G_CG_START_FRAME+'" "' + self.G_CG_END_FRAME +'" "'+self.G_CG_BY_FRAME +'" "'+filePath+'" "'+output+'"'
