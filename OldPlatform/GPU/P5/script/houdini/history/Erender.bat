...
echo RenderBegin
	...
	IF EXIST "%_APP_LOCAL_RUN_PATH%" (
		...
		ECHO rd /q/s %_APP_LOCAL_RUN_PATH% ...
		rd /q /s "%_APP_LOCAL_RUN_PATH%"
		ECHO END DEL  %_APP_LOCAL_RUN_PATH% @ %DATE%-%TIME%
	)
	...
	IF NOT "%_PLUG_HTOA_NV%"=="NULL" (
		...
		echo xcopy /y /f  "%_RESOURCE_SERVER_ROOT%\%_PLUGINS_FIXED_DIR%\%_PLUG_HTOA_NV%\houdini.env" "C:\users\enfuzion\Documents\houdini15.0\"
		xcopy /y /f  "%_RESOURCE_SERVER_ROOT%\%_PLUGINS_FIXED_DIR%\%_PLUG_HTOA_NV%\houdini.env" "C:\users\enfuzion\Documents\houdini15.0\"
