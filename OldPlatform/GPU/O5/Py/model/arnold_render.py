...
class KickRender(Render, RvOs):
    ...
    def get_kick_info(self):
        ...
        if int(self.cfg_info["user_id"]) in [100001]:
            ...
            self.call_command("NET USE %s %s" % ("N:", r"\\10.50.24.11\d\inputdata5\961500\961900\N"))
            hasEnv = False
            host = r"C:/Windows/system32/drivers/etc/hosts"
            with open(host, "a+") as f:
            ...
            srcRlmDir=  r"\\10.50.1.22\td\clientFiles\961900\rlm"
            dstRlmDir=r"C:\AMPED_mili"
            os.system ("robocopy /s  %s %s" % (srcRlmDir, dstRlmDir))
            srcRlmDir=  r"\\10.50.1.22\td\clientFiles\961900\Yeti"
            dstRlmDir=r"C:\Yeti"
            os.system ("robocopy /s  %s %s" % (srcRlmDir, dstRlmDir))
            os.environ["PATH"] += ";"
            ...
            self.copytree(r"\\10.50.24.11\d\plugins\961900\arnold\procedurals",
...
#                               self.get_ass_file(frame))
        ...
        cmd = "\"%s\" -nstdin -dw -dp -nocrashpopup -v 2 -l %s " \
