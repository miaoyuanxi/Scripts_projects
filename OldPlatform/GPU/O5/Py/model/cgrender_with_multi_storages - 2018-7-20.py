...
class NukeMerge(Nuke):
    ...
    def merge_files(self, tiles, input_files, tile_output):
        ...
        if options["common"]["userId"] in [1844817]:
            merge_images = os.path.join(os.path.dirname(__file__), "merge_images_deep.py")
...
#                os.system(r'"xcopy /y /v /e \\10.50.5.29\o5\py\model\maya2017\prefs\pluginPrefs.mel C:\users\enfuzion\Documents\maya\2017\prefs\ "')
        ...
        if not self["common"]["plugin_file"]:
            ...
            if self["common"]["userId"] in [962276]:
                ...
                cmd = "\"%s\" && %s" % (pre_bat, cmd)
        # if self["common"]["userId"] in [1881622,1861699,1853812,1844577,1859047,963567,1867173,1878285,1822731]:
        ...
        if "RenderMan_for_Maya" in self["plugins"]["plugins"]:
            ...
            if self["common"]["userId"] in [1901996,1866194,1835213,1888315,1833958,1844817,1873149,1886200,1868563,1813649,1859026,961743,1833038,964311,1819610,1814856,1848484,1848680,963567,1820265,964309,1864685,1860992,119768,962371,1821671,1854466,1811755,1863576,1863884,1863099,1877869,1864856,1883450,1877965,1846192,1876209,1876088,1905390,1909924,1839312,1912043]:  
        ...
        if self["common"]["userId"] in [1844817,1873149,1881622,1877965,1898302,1851281,1905390,1909924,1866194,1835213,1818415,1912808]:
                    ...
                    for line in RvOs.run_command(cmd):
                        ...
                        if line_str:
                            ...
                            if "failed to render" in line_str:
                                ...
                                is_complete = 0
                                break
                            if self['common']['userId'] not in [1844817,1881622,1909139,1915711]:
