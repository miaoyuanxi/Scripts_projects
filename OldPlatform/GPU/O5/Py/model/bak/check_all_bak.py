...
if sys.version_info[:2] == (2, 6):
    if sys.platform.startswith("win"):
        sys.path.append(r"\\10.90.100.101\o5\py\model\tim\py26")
...
class MayaAnalyze(Analyze):
    ...
    def get_info(self):
        ...
        print "*"*30
        if  self['user_id'] in [119768,1868564]:
    ...
    def run(self):
        ...
        if render_name == "vray":
            ...
            self["dynMemLimit"]  = int(pm.getAttr('vraySettings.sys_rayc_dynMemLimit'))
        print rd_path             
        ...
        self['renderable_layer'] = ",".join([i.name() for i in pm.PyNode("renderLayerManager.renderLayerId").outputs() if i.type() == "renderLayer"  if i.renderable.get()] )        
