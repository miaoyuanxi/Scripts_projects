...
#        return subprocess.call(command, shell=1)
    ...
    def clean_network(self):
        if self["server"]["user_id"] in [1807088, 1837972, 1838377, 1835085, 1839655, 964647, 1812927, 1840274]:
            return 0
...
class MayaRender(MayaClass):
    ...
    def render(self):
        ...
        if self["renderSettings"]["renderableLayer"]:
            if  not self["server"]["user_id"] in [962413,1820727,1821225,1821226,1821227,1821228,1821229,1821230,1821231,1821232,1821233,1821234,1821235,1821236,1821237]:
    ...
    #                    cmd += " -im \"%s\"" % image_name
        ...
        if self["common"]["userId"] in [1814593]:
            cmd += " -preRender \"optionVar  -iv \\\"renderSetupEnable\\\" 0;\""
        #if self["common"]["userId"] in [963480]:
            #cmd += " -preRender \"setAttr \\\"defaultRenderGlobals.modifyExtension\\\" 0;setAttr \\\"vraySettings.srdml\\\" 24000;\""
