...
def get_json(**kwargs):
    ...
    if kwargs["task_id"]:
        ...
        options["renderSettings"]["user_id"] = options["common"]["userId"]
        options["renderSettings"]["task_id"] = options["common"]["taskId"]
...
class Render(dict, Zip7):
    ...
    def check_sweeper(self):
        ...
        if processes:
            ...
            print "start sweeper."
            cmd = "start %s" % (r"C:\work\munu_client\sweeper\sweeper.exe")
            print "cmd: " + cmd
...
class MayaRender(MayaClass):
    ...
    def render(self):
        ...
        # if float(self["common"]["cgv"])==2017:
            # os.system(r'"xcopy /y /v /e \\10.70.242.102\o5\py\model\maya2017\prefs\pluginPrefs.mel C:\users\enfuzion\Documents\maya\2017\prefs\ "')
        # if float(self["common"]["cgv"])==2016.5:
            # os.system(r'"xcopy /y /v /e \\10.70.242.102\o5\py\model\maya2016.5\prefs\pluginPrefs.mel C:\users\enfuzion\Documents\maya\2016.5\prefs\ "')
    ...
    #                    cmd += " -im \"%s\"" % image_name
        ...
        if self["common"]["userId"] in [1814286]:
            cmd += " -preRender \"optionVar  -iv \"miUseMayaAlphaDetection\" 1; \""

        #if self["common"]["userId"] in [1832459]:
            #cmd += " -preRender \"setAttr \\\"vraySettings.animType\\\" 1;\""

        if self["common"]["userId"] in [1814593,119602]:
            cmd += " -preRender \"optionVar  -iv \"renderSetupEnable\" 0;\""
        #if self["common"]["userId"] in [1811653]:
            #cmd += " -preRender \"python \\\"execfile(\\\\\\\"//10.50.5.29/o5/py/model/1811653.py\\\\\\\")\\\"\"; "
        if self["common"]["userId"] in [1017637]:
            cmd += " -preRender \"python \\\"execfile(\\\\\\\"//10.50.5.29/o5/py/model/1017637.py\\\\\\\")\\\"\"; "
        if self["common"]["userId"] in [963287, 963492, 963250, 964462,965366]:
            cmd += " -r arnold -ai:lve 1"
        #elif self["common"]["userId"] in [1805997]:
        #    cmd += " -r maxwell"
        ...
        cmd += ' > ' + "d:/new.log"
        print cmd
        cmdp=subprocess.call(cmd, shell = True)
        exitcode = 0
        return exitcode
        # sys.stdout.flush()
