...
#                os.system(r'"xcopy /y /v /e \\10.50.5.29\o5\py\model\maya2017\prefs\pluginPrefs.mel C:\users\enfuzion\Documents\maya\2017\prefs\ "')
        ...
        if "RenderMan_for_Maya" in self["plugins"]["plugins"]:
            ...
            if self["common"]["userId"] in [1833958,1844817,1873149,1886200,1868563,1813649,1859026,961743,1833038,964311,1819610,1814856,1848484,1848680,963567,1820265,964309,1864685,1860992,119768,962371,1821671,1854466,1811755,1863576,1863884,1863099,1877869,1864856,1883450,1877965,1846192,1876209,1876088]:  
            ...
            if self["common"]["userId"] in [1846192]:
                print "dan ka gpu"
                gpu_n = "0"
            # if self["common"]["taskId"] in [16127821]:
                # print "dan ka gpu"
                # gpu_n = "0"                
            print "gpu_n: %s" % (gpu_n)            
            cmd += " -gpu {%s}" % (gpu_n)
        ...
        if self["common"]["userId"] in [1844817,1873149,1881622,1877965,1898302]:
                    ...
                    for line in RvOs.run_command(cmd):
                        ...
                        if line_str:
                            ...
                            if "failed to render" in line_str:
                                ...
                                is_complete = 0
                                break
                            if self['common']['userId'] not in [1844817]:
