...
def get_json(**kwargs):
    ...
    if kwargs["task_id"]:
        ...
        if options["common"]["father_id"] == 964309:
            options["mount"]["K:"] = options["platform"]["home_path"] + "/964000/964309/jueji1/maya/K"

        if options["common"]["userId"] == 1821185:
            options["common"]["father_id"] = 1821185

        if options["common"]["father_id"] == 1821185:
            options["mount"]["K:"] = options["platform"]["home_path"] + "/1821000/1821185/ths/maya/K"
...
#                os.system(r'"xcopy /y /v /e \\10.50.5.29\o5\py\model\maya2017\prefs\pluginPrefs.mel C:\users\enfuzion\Documents\maya\2017\prefs\ "')
        ...
        if self["common"]["userId"] in [1844577,1859047]:
            cmd += " -preRender \"python \\\"user_id=%s;mapping=%s;plugins=%s;taskid=%s;rendersetting=%s;execfile(\\\\\\\"//10.70.242.102/o5/py/model/prerender.py\\\\\\\")\\\"\"" % (self["common"]["userId"],self["mappings"],self["plugins"]["plugins"],self["common"]["taskId"],self["renderSettings"])
        ...
        if "RenderMan_for_Maya" in self["plugins"]["plugins"]:
            ...
            if self["common"]["userId"] in [961743,964311,1819610,1814856,1848484,1848680,964309,963567]:
                ...
                cmd += " -preRender \"python \\\"user_id=%s;mapping=%s;plugins=%s;taskid=%s;rendersetting=%s;execfile(\\\\\\\"//10.70.242.102/o5/py/model/prerender.py\\\\\\\")\\\"\" -r redshift -logLevel 2 " % (self["common"]["userId"],self["mappings"],self["plugins"]["plugins"],self["common"]["taskId"],self["renderSettings"])
        ...
        if self["common"]["userId"] in [1834123,1020987,962814]:
            cmd += " -preRender \"putenv(\\\"MAYA_DISABLE_BATCH_RUNUP\\\",\\\"1\\\"); global proc dynRunupForBatchRender() {}; \""
        if self["common"]["userId"] in [962814,1849445,1858637]:
            if "mtoa" in self["plugins"]["plugins"]:
                if self["renderSettings"]["tiles"] == 1:
                    cmd += " -r arnold -ai:lve 1 -ai:txmm 20480"
