...
class MayaRender(MayaClass):
    ...
    def render(self):
        ...
        if self["common"]["debug"]:
            print "json info:"
            pprint.pprint(self)
#        if self["common"]["userId"] in [1858637]:
#            if float(self["common"]["cgv"])==2017:
#                os.system(r'"xcopy /y /v /e \\10.50.5.29\o5\py\model\maya2017\prefs\pluginPrefs.mel C:\users\enfuzion\Documents\maya\2017\prefs\ "')
        # if float(self["common"]["cgv"])==2016.5:
            # os.system(r'"xcopy /y /v /e \\10.50.5.29\o5\py\model\maya2016.5\prefs\pluginPrefs.mel C:\users\enfuzion\Documents\maya\2016.5\prefs\ "')
        ...
        if self["server"]["user_id"] in [1868570,1868577,1868572,964309]:
        ...
        if not self["common"]["plugin_file"]:
            ...
            if self["common"]["userId"] in [962276]:
                ...
                cmd = "\"%s\" && %s" % (pre_bat, cmd)
        if self["common"]["userId"] in [1844577,1859047,963567,1867173]:
            cmd += " -preRender \"python \\\"user_id=%s;mapping=%s;plugins=%s;taskid=%s;rendersetting=%s;execfile(\\\\\\\"//10.90.100.101/o5/py/model/prerender.py\\\\\\\")\\\"\"" % (self["common"]["userId"],self["mappings"],self["plugins"]["plugins"],self["common"]["taskId"],self["renderSettings"])

        if "RenderMan_for_Maya" in self["plugins"]["plugins"]:
            cmd += " -r rman"
        elif [i for i in self["plugins"]["plugins"] if i.startswith("3delight")]:
            ...
            if self["common"]["userId"] in [1868563,1813649,1859026,961743,1833038,964311,1819610,1814856,1848484,1848680,963567,1820265,964309,1864685,1860992,119768,962371,1821671,1854466,1811755,1863576]:  
                # if self["common"]["userId"] in [961743]:
                    # cmd +=" -preRender \"python \\\"execfile(\\\\\\\"//10.90.100.101/o5/py/model/961743.py\\\\\\\")\\\"\" -r redshift -logLevel 2"                
                # else:
                if not self["common"]["taskId"] in [16031525]:
                    print "Run Redshift custom script "
                #cmd +=" -preRender \"python \\\"execfile(\\\\\\\"//10.60.100.101/o5/py/model/961743.py\\\\\\\")\\\"\" -r redshift -logLevel 2"
                #cmd += " -preRender \"python \\\"user_id=%s;mapping=%s;plugins=%s;taskid=%s;execfile(\\\\\\\"//10.70.242.102/o5/py/model/prerender.py\\\\\\\")\\\"\" -r redshift -logLevel 2 " % (self["common"]["userId"],self["mappings"],self["plugins"]["plugins"],self["common"]["taskId"])
                    cmd += " -preRender \"python \\\"user_id=%s;mapping=%s;plugins=%s;taskid=%s;rendersetting=%s;start=%s;end=%s;execfile(\\\\\\\"//10.90.100.101/o5/py/model/prerender.py\\\\\\\")\\\"\" -r redshift -logLevel 2 " % (self["common"]["userId"],self["mappings"],self["plugins"]["plugins"],self["common"]["taskId"],self["renderSettings"],self["renderSettings"]["start"],self["renderSettings"]["end"])
                else :
            ...
            # if self["common"]["userId"] in [1868563]:
                # print "dan ka gpu"
                # gpu_n="0"
            # if self["common"]["taskId"] in [16022385]:
                # print "dan ka gpu"
                # gpu_n="0"
            cmd += " -gpu {%s}" % (gpu_n)
        ...
        if self["common"]["userId"] in [1859026]:
            cmd = cmd.replace("-logLevel 2" ," ")
        #tsanghao 1811213
        if self["common"]["userId"] in [1846778]:
        ...
        if self["common"]["userId"] in [1847045]:
            if "mtoa" in self["plugins"]["plugins"]:
                cmd += " -r arnold -ai:lve 1"
        if self["common"]["userId"] in [1811213,1300775]:
            cmd += " -preRender \"source \\\"//10.50.5.29/o5/py/model/963683.mel\\\"\""
        if self["common"]["userId"] in [1833687]:
            cmd += " -preRender \"\" "
        if self["common"]["userId"] in [962413,1820727,1821225,1821226,1821227,1821228,1821229,1821230,1821231,1821232,1821233,1821234,1821235,1821236,1821237]:
            cmd += " -preRender \"source \\\"//10.50.1.22/td/clientFiles/962413/render_command_light_new_yeti2.0.24.mel\\\"\""
        if self["common"]["userId"] in [963683]:
            cmd += " -preRender \"source \\\"//10.50.5.29/o5/py/model/963683.mel\\\";putenv(\\\"MAYA_DISABLE_BATCH_RUNUP\\\",\\\"1\\\"); global proc dynRunupForBatchRender() {}; \""

        if self["common"]["userId"] in [1813119]:
            cmd += " -preRender \"python \\\"renderableCamera=\\\\\\\"%s\\\\\\\";execfile(\\\\\\\"//10.50.5.29/o5/py/model/1813119.py\\\\\\\")\\\"\"" % (self["renderSettings"]["renderableCamera"])
        if self["common"]["userId"] in [1841237]:
            cmd += " -preRender \"python \\\"execfile(\\\\\\\"//10.50.5.29/o5/py/model/1841237.py\\\\\\\")\\\"\""


        if self["common"]["userId"] in [1812148,1830380]:
            cmd += " -preRender \"python \\\"execfile(\\\\\\\"//10.50.5.29/o5/py/model/1812148.py\\\\\\\")\\\"\""
        if self["common"]["userId"] in [963987]:
            cmd += " -preRender \"python \\\"execfile(\\\\\\\"//10.50.5.29/o5/py/model/963987.py\\\\\\\")\\\"\""
        """
        if self["common"]["userId"] in [1812148]:
            cmd += " -preRender \"source \\\"//10.50.1.22/td/clientFiles/1812148/meshShapeAttrSet.mel\\\"\" "
        """
        # if self["common"]["userId"] in [1858637]:
            # cmd += " -preRender \"python \\\"user_id=%s;mapping=%s;plugins=%s;taskid=%s;start=%s;execfile(\\\\\\\"//10.70.242.102/o5/py/model/prerender.py\\\\\\\")\\\"\"" % (self["common"]["userId"],self["mappings"],self["plugins"]["plugins"],self["common"]["taskId"])
        if self["common"]["userId"] in [100001]:
            cmd += " -preRender \"source \\\"//10.50.5.29/o5/py/model/100001.mel\\\"\""
        if self["common"]["userId"] in [1838785]:
            cmd += " -preRender \"\""
        if self["common"]["userId"] in [1834123,1020987,962814]:
            cmd += " -preRender \"putenv(\\\"MAYA_DISABLE_BATCH_RUNUP\\\",\\\"1\\\"); global proc dynRunupForBatchRender() {}; \""
        if self["common"]["userId"] in [962814,1849445,1858637,1861989]:
            if "mtoa" in self["plugins"]["plugins"]:
                if self["renderSettings"]["tiles"] == 1:
                    cmd += " -r arnold -ai:lve 1 -ai:txmm 20480"
        if self["common"]["userId"] in [1832459]:
            cmd += " -preRender \"setAttr \\\"vraySettings.animType\\\" 1;\" "
        if self["common"]["userId"] in [1814593]:
            cmd += " -preRender \" optionVar  -iv \\\"renderSetupEnable\\\" 0;source \\\"//10.50.5.29/o5/py/model/1814593.mel\\\";\""
        if self["common"]["userId"] in [963480]:
            cmd += " -preRender \"python \\\"execfile(\\\\\\\"//10.50.5.29/o5/py/model/963480.py\\\\\\\")\\\"\"; "
        if self["common"]["userId"] in [1834714]:
            cmd += " -preRender \"setUpAxis \\\"z\\\";\" "
        ...
        if self["server"]["user_id"] in [119768]:
            if self["renderSettings"]["end"] > self["renderSettings"]["start"]:
                cmd += " -postFrame \"python \\\"execfile(\\\\\\\"//10.90.100.101/o5/py/model/rendered_frames_%s.py\\\\\\\");set_rendered_frame(\\\\\\\"%s\\\\\\\",\\\\\\\"%s\\\\\\\",\\\\\\\"%s\\\\\\\")\\\";\"" % (self["server"]["user_id"],self["platform"]["cfg_path"],self["common"]["taskId"],self["renderSettings"]['output'])
        ...
        if self["server"]["user_id"] in [1820390,1811737,1811222,1301816,963433,963685,1816635,1819610,963260]:
        ...
        if self["server"]["father_id"] in [1819310, 1821185]:
            if not os.path.exists(self["renderSettings"]["output_redshift"]):
                os.makedirs(self["renderSettings"]["output_redshift"])
            self["renderSettings"]["bat"] = r"\\10.50.244.116\p5\script\maya\of3d.bat"
            self["renderSettings"]["task_id"] = self["common"]["taskId"]
            cmd = "\"%(bat)s\" %(user_id)s %(task_id)s \"%(render.exe)s\" "\
                "\"%(renderableLayer)s\" %(start)s %(end)s %(by)s " \
                "\"%(projectPath)s\" \"%(maya_file)s\" \"%(output_redshift)s\"" \
        ...
        if self["server"]["father_id"] == 1818936:
            ...
            cmd = "eco -t rayvision_maya -r \"render.exe -s %(start)s" \
                  ...
                  " -cam \\\"%(renderableCamera)s\\\"" \
                  " -rd \\\"%(output)s\\\" -x %(width)s -y %(height)s" \
                  " \\\"%(maya_file)s\\\"" \
                  % self["renderSettings"]
