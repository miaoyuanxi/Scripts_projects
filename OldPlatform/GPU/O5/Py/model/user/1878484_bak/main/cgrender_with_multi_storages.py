...
class MayaRender(MayaClass):
    ...
    def render(self):
        ...
        if "redshift_GPU" in self["plugins"]["plugins"]:
            ...
            if self["common"]["taskId"] in [16158666]:
                cmd += " -preRender \"python \\\"pre_render_dict=%s;execfile(\\\\\\\"//10.90.100.101/o5/py/model/user/1878484/prerender_test/C_PreRender.py\\\\\\\")\\\"\"" % (pre_render_dict)
            else:
