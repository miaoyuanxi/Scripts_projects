...
def get_platfom(platform):
    ...
    if info["platform"] == 1002:
        ...
        info["custom_config"] = r"\B\custom_config"
    for i in info:
...
class NukeMerge(Nuke):
    ...
    def get_input_output(self):
        ...
        #    tile_path = "%s/tiles/%s/%s_%s_%s/%s" % (self["platform"]["storage_path"], self["common"]["taskId"], self["renderSettings"]["start"],self["renderSettings"]["end"],self["renderSettings"]["by"],self["renderSettings"]["start"])   
        ...
        if "gpuid" in os.environ:
            print "gpuid +++++++++++++++++++++++++++++++++++++"
            self["renderSettings"]["output"] = "/tmp/nzs-data/work/render/%s/output/%s_%s_%s/" % (self["common"]["taskId"],
                                      self["renderSettings"]["start"],
    ...
    def render(self):
        ...
        for i in self.tile_files:
            ...
            print i
            # if "gpuid" in os.environ:
                # tile_output = self["renderSettings"]["output"] + re.sub(r'(.+/tiles/[-]?\d+/[-]?\d+[_]\d+[_]\d+/[-]?\d+)(/0/)(.+)', r"\3", i[0])
            # else:
...
class MayaRender(MayaClass):
    ...
    def render(self):
        ...
        if "gpuid" in os.environ:
            self["renderSettings"]["output"] = "/tmp/nzs-data/work/render/%s/" \
        ...
        if self["common"]["userId"] in [119768,1914867]:
            REDSHIFT_LOCALDATAPATH_com="/tmp/nzs-data/redshift_cache"
        ...
        cmd += " -r redshift -logLevel 2 -gpu \{0,1\} "    
        #cmd += " -r redshift  "    
        ...
        cmd += " -x %(width)s -y %(height)s  " % self["renderSettings"]
            ...
            # if self["common"]["userId"] in [1866194,1835213,1888315,1833958,1844817,1873149,1886200,1868563,1813649,1859026,961743,1833038,964311,1819610,1814856,1848484,1848680,963567,1820265,964309,1864685,1860992,119768,962371,1821671,1854466,1811755,1863576,1863884,1863099,1877869,1864856,1883450,1877965,1846192,1876209,1876088,1905390,1909924,1839312]:  
...
if __name__ == '__main__':
    ...
    if str(options["renderSettings"]["tiles"]) != '1' and options["renderSettings"]["tiles"] == options["renderSettings"]["tile_index"]:
        ...
        if options["common"]["cgSoftName"] == "maya":
            if options["renderSettings"]["renderType"] == "render":
                render = MayaRender(options)
            elif options["renderSettings"]["renderType"] == "mayabatch":
                render = MayaBatch(options)
            elif options["renderSettings"]["renderType"] == "maya":
                render = MayaGui(options)
        elif options["common"]["cgSoftName"] == "arnold":
            if options["renderSettings"]["renderType"] == "kick":
