...
class OiiO(Render):
    ...
    def merge_files(self, read1, read2, output):
        ...
        cmd = "\"%s\" -add %s %s -o %s" % (oiio_exe, read1, read2, output)
...
class MayaRender(MayaClass):
    ...
    #                    cmd += " -im \"%s\"" % image_name
        ...
        if self["server"]["user_id"] in [1301816,963433,963685,1816635]:
            if not os.path.exists(self["renderSettings"]["output_redshift"]):
                os.makedirs(self["renderSettings"]["output_redshift"])
            self["renderSettings"]["bat"] = r"\\10.50.242.1\td\bat\Erender_RS.bat"
...
if __name__ == '__main__':
    ...
    if render:
        subprocess.call(r'C:\Program Files\Autodesk\Maya2013\bin\maya.exe')
