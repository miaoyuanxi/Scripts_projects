...
def Check_vers(NK_file,_CGV):

    print "-----------------------view  start-----------------------"
    print "User choose version: " + _CGV
    mbool = 0
    vers = ''
    if _CGV == '9.0':
        vers = 'Nuke9.0v9'
    ...
    if vers == None:
        ...
        with open(NK_file,"r") as f:
            lines = f.readlines()
            print "Nukefile version:  " + lines[0]

            for i,value in enumerate(lines):
                if re.match(r'^version \d{1,2}.\d{1} v\d{1}$', value):
                    print "Nukefile version   : %s" % value
...
def _RV_changeNUKEVersion(NK_file,_CGV):
    ...
    vers = ''
    if _CGV == '9.0':
        mstr = '#! C:/Program Files/Nuke9.0v9/nuke-9.0.9.dll -nx\n'
        addinfo_1 = '''#! C:/Program Files/Nuke9.0v9/nuke-9.0.9.dll -nx\n'''
        addinfo_2 = '''version 9.0 v9\n'''
        vers = 'Nuke9.0v9'

    elif _CGV == '10.0':
        mstr = '#! C:/Program Files/Nuke10.0v6/nuke-10.0.6.dll -nx\n'
        addinfo_1 = '''#! C:/Program Files/Nuke10.0v6/nuke-10.0.6.dll -nx\n'''
        addinfo_2 = '''version 10.0 v6\n'''
        vers = 'Nuke10.0v6'

    elif _CGV == '10.5':
        mstr = '#! C:/Program Files/Nuke10.5v5/nuke-10.5.5.dll -nx\n'
        addinfo_1 = '''#! C:/Program Files/Nuke10.5v5/nuke-10.5.5.dll -nx\n'''
        addinfo_2 = '''version 10.5 v5\n'''
        vers = 'Nuke10.5v5'
    else:
        mstr = None
        addinfo_1 = None
    ...
    if mstr == None:
        ...
        print "mstr = " +mstr
        print "addinfo_1 = " + addinfo_1
        print "addinfo_2 = " + addinfo_2
        print "vers = " + vers

        with open(NK_file,"r") as f:
            lines = f.readlines()
        with open(NK_file,"w") as f_w:
            print "Source nukefile version:  " + lines[0]
            print "Modify nukefile version:  " + mstr

            if lines[0] == mstr:
                mbool = 0
                print "The nukefile_version is same-------------------------"
            else:
                mbool = 1
                for i,value in enumerate(lines):
                    if i == 0:
                        print "Old NK line num %d  : %s"%(i,value)
                        lines[i] = "#" + value
                        first = i
                        print "index : %d" % first
                    elif re.match(r'^version \d{1,2}.\d{1} v\d{1}$', value):
                        print "Old NK line num %d  : %s"%(i,value)
                        lines[i] = "#" + value
                        second = i
                        print "NK version : %d" % second
        ...
        if mbool == 1:
            ...
            with open(NK_file,"r") as fileold:
                nukefile_list = fileold.readlines()

            print "length of old cluster.conf:",nukefile_list.__len__()

            nukefile_list.insert(0,addinfo_1)
            print "length of new1 cluster.conf:",nukefile_list.__len__()
            print "add <NK  version=**** at line:",second

            tarline = int(second)+1
            nukefile_list.insert(tarline,addinfo_2)
            print "length of cluster.conf after change:",nukefile_list.__len__()
            filenew= open(NK_file,"wb")
            for item in nukefile_list:
                filenew.write(item)
            filenew.close()
