    ...
    for frames in frameList:
        if frames != "":
            ...
            if nukeExecutePath.find("/") != -1:
                cmd = '"%s" -x -m %s -X %s --cout -F %s,%s %s' % (nukeExecutePath, renderThreads, write, frames, _BFM, nkPath)
            else:
                cmd = '%s -x -m %s -X %s --cout -F %s,%s %s' % (nukeExecutePath, renderThreads, write, frames, _BFM, nkPath)
            # # print cmd
            cmdList.append(cmd)
...
###############################################################################
    ...
    # else:
        # print nuke_path
    if _CGV == '11.0':      
        nuke_path = _NUKE_RUN_PATH + "/" + "Nuke11.0.exe"        
    if _CGV == '10.5':      
        nuke_path = _NUKE_RUN_PATH + "/" + "Nuke10.5.exe"
    if _CGV == '10.0':
        nuke_path = _NUKE_RUN_PATH + "/" + "Nuke10.0.exe"
    if _CGV == '9.0':
...
def addenvpath(_CGV):
    ...
    with open(initfile, 'w') as f:
        ...
        addpath_  =r"B:/plugins/nuke/plugins/Nuke/10.5/plugins/CaraVR/1.0"
        addpath_1 = r'B:/plugins/nuke/plugins/nuke10.0_vray_data'

        inf = "import nuke\nnuke.pluginAddPath('%s')\nnuke.pluginAddPath('%s')\nnuke.pluginAddPath('%s')" % (addpath,addpath_,addpath_1)

        f.write(inf)
        
def copy_plugins(_NUKE_RUN_PATH):
    # _NUKE_RUN_PATH = r'"C:\Program Files\Nuke10.0v4"'
    srcDir01 = r'B:\plugins\nuke\plugins\nuke10.0_vray\01'
    srcDir02 = r'B:\plugins\nuke\plugins\nuke10.0_vray\02\vray'
    srcDir03 = r'B:\plugins\nuke\plugins\nuke10.0_vray\03\vray'
    srcDir04 = r'B:\plugins\nuke\plugins\nuke10.0_vray\04\V-Ray'
    _NUKE_RUN_PATH=r'"%s"' %_NUKE_RUN_PATH
    
    
    dstDir01 = _NUKE_RUN_PATH
    dstDir01 = dstDir01.replace('\\','/')
    dstDir02 = os.path.join(_NUKE_RUN_PATH,'plugins','vray').replace('\\','/')
    dstDir02 = dstDir02.replace('\\','/')
    dstDir03 = r'"C:\Program Files\Common Files\Nuke\10.0\plugins\vray"'
    dstDir03 = dstDir03.replace('\\','/')
    dstDir04 = r'"C:\Program Files\Chaos Group\V-Ray"'
    dstDir04 = dstDir04.replace('\\','/')
    dstDir01=r'%s' %dstDir01
    dstDir02=r'%s' %dstDir02
    dstDir03=r'%s' %dstDir03
    dstDir04=r'%s' %dstDir04

    os.system ("robocopy /e /ns /nc /nfl /ndl /np  %s %s" % (srcDir01, dstDir01))
    os.system ("robocopy /e /ns /nc /nfl /ndl /np  %s %s" % (srcDir02, dstDir02))
    os.system ("robocopy /e /ns /nc /nfl /ndl /np  %s %s" % (srcDir03, dstDir03))
    os.system ("robocopy /e /ns /nc /nfl /ndl /np  %s %s" % (srcDir04, dstDir04))        
    
    
    srcDir05 = r'B:\plugins\nuke\plugins\RSMB'
    dstDir05 = r'"C:\Program Files\Common Files\OFX\Plugins"'
    dstDir05 = dstDir05.replace('\\','/')
    dstDir05=r'%s' %dstDir05 
    os.system ("robocopy /e /ns /nc /nfl /ndl /np  %s %s" % (srcDir05, dstDir05))    
