        ...
        if objTypeName in ['ifd','arnold']:
            if objPath == _ROP:
                ...
                if _PIC_NAME_ORG !='' and _PIC_NAME_ORG != 'ip':
                    ...
                    obj.parm(_PIC_TYPE).set(_PIC_NAME_NEW)
                    _THIS_PIC_NAME_NEW = obj.evalParm(_PIC_TYPE)
                    print ('    RENDER REDIRECT AS: ' + _THIS_PIC_NAME_NEW + '\r')
                ...
                EF = obj.evalParm('f2')
                BF = obj.evalParm('f3')
                print ('    RENDERING '+ _ROP + ' @ ' + str(SF) + '~' + str(EF) + '-' + str(BF))
    ...
    for obj in hou.node('/').allSubChildren():
        ...
        if objTypeName in ['file']:
            try:
                ...
                print "    missingframe skipped @ ", objName
            except:
                print "    Node or attribute locked", objName
        if objTypeName in ['alembic']:
            ...
            if not os.path.exists(_fileName_alembic) and os.path.isfile(_fileName_alembic):
                try:
                    ...
                    print "    missing fileName skipped @ ", objName
                except:
                    print "    Node or attribute locked", objName
