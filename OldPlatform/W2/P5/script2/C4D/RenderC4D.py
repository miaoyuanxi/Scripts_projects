...
class C4D(RenderBase):
    def __init__(self, **paramDict):
        ...
        self.G_PROCESS_LOG.info('[c4d argv]' + sys.argv[5])
        # self.G_IMAGE_WIDTH=""
        # self.G_IMAGE_HEIGHT=""
        self.G_JOB_NAME = ""
        self.G_IFORMAT = ""
        ...
        for files in fileList:
            ...
            for name in sample_list:
                if files == name:
                    isExist = True
            if isExist:
                self.G_PROCESS_LOG.info(files + '----exists---')
                existCount = existCount + 1
            else:
                self.G_PROCESS_LOG.info(files + '----not exists---')
    ...
    def setPluginByPyFile(self):
        self.G_PROCESS_LOG.info('\n')
        self.G_PROCESS_LOG.info('[c4d.Plugin.config.start...]')
        self.G_PROCESS_LOG.info('[c4d.配置插件.开始...]'.decode('utf-8').encode('gbk'))
        if os.path.exists(self.G_PLUGINS):
            ...
            if isinstance(configJson["plugins"], dict):
                ...
                if os.path.exists(plugin_custom_script):#判断文件夹是否存在
                    self.G_PROCESS_LOG.info('plugin_custom_script: %s' % plugin_custom_script)
                    self.G_PROCESS_LOG.info("loading custom config!!!")
                    ...
                    self.G_PROCESS_LOG.info("loading default config!!!")
                    from C4DPluginManager import C4DPlugin, C4DPluginManagerCenter
                '''
                ...
                #self.G_PROCESS_LOG.info('[copy_R18_exe.done...]')
                #self.G_PROCESS_LOG.info('[R18的64bit拷贝完成...]'.decode('utf-8').encode('gbk'))
                #plugin_list = ()
                for key in configJson["plugins"]:
                    ...
                    self.G_PROCESS_LOG.info('[ python = C:\\Python27\\python.exe "B:\\plugins\\C4D\\script\\C4DPluginManager.py" "%s" "%s" "%s" "%s" ]' % (plugin.plugin_name, \
                                                                                plugin.plugin_version, \
                                                                                plugin.soft_version, \
                                                                                plugin.node))
                    #plugin_list.append(plugin)  
                    plugin_mgr.get_dll(plugin.soft_version)
                    self.G_PROCESS_LOG.info('[c4d.libmmd.dll copy done..]')
                    self.G_PROCESS_LOG.info('[c4d.R19.053 copy done..]')
                    
                    result = plugin_mgr.set_custom_env(plugin)

                #result = plugin_mgr.set_custom_envs(plugin_list)     
        
        self.G_PROCESS_LOG.info('[c4d.Plugin.config.end...]')
        self.G_PROCESS_LOG.info('[c4d.配置插件.结束...]'.decode('utf-8').encode('gbk'))
        self.G_PROCESS_LOG.info('\n')
    ...
    def RBrender(self):  # 7
        self.G_PROCESS_LOG.info('[C4D.RBrender.start...]')
        self.G_PROCESS_LOG.info('[C4D.cmd渲染命令.开始...]'.decode('utf-8').encode('gbk'))
        startTime = time.time()
        ...
        self.G_PROCESS_LOG.info('IMAGENAME = ' + os.environ.get('IMAGENAME'))
        if os.path.exists(c4dExePath) == False:
            self.G_PROCESS_LOG.error(c4dExePath + ' is not exists!!!')
            self.G_PROCESS_LOG.error('IMAGENAME = ' + os.environ.get('IMAGENAME'))
            return
        ...
        # C:\Program Files\MAXON\CINEMA 4D R15\CINEMA 4D 64 Bit.exe -noopengl -nogui -frame 1 1 -render "D:\pipi.c4d" -oresolution 1920 1080 -oformat TGA -oimage C:\enfwork\213282\output\warehouse_4
        ...
        #self.G_PROCESS_LOG.info("Cmds渲染命令: %s"%renderCmd)
        ...
        self.G_PROCESS_LOG.info('[C4D.RBrender.end...]')
        self.G_PROCESS_LOG.info('[C4D.cmd渲染命令.结束...]'.decode('utf-8').encode('gbk'))
        self.G_PROCESS_LOG.info('\n')
    ...
    def RBexecute(self):  # Render
        ...
        self.RBBackupPy()
        self.RBinitLog()
        self.G_PROCESS_LOG.info('[C4D.RBexecute.start.....]')
        self.G_PROCESS_LOG.info('[C4D.渲染.开始.....]'.decode('utf-8').encode('gbk'))
        self.RBnodeClean()
        self.RBprePy()
        self.RBmakeDir()
        self.RBcopyTempFile()
        ...
        self.RBhanResult()
        self.RBpostPy()
        self.G_PROCESS_LOG.info('[C4D.RBexecute.end.....]')
        self.G_PROCESS_LOG.info('[C4D.渲染.结束.....]'.decode('utf-8').encode('gbk'))