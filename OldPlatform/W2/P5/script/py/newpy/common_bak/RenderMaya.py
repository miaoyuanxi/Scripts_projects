...
class Maya(RenderBase):
	...
	def copyBlack(self):
		...
		blackPath="\\\\10.60.100.7\\vcfs\\input\\td\\tools\\sweeper\\black.xml".replace(self.inputIp,self.ip)
		#blackPath="B:\\td\\tools\\sweeper\\black.xml"
		if self.G_RENDEROS=='Linux':
		...
		if os.path.exists(self.G_CONFIG):
			...
			if isinstance(configJson["mntMap"],dict):
				for key in configJson["mntMap"]:
					...
					cmd='mount -t cifs -o username=enfuzion,password=abc123456,codepage=936,iocharset=gb2312 '+mntPath.replace(self.inputIp,self.ip).replace('\\','/')+' '+key
...
class MayaClient(Maya):
	...
	def RBrender(self):#7
		...
		ip = 'www.storage.com'#self.getIpByNode()
		inputIp='10.60.100.7'
		 #C:/script/maya/Render4.bat "$userId"  "$taskId"  "$enginePath/bin/Render.exe" "C:/enfwork/$taskId/sc13_bg_scenes_TXCQ_SQ013_bg_lt_color_0210_cam_M.mb_20140210154833_487211732" $frame  $frame 1  "$projectPath"  "$filePath"  >>$logDir$ENFJOBNAME.txt 2>&1  
		renderCmd=renderBat +' "'+self.G_USERID+'"  "'+self.G_TASKID+'"  "'+mayaExePath+'" "" "'+self.G_CG_START_FRAME+'" "' + self.G_CG_END_FRAME +'" "'+self.G_CG_BY_FRAME +'" "'+ self.G_PATH_INPUTPROJECT.replace(inputIp,ip) +'" "'+ self.G_PATH_INPUTFILE+'" "'+ self.G_CG_TILE+'" "'+ self.G_CG_TILECOUNT+'" '
