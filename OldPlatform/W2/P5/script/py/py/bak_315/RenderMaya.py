...
class Maya(RenderBase):
	...
	def RBhanFile(self):#3 copy script,copy from pool,#unpack
		self.G_RENDER_LOG.info('[Maya.RBhanFile.start.....]'+self.G_NODE_TYPE)
		
		if self.G_NODE_TYPE=="gpu":
	...
	def webNetPath(self):
		if os.path.exists(self.G_CONFIG):
			...
			print configJson
			if isinstance(configJson["mntMap"],dict):
				# if not self.G_NODE_TYPE=="gpu":
				# 	self.RBcmd("net use * /del /y")
				for key in configJson["mntMap"]:
					self.RBTry3cmd("net use "+key+" "+configJson["mntMap"][key])		
