...
#           os.system('xcopy /y /v /f  "\\\\10.50.1.3\\p\\script\\py\\cleanNode.py" "c:/script/py/"')
            ...
            for root, dirs, files in list_dirs: 
                ...
                for name in files:
                    ...
                    if ext=='.exr' and os.path.exists(oiioPath):
                        ...
                        convertCmd=oiioPath+' "'+workBigPic+'" -resize '+self.getConvertR(workBigPic,oiioPath)+' -o "'+workSmallPic+'"'
                    #print workBigPic
    ...
    def RBresultAction(self):
        ...
        if self.G_RENDEROS=='Linux':
            ...
            outputFolder=self.G_PATH_USER_OUTPUT[:self.G_PATH_USER_OUTPUT.rfind(self.G_USERID)+len(self.G_USERID)]
            userLog=os.path.join(outputFolder,"renderlog",datetime.datetime.now().strftime('%Y-%m-%d'),self.G_TASKID)
            processLogDir=os.path.join(self.G_LOG_WORK,self.G_TASKID)
            ...
            logcmd = 'c:\\fcopy\\FastCopy.exe /speed=full /force_close  /no_confirm_stop /force_start "' +processLogDir.replace('/','\\')+'\\*.*" /to="'+userLog.replace('/','\\')+'"'
            self.G_PROCESS_LOG.info('[logcmd.....]'+logcmd)
