...
class C4D(AnalysisBase):
	...
	def getNode(self):
		node=sys.argv[5]
		if "A" in node or "B" in node or "C" in node or "D" in node or "E" in node or "F" in node or "G" in node or "H" in node or "J" in node:
			return "ABCDEFGHJ"
		if "K" in node:
			return "K"
		if "L" in node or "N" in node or "O" in node or "P" in node or "Q" in node:
			return "LNOPQ"
		return "ABCDEFGHJ"
