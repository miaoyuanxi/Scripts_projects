...
class RenderBase():
    ...
    def __init__(self,**paramDict):
        ...
        self.ip = 'www.storage.com'#self.getIpByNode()
        self.inputIp='10.60.100.7'
        self.G_POOL=paramDict['G_POOL']
        self.G_POOL= self.G_POOL.replace(self.inputIp,self.ip)
        # 开始帧 
        ...
        if 'G_CONFIG' in paramDict:
            ...
            self.G_CONFIG= self.G_CONFIG.replace(self.inputIp,self.ip)
        ...
        if 'G_PLUGINS' in paramDict:
            ...
            #nono__________self.G_PLUGINS= self.G_PLUGINS.replace(self.inputIp,self.ip)
        self.G_CG_TILE="0"
        ...
        if self.G_USERID=='1801684' or self.G_USERID=='1811222':
...
#           os.system('xcopy /y /v /f  "\\\\10.50.1.3\\p\\script\\py\\cleanNode.py" "c:/script/py/"')
    ...
    def netPath(self):
        ...
        inputDataPath=self.argsmap['inputDataPath']#nono_______.replace(self.inputIp,self.ip)
        try:
            self.delSubst()
        except:
            pass
        ...
        cmd='try3 net use B: '+pluginPath.replace('/','\\')#nono_______.replace(self.inputIp,self.ip).replace('/','\\')
        self.RBcmd(cmd) 
    ...
    def RBreadCfg(self):#4
        ...
        for eachLine in allLines:
            if eachLine.startswith('#') or eachLine=='\n' or eachLine.startswith('self.'):
                continue
            execStr='self.'+eachLine.strip().replace(self.inputIp,self.ip)
            print execStr
            exec(execStr)
    ...
    def RBconvertSmallPic(self):    
        ...
        #self.G_PATH_SMALL = self.G_PATH_SMALL.replace(self.inputIp,self.ip)
    ...
    def RBresultAction(self):
        ...
        feeLogFile=self.G_USERID+'-'+self.G_TASKID+'-'+self.G_JOB_NAME+'.txt'
        feeTxt=os.path.join(self.G_RENDER_WORK_TASK,feeLogFile)
        #nono____________self.G_PATH_USER_OUTPUT =self.G_PATH_USER_OUTPUT.replace(self.inputIp,self.ip)
        if self.G_RENDEROS=='Linux':
            outputPath="/output"
            ...
            #nono__________cmd2='"' +frameCheck.replace(self.inputIp,self.ip) + '" "' + self.G_RENDER_WORK_OUTPUT + '" "'+ output+'"'
            ...
            #nono__________cmd4='xcopy /y /f "'+feeTxt+'" "'+self.G_PATH_COST.replace(self.inputIp,self.ip).replace('/','\\')+'/"'#.replace("\\mnt\\","\\\\10.50.244.116\\")+'/" '
                ...
                if self.argsmap['onlyphoton'] in 'true' and self.argsmap['kg'] != '102':
                    ...
                    if self.argsmap.has_key('currentTask'):
                        if self.argsmap['currentTask'] in 'photon':
                            #nono_________photonPath=self.argsmap['inputDataPath'].replace(self.inputIp,self.ip)+self.G_USERID_PARENT+"/"+self.G_USERID+"/"+self.argsmap['projectSymbol']+"/max/photon/"
    ...
    '''
        w2机房获取对应的存储，预计每个节点机段为240台，现在只有4段
    '''
    def getIpByNode(self):
