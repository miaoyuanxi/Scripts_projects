...
class RenderBase():
    ...
    def __init__(self,**paramDict):
        ...
        if 'G_PLUGINS' in paramDict:
            ...
            self.G_PLUGINS= self.G_PLUGINS.replace(self.inputIp,self.ip)
        self.G_CG_TILE="0"
...
#           os.system('xcopy /y /v /f  "\\\\10.50.1.3\\p\\script\\py\\cleanNode.py" "c:/script/py/"')
    ...
    def netPath(self):
        ...
        inputDataPath=self.argsmap['inputDataPath'].replace(self.inputIp,self.ip)
        try:
            self.delSubst()
        except:
            pass
        ...
        cmd='try3 net use B: '+pluginPath.replace(self.inputIp,self.ip).replace('/','\\')
        self.RBcmd(cmd) 
    ...
    def RBresultAction(self):
        ...
        feeLogFile=self.G_USERID+'-'+self.G_TASKID+'-'+self.G_JOB_NAME+'.txt'
        feeTxt=os.path.join(self.G_RENDER_WORK_TASK,feeLogFile)
        self.G_PATH_USER_OUTPUT =self.G_PATH_USER_OUTPUT.replace(self.inputIp,self.ip)
        if self.G_RENDEROS=='Linux':
            outputPath="/output"
            ...
            cmd2='"' +frameCheck.replace(self.inputIp,self.ip) + '" "' + self.G_RENDER_WORK_OUTPUT + '" "'+ output+'"'
            ...
            cmd4='xcopy /y /f "'+feeTxt+'" "'+self.G_PATH_COST.replace(self.inputIp,self.ip).replace('/','\\')+'/"'#.replace("\\mnt\\","\\\\10.50.244.116\\")+'/" '
                ...
                if self.argsmap['onlyphoton'] in 'true' and self.argsmap['kg'] != '102':
                    ...
                    if self.argsmap.has_key('currentTask'):
                        if self.argsmap['currentTask'] in 'photon':
                            photonPath=self.argsmap['inputDataPath'].replace(self.inputIp,self.ip)+self.G_USERID_PARENT+"/"+self.G_USERID+"/"+self.argsmap['projectSymbol']+"/max/photon/"
                            frameCheck = os.path.join(self.G_POOL,'tools',self.G_SINGLE_FRAME_CHECK)
