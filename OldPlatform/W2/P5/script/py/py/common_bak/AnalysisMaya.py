			...
			if isinstance(configJson["mntMap"],dict):
				for key in configJson["mntMap"]:
					if not os.path.exists(key):
						os.makedirs(key)
					cmd=cmd='mount -t cifs -o username=enfuzion,password=abc123456,codepage=936,iocharset=gb2312 '+configJson["mntMap"][key].replace(self.inputIp,self.ip).replace('\\','/')+' '+key
