...
class Merge(RenderBase):
    ...
    def RBrender(self):
        ...
        if self.isScriptUpdateOk(20160900):
            photonPath=os.path.join(self.G_RENDER_WORK_TASK,'photon/source/')
            photonUserPath=os.path.join(self.argsmap['tempPath'],self.G_TASKID,'photon','*.vrmap')
            #photonUserPath=self.argsmap['inputDataPath']+self.G_USERID_PARENT+"/"+self.G_USERID+"/photon/"+self.G_TASKID+"/*.vrmap"
            copyVrmapCmd=r'c:\fcopy\FastCopy.exe /speed=full /force_close /no_confirm_stop /force_start "'+photonUserPath.replace('/','\\')+'" /to="'+photonPath.replace('/','\\')+'"'
    ...
    def RBhanResult(self):#8
        ...
        # 只渲染光子
        if self.argsmap.has_key('onlyphoton'):
            self.G_PROCESS_LOG.info('[onlyphoton.....]'+self.argsmap['onlyphoton'])
            if self.argsmap['onlyphoton'] in 'true':
                self.RBresultAction()
                pass
            else:
