...
#           os.system('xcopy /y /v /f  "\\\\10.50.1.3\\p\\script\\py\\cleanNode.py" "c:/script/py/"')
    ...
    def RBBackupPy(self): #
        ...
        runBat=os.path.join(self.G_RENDER_WORK_TASK_CFG,runBatName)  #C:\work\render\9321156\cfg\script_2017061301461_0.bat
    ...
    def readPyCfg(self):
        ...
        pypath=self.G_CFG_PYNAME  #C:/work/render/9321156/cfg/py.cfg
        if self.G_CONFIG !=None and os.path.exists(self.G_CONFIG):
            pypath=self.G_CONFIG  #\\10.60.100.101\p5\temp\9321156_render\cfg\py.cfg
    ...
    def RBinitLog(self):#2
        ...
        if not os.path.exists(processLogDir):
            os.makedirs(processLogDir)  #C:/log/render/9321156
        ...
        feeTxt=os.path.join(self.G_RENDER_WORK_TASK,feeLogFile)  #C:\work\render\9321156\1852991-9321156-frame0000.txt
    ...
    def RBmakeDir(self):#1
        ...
        #renderwork
        if not os.path.exists(self.G_RENDER_WORK_TASK):
            os.makedirs(self.G_RENDER_WORK_TASK)  #C:/work/render/9321156
        
        #renderwork/output
        if not os.path.exists(self.G_RENDER_WORK_OUTPUT):
            os.makedirs(self.G_RENDER_WORK_OUTPUT)  #C:/work/render/9321156/output
            
        #renderwork/outputbak
        if not os.path.exists(self.G_RENDER_WORK_OUTPUTBAK):
            os.makedirs(self.G_RENDER_WORK_OUTPUTBAK)  #C:/work/render/9321156/outputbak

        #RB_small
        rbSamll = os.path.join(self.G_RENDER_WORK,self.G_TASKID,'RB_small')
        self.RBlog(rbSamll)
        if not os.path.exists(rbSamll):
            os.makedirs(rbSamll)  ##C:/work/render/9321156/RB_small
    ...
    def RBhanFile(self):#3 copy script,copy from pool,#unpack
        ...
        if self.G_RENDERTYPE=="gpu":
            self.G_RENDER_WORK_OUTPUT = os.path.join(self.G_RENDER_WORK_TASK,'output',self.G_CG_START_FRAME+'_'+self.G_CG_END_FRAME+'_'+self.G_CG_BY_FRAME)  #C:/work/render/9321156/output/0/0/1
            self.G_RENDER_WORK_OUTPUTBAK = os.path.join(self.G_RENDER_WORK_TASK,'outputbak',self.G_CG_START_FRAME+'_'+self.G_CG_END_FRAME+'_'+self.G_CG_BY_FRAME)  #C:/work/render/9321156/outputbak/0/0/1
            if not os.path.exists(self.G_RENDER_WORK_OUTPUT):
    ...
    def RBcopyTempFile(self):
        ...
        tempFull=self.G_POOL_TASK  #\\10.60.100.101\p5\temp\9321156_render
        self.RBlog(tempFull)
        #if self.G_RENDEROS=='Linux':
        #    tempFull=self.G_POOL_TASK
        self.pythonCopy(tempFull.replace('\\','/'),self.G_RENDER_WORK_TASK.replace('\\','/'))  #\\10.60.100.101\p5\temp\9321156_render --> C:/work/render/9321156
        ...
        if  os.path.exists(dir):
            ...
            for root, dirs, files in list_dirs: 
                for name in files:
                    ...
                    self.G_PROCESS_LOG.info('name='+name)  #rayvision_0000.jpg
                    if ext == '.vrmap' or   ext == '.vrlmap' or ext=='.exr':
                        continue    
                    workBigPic =os.path.join(root, name)  #C:/work/render\9321156\output\rayvision_0000.jpg
                    smallName=workBigPic.replace(dir+'\\','') #rayvision_0000.jpg
                    smallName=self.G_JOB_NAME+"_"+smallName.replace('\\','[_]').replace('.','[-]')+'.jpg'  #frame0000_rayvision_0000[-]jpg.jpg
                    smallTmpName = self.G_JOB_NAME+'_tmp.jpg'  #frame0000_tmp.jpg
                    workSmallPic=os.path.join(workSmallPath,smallName)  #C:/work/render\9321156\small\frame0000_rayvision_0000[-]jpg.jpg
                    workSmallTmpPic=os.path.join(workSmallPath,smallTmpName)  #C:/work/render\9321156\small\frame0000_tmp.jpg


                    workBigPicTmp=os.path.join(root, 'tmp'+os.path.splitext(workBigPic)[1] )    #C:/work/render\9321156\output\tmp.jpg
                    try:
                    ...
                    #print workBigPic
                    try:
                        self.RBcmd(convertCmd,True,False)  #c:/ImageMagick/nconvert.exe -out jpeg -ratio -resize 200 0 -overwrite -o "C:/work/render\9321156\small\frame0000_tmp.jpg" "C:/work/render\9321156\output\tmp.jpg"
                    except Exception, e:
    ...
    def RBconvertSmallPic(self):    
        ...
        workSmallPath=os.path.join(self.G_RENDER_WORK_TASK,'small')  #C:/work/render/9321156/small
        self.RBlog(workSmallPath)
        self.RBlog(self.G_PATH_SMALL)
        if not os.path.exists(workSmallPath):
            os.makedirs(workSmallPath)
            
        exrToJpg=os.path.join(self.G_RENDER_WORK_TASK,'exr2jpg')  #C:/work/render/9321156/exr2jpg
        exrToJpgBak=os.path.join(self.G_RENDER_WORK_TASK,'exr2jpgbak')  #C:/work/render/9321156/exr2jpgbak
        self.RBconvertDir(self.G_RENDER_WORK_OUTPUT,workSmallPath)  #C:/work/render/9321156/output,C:/work/render/9321156/small
        self.RBconvertDir(exrToJpg,workSmallPath)  #C:/work/render/9321156/exr2jpg,C:/work/render/9321156/small
    ...
    def RBresultAction(self):
        ...
        if not os.path.exists(self.G_PATH_SMALL):
            os.makedirs(self.G_PATH_SMALL)  #\\10.60.100.201\d\small5\1852500\1852991\9321156\
        frameCheck = os.path.join(self.G_POOL,'tools',"SingleFrameCheck.exe")  #\\10.60.100.101\p5\tools\SingleFrameCheck.exe
        feeLogFile=self.G_USERID+'-'+self.G_TASKID+'-'+self.G_JOB_NAME+'.txt'
        feeTxt=os.path.join(self.G_RENDER_WORK_TASK,feeLogFile)  #C:/work/render/9321156/1852991-9321156-frame0000.txt
        if self.G_RENDEROS=='Linux':
            outputPath="/output"
