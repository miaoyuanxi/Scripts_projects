...
class MaxClient(RenderBase):
    ...
    def startGrab(self):
        ...
        startSocket='rdf1grab|'+grabRate+'|'+grabFolderLocal+'|'+grabFolderServer+'|'+self.G_JOB_NAME_STR+'|'+self.G_NODE_NAME
    ...
    def RBrender(self):#7
        ...
        if self.MAX_VRAY_DISTRIBUTE:
            ...
            def send_cmd(thread_name,localIP,nodeIP):
                ...
                from_addr = self.G_POOL+"\\script\\py\\py\\962712\\MaxDistribute.py"
                from_addr2 = self.G_POOL+"\\script\\py\\py\\962712\\MaxPlugin.py"
                to_addr = r'\\' + nodeIP + r'\c$\script\py'
                print to_addr
                copyNodeScriptPath='xcopy /Y /E /F "%s" "%s"' % (from_addr,to_addr)
                copyNodeScriptPath2='xcopy /Y /E /F "%s" "%s"' % (from_addr2,to_addr)
            ...
            if nodeIP_list:
                ...
                if self.PLUGIN_DICT['plugins']['vray'].startswith('1'):
                    print 'vray1\n'
                    content_bottom = """restart_slaves 0
list_in_scene 0
"""
