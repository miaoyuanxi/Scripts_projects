...
:render_18445
	...
	set projPath=%proj:18445\\=18445\%
	set mayaFile=%mb:18445\\=18445\%
	set projPath=%projPath:\=/%
	set mayaFile=%mayaFile:\=/%
	...
	set MAYA_SCRIPT_PATH=\\10.50.8.2\p\script\maya\nCZSupport;\\10.50.8.2\p\script\maya\support\maya\huanqiu_scripts
	set MAYA_PLUG_IN_PATH=\\10.50.8.2\p\script\maya\support\plugins\18445_huanqiu
	
	echo set mel=%MAYA_SCRIPT_PATH%/FixRenderLayerOutAdjustmentErrors.mel

	"%render%" -s %startframe% -e %endframe% -b %byframe% -proj "%projPath%" -rd %rd% -preRender "source \"%mel%\";" -mr:art -mr:aml "%mayaFile%"  
	goto end
