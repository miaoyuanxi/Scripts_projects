...
:default
    ...
    set user_path="%py_path%\User\%userId%\cgrender.py"
    echo user_path :: %user_path%
    if exist %user_path%  set py_path=%py_path%\User\%userId%
	echo "Using cgrender.py to render"
	echo "c:\python27\python.exe" "%py_path%\cgrender.pyc" --js %json% --pl %plugins_json% --sf %startframe% --ef %endframe% --by %byframe% --sp %proj% --tile_index %tile_index% --tiles %tiles% --pt %pt%
	"c:\python27\python.exe" "%py_path%\cgrender.pyc" --js %json% --pl %plugins_json% --sf %startframe% --ef %endframe% --by %byframe% --sp %proj%  --tile_index %tile_index% --tiles %tiles%  --pt %pt%
goto end
