...
module SketchupRenderModule
    ...
    def render_vray
        begin
            ...
            if $total_frame==1
                ...
                outputLog("outputfile:"+""+$output_directory+$output_name+ "%05d" % $currentframe + "#{$format}" +"")
                VRayForSketchUp.setOutputPath(""+$output_directory+$output_name+ "%05d" % $currentframe + "#{$format}" +"")
    ...
    def render_image
        ...
        $output_path = File.join(""+$output_directory+$output_name+ "%05d" % $currentframe + "#{$format}" +"")
        outputLog("$output_path:" + $output_path)
        begin
            if $use_vray == "True"
                render_vray
            else
