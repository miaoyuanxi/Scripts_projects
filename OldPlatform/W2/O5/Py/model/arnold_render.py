...
#                    print a
    ...
    @staticmethod
    def kill_exe(exe):
        processes = RvOs.get_process_list(exe)
        if processes:
            for i in processes:
                print "kill %s %s" % (exe, str(i))
                sys.stdout.flush()
...
class Render(object):
    ...
    def __init__(self, task_id, start, end, by, pt=1005):
        self.pt = int(pt)
...
class KickRender(Render, RvOs):

    def __init__(self, task_id, start, end, by=1, pt=1005):
        Render.__init__(self, task_id, start, end, by, pt)
        if self.cfg_info:
            self.kill_exe("kick.exe")
            self.get_kick_info()
            self.render()
        else:
    ...
    def get_kick_info(self):
        ...
        elif int(self.cfg_info["user_id"]) in [961900, 963697]:
            if self.pt == 1005:
                plugin_server = "10.50.24.11"
                custom_server = "10.50.1.22"
            elif self.pt == 1002:
                plugin_server = "10.60.100.102"
                custom_server = "10.60.100.102"

            self.cfg_info["kick_exe"] = '//%s/d/plugins/961900/arnold/bin/kick.exe' % (plugin_server)
            self.cfg_info["shader_path"] = '//%s/d/plugins/961900/arnold/shaders' % (plugin_server)

            srcDir1= r"\\%s\td\clientFiles\961900\rlm" % (custom_server)
            dstDir1=r'"C:\AMPED_mili"'
            os.system ("robocopy /s  %s %s" % (srcDir1, dstDir1))

            srcDir= r"\\%s\td\clientFiles\961900\Yeti" % (custom_server)
            dstDir=r'"C:\Yeti"'
            os.system ("robocopy /s  %s %s" % (srcDir, dstDir))

            os.environ["PATH"] += ";"
            os.environ["PATH"] += r"C:/Yeti/bin"
            os.environ['MTOA_EXTENSIONS_PATH'] = r'C:\Yeti\plug-ins'
            os.environ['MTOA_PROCEDURAL_PATH'] = r'C:\Yeti\bin'
            os.environ['PEREGRINEL_LICENSE'] = r'C:\AMPED_mili\peregrinel.lic'
            # os.environ['PATH'] = os.environ.get('PATH') + r';C:\Yeti\bin'
            os.system(r'wmic process where name="JGS_mtoa_licserver.exe" delete')
            os.system(r'wmic process where name="rlm.exe" delete')
            os.system(r'start C:\AMPED_mili\rlm.exe')
            time.sleep(2)
            hasEnv = False
            host = r"C:/Windows/system32/drivers/etc/hosts"
            with open(host, "a+") as f:
                lines = [i.strip() for i in f.readlines()]
                if "%s hnas02" % (plugin_server) in lines and "%s hnas01" % (plugin_server) in lines:
                        hasEnv = True
                if not hasEnv:
                    f.write('\n')
                    f.write("%s hnas02" % (plugin_server))
                    f.write('\n')
                    f.write("%s hnas01" % (plugin_server))
                    f.write('\n')

            os.environ["PATH"] += ";"
            # os.environ["PATH"] += "//10.50.24.11/d/plugins/961900/Yeti/bin;"
            os.environ["PATH"] += r"\\%s\d\plugins\961900\dll" % (plugin_server)

            self.copytree(r"\\%s\d\plugins\961900\dso" % (plugin_server),
                          r"C:/solidangle/dso")
            self.copytree(r"\\%s\d\plugins\961900\dll" % (plugin_server),
                         r"C:\Windows\system32")
            self.copytree(r"\\%s\d\plugins\961900\arnold\procedurals" % (plugin_server),
                         r"C:/solidangle/mtoadeploy/2014/procedurals")
        else:
