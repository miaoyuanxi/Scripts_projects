...
class ArnoldClass(Render):
    ...
    def set_plugins(self):
        ...
        if self["common"]["userId"] in [1841236]:
...
class MayaClass(Render):
    ...
    def set_plugins(self):
        if self["common"]["plugin_file"]:
            ...
            sys.stdout.flush()
    def import_file(self,py_file,def_name,arg=''):
        if os.path.exists(py_file):
            py_file_dir = os.path.split(py_file)[0]
            py_file_name_ext = os.path.split(py_file)[1]
            py_file_name = os.path.splitext(py_file_name_ext)[0]
            py_file_ext = os.path.splitext(py_file_name_ext)[1]
            
            if py_file_ext.lower() in ['.py','.pyc']:
                sys.path.append(py_file_dir)
                try:                    
                    pyf = __import__(py_file_name)
                    reload(pyf)
                    #str = "pyf."+ def_name +"(" + "%s" +")"
                    str = "pyf.%s(%s)" % (def_name,arg)
                    print str
                    exec(str)
                    
                except Exception as err:
                    print('=== Error occur import/execute "%s"! ===\n=== Error Msg : %s ===' % (py_file, err))
                try:
                    while True:
                        sys.path.remove(py_file_dir)            
                except:
                    pass
...
class MayaRender(MayaClass):
    ...
    def net_analyse_dict(self):
        analyse_net_file = self["common"]["analyseTxt"].replace("\\","/")
        print analyse_net_file
        with open(analyse_net_file,"r") as f:
            analyse_dict ={}
    ...
    #                    cmd += " -im \"%s\"" % image_name
        ...
        print self["mappings"]
        if self["common"]["userId"] in [1017637,1844577,1820999,1857698]:
        ...
        if self["common"]["userId"] in [1850239,963287, 963492, 963250, 964462,965366]:
            cmd += " -r arnold -ai:lve 1"
        if self["common"]["userId"] in [1849549,1824747]:
            if "mtoa" in self["plugins"]["plugins"]:
                cmd += " -r arnold -ai:lve 1"
        if self["common"]["userId"] in [1840132]:
            if "mtoa" in self["plugins"]["plugins"]:
                cmd += " -r arnold -ai:lve 1 -ai:txmm 10240"
        if self["common"]["userId"] in [1842263]:
        ...
        if self["common"]["userId"] and "-prerender" not in cmd:
            custom_file = self["platform"]["custom_config"] + "/" + str(self["common"]["userId"]) + "/prerender_CustomConfig.py"
            '''
            print self
            print "write self to json"
            try:
                
                inof_json_path = self["common"]["tmp"].replace("\\", "/")+r"/selft_json.json"
                
                print inof_json_path
                with open(inof_json_path, 'w') as f:
                    f.write(json.dumps(self))
                    f.close()
                cmd += " -preRender \"python \\\"self_json = \\\\\\\"%s\\\\\\\";execfile(\\\\\\\"%s\\\\\\\")\\\"\"" % (inof_json_path,custom_file)

            except Exception as err:
                print  err
                pass
            '''
            analyse_dict = self.net_analyse_dict()
            self = dict(self, **analyse_dict)
            print "\n\n\n"*5
            print self
            print "\n\n\n"*5
            #self.import_file(custom_file,"pre_Config",self["renderSettings"])
            #cmd += " -preRender \"python \\\"user_id=%s;mapping=%s;plugins=%s;taskid=%s;rendersetting=%s;execfile(\\\\\\\"%s\\\\\\\")\\\"\"" % (self["common"]["userId"], self["mappings"], self["plugins"]["plugins"], self["common"]["taskId"],self["renderSettings"] ,custom_file)
            #cmd += " -preRender \"python \\\"user_id=%s;mapping=%s;plugins=%s;taskid=%s;rendersetting=%s;platform = %s;execfile(\\\\\\\"%s\\\\\\\")\\\"\"" % (self["common"]["userId"], self["mappings"], self["plugins"], self["common"]["taskId"],self["renderSettings"] ,self["platform"],custom_file)
            #cmd += " -preRender \"python \\\"self=\\\\\\\"%s\\\\\\\";execfile(\\\\\\\"%s\\\\\\\")\\\"\"" % (self,currentPath)
