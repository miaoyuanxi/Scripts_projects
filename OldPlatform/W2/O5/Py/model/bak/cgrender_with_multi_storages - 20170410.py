...
def get_json(**kwargs):
    ...
    if kwargs["task_id"]:
        ...
        options["common"]["cgFile"] = cfg_info["cgFile"]
...
class Render(dict, Zip7):
    ...
    def __init__(self, options):
        ...
        self.check_mapping()
        # if self["common"]["userId"] not in [1844059]:
        #     self.check_7z()
        # else:
...
#        return subprocess.call(command, shell=1)
    ...
    def clean_network(self):
        return 0
        # if self["server"]["user_id"] in [1842805,961743,1842126,962371,963433]:
        #     return 0
    ...
    def render(self):
        ...
        if not self["common"]["plugin_file"]:
            ...
            if self["common"]["userId"] in [962276]:
                ...
                cmd = "\"%s\" && %s" % (pre_bat, cmd)
        if self["common"]["userId"] in [1017637,1843698,1832764,1833080,1833047]:
            cmd += " -preRender \"python \\\"user_id=%s;mapping=%s;plugins=%s;taskid=%s;execfile(\\\\\\\"//10.60.100.101/o5/py/model/prerender.py\\\\\\\")\\\"\"" % (self["common"]["userId"],self["mappings"],self["plugins"]["plugins"],self["common"]["taskId"])
        ...
        if "RenderMan_for_Maya" in self["plugins"]["plugins"]:
            ...
            if self["common"]["userId"] in [961743,1844765,964311]:
                #cmd +=" -preRender \"python \\\"execfile(\\\\\\\"//10.60.100.101/o5/py/model/961743.py\\\\\\\")\\\"\" -r redshift -logLevel 2"
                cmd +=" -preRender \"python \\\"user_id=%s;mapping=%s;execfile(\\\\\\\"//10.60.100.101/o5/py/model/prerender.py\\\\\\\")\\\"\" -r redshift -logLevel 2" % (self["common"]["userId"],self["mappings"])
            else:
        ...
        if self["server"]["father_id"] in [1819310, 1821185,1845831]:
            if not os.path.exists(self["renderSettings"]["output_redshift"]):
                os.makedirs(self["renderSettings"]["output_redshift"])
