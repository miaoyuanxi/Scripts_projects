...
class NukeMerge(Nuke):
    ...
    def render(self):
        ...
        for i in self.tile_files:
            if self["common"]["debug"]:
                print i
            tile_output = self["renderSettings"]["output"] + i[0].split("/0/")[1]
    ...
    def render(self):
        ...
        if not self["common"]["plugin_file"]:
            ...
            if self["common"]["userId"] in [962276]:
                ...
                cmd = "\"%s\" && %s" % (pre_bat, cmd)
        #if self["common"]["userId"] in [1814975]:
            #cmd += " -preRender \"python \\\"user_id=%s;execfile(\\\\\\\"//10.50.5.29/o5/py/model/prerender.py\\\\\\\")\\\"\"" % (self["common"]["userId"])
