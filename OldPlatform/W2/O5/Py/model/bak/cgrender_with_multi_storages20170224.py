...
class MayaRender(MayaClass):
    ...
    def render(self):
        ...
        if float(self["common"]["cgv"])==2017:
            os.system(r'"xcopy /y /f \\10.60.100.101\o5\py\model\maya2017\pluginPrefs.mel C:\users\enfuzion\Documents\maya\2017\prefs\ "')
        ...
        if "RenderMan_for_Maya" in self["plugins"]["plugins"]:
            ...
            if self["common"]["userId"] in [1820265]:
            ...
            if self["common"]["userId"] in [123456789]:
                gpu_n="0,1"
            cmd += " -gpu {%s}" % (gpu_n)
        ...
        if self["common"]["userId"] in [1838213]:
        ...
        if self["server"]["user_id"] in [1811222,1301816,963685,1816635,1801684]:
            ...
            cmd = "\"%(bat)s\" %(user_id)s %(task_id)s \"%(render.exe)s\" "\
                ...
                % self["renderSettings"]
        if self["server"]["user_id"] == 1842126:
            self["server"]["father_id"] = 1842126
        if self["server"]["user_id"] == 1841567:
            self["server"]["father_id"] = 1841567
        if self["server"]["user_id"] in [1842126,1841567]:
            if not os.path.exists(self["renderSettings"]["output_redshift"]):
                os.makedirs(self["renderSettings"]["output_redshift"])
