...
def get_platfom(platform):
    ...
    if info["platform"] == 1002:
        info["7z.exe"] = r"\\10.60.100.7\vcfs\input\o5\py\model\7z\7z.exe"
        info["plugin_path"] = r"\\10.60.100.7\vcfs\input\d\plugins"
        info["auto_plugins"] = r"\\10.60.100.7\vcfs\input\td"
        info["cfg_path"] = r"\\10.60.100.7\vcfs\input\d\ninputdata5"
        info["home_path"] = r"\\10.60.100.7\vcfs\input\d\inputdata5"
        info["custom_config"] = r"\\10.60.100.7\vcfs\input\td\custom_config"
...
class MayaRender(MayaClass):
    ...
    #                    cmd += " -im \"%s\"" % image_name
        ...
        if "RenderMan_for_Maya" in self["plugins"]["plugins"]:
            ...
            cmd += " -r redshift"
        elif self["common"]["userId"] in [962640,1814964,1814856,963287, 963492, 963250]:
        ...
        if self["server"]["father_id"] == 964309:
            if not os.path.exists(self["renderSettings"]["output_redshift"]):
                os.makedirs(self["renderSettings"]["output_redshift"])
            self["renderSettings"]["bat"] = r"\\10.172.69.203\p5\script\maya\of3d.bat"
            self["renderSettings"]["task_id"] = self["common"]["taskId"]
            cmd = "\"%(bat)s\" 964309 %(task_id)s \"%(render.exe)s\" "\
                "\"%(renderableLayer)s\" %(start)s %(end)s %(by)s " \
                "\"%(projectPath)s\" \"%(maya_file)s\" \"%(output_redshift)s\"" \
                % self["renderSettings"]
