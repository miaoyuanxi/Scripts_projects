...
class PreRender(dict,PreRenderBase):
    ...
    def rd_path(self):
        ...
        if rd_path:
            ...
            self.log_scene_set("Current Outpu setting ",self.unicode_to_str(rd_path))
        else:
            pass
    ...
    def han_custome_prerender_bak(self):
            ...
            for i in pm.ls(type="VRaySettingsNode"):
                if self.rendersetting:
                    ...
                    if 'height' in self.rendersetting and i.hasAttr("height"):
                        i.height.set(self.rendersetting['height'])
                        # if i.hasAttr("srdml"):
                        # if self.taskid in [9277600,9277601]:
                        # dml = 2500
                        # i.srdml.set(dml)
                        # print "set vray srdml %s MB" %(dml)
            if self.taskid in [9289969]:
        ...
        if self.user_id in [1845209]:
            for i in pm.ls(type="aiStandIn"):
                ...
                # if i.hasAttr("srdml"):
                # dml = 20000
                # if self.taskid in [9183335,9183346,9183337,9183224]:
                # dml = 32000
                # i.srdml.set(dml)
                # print "set vray srdml %s MB" %(dml)
        ...
        if self.user_id in [1855708, 900045, 1855110, 1833038, 1834849, 1857988, 1826800, 1860992, 1163411, 1862577,
                            ...
                            1833958,
                            1895687, 1869176, 1814856, 1894815, 1866675, 1857429, 1820648]:
            print self.start
            print int(self.start)
            if self.start:
                cmds.currentTime(int(self.start), edit=True)
        ...
        if self.user_id in [1814856]:
            ...
            self.render_node.preMel.set(l=0)
            self.render_node.preMel.set("")
            self.render_node.postMel.set(l=0)
            self.render_node.postMel.set("")
            self.render_node.preRenderLayerMel.set(l=0)
            self.render_node.preRenderLayerMel.set("")
