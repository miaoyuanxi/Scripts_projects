...
class MayaPluginBase():
    def __init__(self,pluginCfg = None,pluginPost = None,userId = '000',taskId = '000',myLog=None):
        ...
        self.MAYA_Plugin_Dir = self.GetJsonIni('MAYA_Plugin_Dir')
        self.Node_D = self.GetJsonIni('Node_D')
        self.toolDir = self.GetJsonIni('toolDir')
        
        self.solidangle_LICENSE = self.GetJsonIni('solidangle_LICENSE')
        self.Zipexe = "%s/7z.exe"%self.toolDir
        self.tempPath = os.getenv('temp').replace('\\','/')
        
        
        self.pluginPost = pluginPost
    ...
    def GetJsonIni(self,objkey,default = None,idex=0):
        info_path = self.JSON_PATH
        with open(info_path,'r') as fn:
            fn_dict = json.load(fn)
            tmp_dict = fn_dict[self.currentOS]
            def getdict(tmp_dict,objkey,default = None,idex=0):
                for k,v in tmp_dict.items():
                    if k == objkey:
                        ...
                        if type(v) is types.DictType:
                            re = getdict(v,objkey,default,idex)
                            if re is not default:
                                ...
                                return re
                return default
            key_v = getdict(tmp_dict,objkey,default,idex)
        return key_v
    ...
    def Un7z(self,zipfile,to,mylog=None):
        if mylog == None:
            mylog = self.tempPath + '/' + 'plugin.txt'
        un_times = 1
        while un_times<3:
            cmd = r'"%s" x -y -aos "%s" -o"%s"' % (self.Zipexe, zipfile, to)
            un_log = open(mylog,"wt")
    ...
    def Copy7z(self,source,to,mylog=None):
        if mylog == None:
            mylog = self.tempPath + '/' + 'plugin.txt'	
        cp_times = 0
        while cp_times < 3:
            cp_log = open(mylog,"wt")
            cmds_copy = "copy %s %s" % (os.path.abspath(source),
                        os.path.abspath(to))
    ...
    def doLoadPluginsCfg(self, *args):        
        ...
        if allPlugDict and allPlugDict.has_key('plugins'):
            ...
            if pluginDict:
                for pluginName in pluginDict:  
                    ...
                    pluginZipName = self.G_CG_NAME + self.G_CG_VERSION + '_' + pluginName + pluginVersion + self.ZipType
                    ...
                    messages = self.G_CG_NAME+self.G_CG_VERSION + '' + pluginName+'_'+pluginVersion
                    if os.path.exists(pluginConfigFile) and os.path.exists(pluginZipSource):     
                        ...
                        self.MyLog('[Error]----The  %s  file  is not exists!' % messages) 
            else:
    ...
    def configPlugin(self, *args):
        ...
        pluginValidList = self.getPluginIni()
        ...
        if len(pluginValidList) > 0:
            for plugindict in pluginValidList:
                if plugindict:
                    ...
                    self.CleanDir(NodeSoftware)
                    un_log = os.path.join(self.Node_D,plugindict['pluginName'],'logs','un_log.txt').replace('\\','/')
                    cp_log = os.path.join(self.Node_D,plugindict['pluginName'],'logs','cp_log').replace('\\','/')
                    if self.Find7z(pluginZipSource,NodeSourcePath):
                        self.Un7z(pluginZipNode,PluginDir,un_log)
                    else:
                        self.Copy7z(pluginZipSource,NodeSourcePath,cp_log)
                        self.Un7z(pluginZipNode,PluginDir,un_log)

                    self.doLoadPluginsCfg([pluginPy],validInfo)                    
        ...
        if self.pluginPost:
            self.doLoadPluginsCfg(self.pluginPost,validInfoPost)
    ...
    def config(self):
        ...
        self.MyLog( "RunningTime----%s".format(str(timeOut)))
        self.MyLog('\n\n-------------------------------------------------------[MayaPlugin]end----------------------------------------------------------\n\n')
