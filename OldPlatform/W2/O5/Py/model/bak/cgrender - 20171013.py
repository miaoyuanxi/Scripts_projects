...
class MayaRender(MayaClass):
    ...
    def render(self):
        ...
        if self["common"]["userId"] in [1865352]:
            cmd = "\"%(render.exe)s\" -s %(start)s -e %(end)s -b %(by)s " \
            "-proj \"%(projectPath)s\" -rd \"%(output)s\720\"" \
            % self["renderSettings"]
    ...
    #                    cmd += " -im \"%s\"" % image_name
        ...
        print self["mappings"]
        if self["common"]["userId"] in [1017637,1844577,1820999,1857698,1853469,1843589,1861628,1805997,1862416,1849290,1863429,1840132,1863854,1833038,1826800,1862577]:
        ...
        # if self["common"]["userId"] in [1840132]:
            # if "mtoa" in self["plugins"]["plugins"]:
                # cmd += " -r arnold -ai:lve 1 -ai:txmm 20480"
        if self["common"]["userId"] in [1857702]:
