...
if user_id in [1848725]:
    for i in pm.ls(type="VRaySettingsNode"):
        if rendersetting:
            ...
            if 'height' in rendersetting and i.hasAttr("height") :
                i.height.set(rendersetting['height'])    
if user_id in [1844765]:
    for i in pm.ls(type="RedshiftOptions"):
        if i.hasAttr("exrForceMultilayer"):
            i.exrForceMultilayer.set(False)
        if i.hasAttr("exrMultipart"):
            i.exrMultipart.set(True)
    for i in pm.ls(type="RedshiftAOV"):  
        if i.hasAttr("enabled"):
            if i.enabled.get():
                name_aov = i.attr('name').get()
                aov_fileprefix = i.filePrefix.get()            
                i.filePrefix.set(aov_fileprefix+'/'+name_aov)
                print i.filePrefix.get() 
