...
class MayaRender(Render):
    ...
    def set_plugins(self):
        ...
        if self.cfg_info["user_id"] in [120151]:
            ...
            plugin_path = r"\\10.50.24.11\d\plugins\120151"
        ...
        if self.cfg_info["user_id"] in [962413]:
            src = r"\\10.50.24.11\d\plugins\962413\anzovinRigNodes_1.0\maya_root"
            dst = r"C:\Program Files\Autodesk\Maya2015"
            print "copy all the files in %s to %s" % (src, dst)
            self.copytree(src, dst)

            src = r"\\10.50.8.16\td\arnold_gongxiang\solidangle\mtoadeploy\2015_1.2.1.0"
            dst = r"C:\solidangle\mtoadeploy\2015_1.2.1.0"
            print "copy all the files in %s to %s" % (src, dst)
            self.copytree(src, dst)

            src = r"\\10.50.24.11\d\plugins\962413\y1319m201315\Yeti-v1.3.19_Maya2015-windows64"
            dst = r"C:\Yeti-v1.3.19_Maya2015-windows64"
            print "copy all the files in %s to %s" % (src, dst)
            self.copytree(src, dst)

            modules_path = r'C:\users\enfuzion\Documents\maya\2015-x64\modules'
            env_path = r'C:\users\enfuzion\Documents\maya\2015-x64'

            plugin_path = r"\\10.50.24.11\d\plugins\962413"
...
#            mtoa_root = r"C:\solidangle\mtoadeploy\2015"
#            yeti_root = r"C:\Yeti-v1.3.19_Maya2015-windows64"
#
#            os.environ["MAYA_RENDER_DESC_PATH"] = mtoa_root
#            os.environ["PATH"] = "%PATH%;" + os.path.join(mtoa_root, "bin") + \
#                ";" + os.path.join(yeti_root, "bin")
#            os.environ["ARNOLD_PLUGIN_PATH"] = os.path.join(mtoa_root, "plug-ins")
#            os.environ["MTOA_EXTENSIONS_PATH"] = os.path.join(mtoa_root, "extensions") + \
#                ";" + os.path.join(yeti_root, "plug-ins")
#            os.environ["MAYA_SCRIPT_PATH"] = os.path.join(mtoa_root, "scripts")
#            os.environ["PYTHONPATH"] = os.path.join(mtoa_root, "scripts")
#            os.environ["ARNOLD_PLUGIN_PATH"] = os.path.join(mtoa_root, "shaders")
#            os.environ["peregrinel_LICENSE"] = os.path.join(yeti_root, "yeti.lic")
#            os.environ["MAYA_MODULE_PATH"] = yeti_root
#            os.environ["MTOA_PROCEDURAL_PATH"] = os.path.join(yeti_root, "bin")
