            ...
            if node.hasAttr(attr_name):
                ...
                if file_path != None and file_path.strip() !="" :
                    ...
                    if os.path.exists(file_path) == 0:                        
                        ...
                        if os.path.exists(file_path_new) == 0:
                            print "the %s node %s of  %s is not find" % (node_type,attr_name,node)
                        else:
...
attr_list = [{"VRayMesh": "fileName"}, {"AlembicNode": "abc_File"},{"pgYetiMaya": "groomFileName"},{"pgYetiMaya": "cacheFileName"},\
            ...
            {"VRayVolumeGrid": "inFile"},{"file":"fileTextureName"}\
             ]    
        ...
        # if len(all_file) != 0:
            # for file_a in all_file:
                ...
                # if os.path.exists(file_path)==0:
                    ...
                    # c = os.path.split(file_path)[1]
                    # file_path_new="%s/%s" %(source_path,c)
                    # if os.path.exists(file_path_new)==0:
                        
                        # file_path_new="%s/%s" %(scenes_path,c)
                    # if os.path.exists(file_path_new)==0:
