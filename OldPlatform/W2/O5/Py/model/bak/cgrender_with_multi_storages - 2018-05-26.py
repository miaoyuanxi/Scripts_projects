...
class NukeMerge(Nuke):
    ...
    def get_input_output(self):
        ...
        self.tile_files_list = [[re.sub(r'(.+/tiles/\d+/[-]?\d+)/(\d+)/(.+)', r"\1/%s/\3" % (index), i.replace("\\", "/"))
...
class MayaRender(MayaClass):
    ...
    def render(self):
        ...
        if self["common"]["userId"] in [962814,1849749,1858642,1858237,1858240,1858241,1858637,1858638,874812,1879925,1879928,1879929,1879930,1879931,1879932,1879934,1879935,1879936,1879937,1879938,1879963,1879966,1879967,1879968,1879969,1879970,1882048,1884724,1886445,1894815,1894816,1894817,1894831,1897697,1897990,1897991,1897992,1897994,1897996,1897997,1897998,1897999,1898000,1898001,1898002,1898003,1898004,1898006,1898007,1898008]:
        ...
        if self["common"]["userId"] not in [1894817,1818014,1844854,1875231,1846887,1854355,1814593,963931,1873017,1812148,1812148,1830380,1871175,1899488,1820991,1904764] and HQID:
...
#----------------------------------------------------------------------------------------------------------------------------------------------
        ...
        #tsanghao 1811213
        if self["common"]["userId"] in [1803147]:
            cmd += " -preRender \"python \\\"user_id=%s;mapping=%s;plugins=%s;taskid=%s;rendersetting=%s;start=%s;execfile(\\\\\\\"//10.60.100.101/o5/py/model/prerender.py\\\\\\\")\\\"\"" % (self["common"]["userId"],self["mappings"],self["plugins"]["plugins"],self["common"]["taskId"],self["renderSettings"],self["renderSettings"]["start"])
        ...
        # if self["common"]["taskId"] in [10299044]:
                # cmd += " -r arnold -ai:lve 1 -ai:as 2 -ai:alf 1 -ai:slc 0"
...
#            cmd +=" -r file "
        ...
        # if self["common"]["userId"] in [1863843]:
            # rsmap_list = []
