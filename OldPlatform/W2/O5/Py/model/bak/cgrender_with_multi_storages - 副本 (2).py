...
class MayaRender(MayaClass):
    ...
    def render(self):
        ...
        if self["common"]["userId"] in [962814]:
        ...
        if self["common"]["userId"] in [1017637,1843698,1832764,1833080,1833047,1833042,1847323,1300061,1814856,1818411,1840793,1848725,1815272,963260,1852550,1843088,1852657,1016752,830123,1818014]:
        ...
        if self["common"]["userId"] in [1850226]:
            cmd += " -preRender \"python \\\"user_id=%s;mapping=%s;plugins=%s;taskid=%s;start=%s;execfile(\\\\\\\"//10.60.100.101/o5/py/model/prerender.py\\\\\\\")\\\"\"" % (self["common"]["userId"],self["mappings"],self["plugins"]["plugins"],self["common"]["taskId"],self["renderSettings"]["start"])
        ...
        if "RenderMan_for_Maya" in self["plugins"]["plugins"]:
            ...
            if self["common"]["userId"] in [961743,964311,1819610,1814856,1848484,1848680,]:
            ...
            gpu_n="0,1"
            cmd += " -gpu {%s}" % (gpu_n)
        elif self["common"]["userId"] in [962640,1814964,963287, 963492]:
        ...
        if self["common"]["userId"] in [963250,1843663,1840038]:
        ...
        if self["common"]["userId"] in [1846778,120151]:
            if "mtoa" in self["plugins"]["plugins"]:
        ...
        if self["common"]["userId"] in [1831496,1854184]:
            if "mtoa" in self["plugins"]["plugins"]:
        ...
        if self["common"]["taskId"] in [9373662]:
