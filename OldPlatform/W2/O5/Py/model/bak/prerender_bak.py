...
try:
    ...
    print("Current renderner: %s"%RenderNode.currentRenderer.get())
...
def resetup():
        ...
        if i.hasAttr("threads_autodetect"):
            ...
            if threads_autodetect == 0:
                if i.hasAttr("threads"):
                    ...
                    print  "Scene Arnold render threads is : %s" % str(threads)
    ...
    for i in pm.ls(type="aiOptions"):
        if i.hasAttr("textureMaxMemoryMB"):
            textureMaxMemoryMB = i.textureMaxMemoryMB.get()
            if isinstance(textureMaxMemoryMB,float):
                if textureMaxMemoryMB == float(1024) or textureMaxMemoryMB == float(2048):
                    i.textureMaxMemoryMB.set(20480)
                    print "your scene textureMaxMemoryMB is lower ,set it to %s"  % (i.textureMaxMemoryMB)
             
            if textureMaxMemoryMB 
            print "Scene textureMaxMemoryMB is :%s" % str(textureMaxMemoryMB)   
