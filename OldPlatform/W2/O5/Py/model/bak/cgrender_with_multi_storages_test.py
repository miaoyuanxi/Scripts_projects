...
class MayaClass(Render):
    ...
    def set_variables(self):
        # if self["common"]["userId"] in [1868564]:
            # if self["common"]["plugin_file"]:
                # softwareVer = self["common"]["plugin_file"]['softwareVer']
                # print softwareVer
                # print "*"*30
                # if int(maya_v[:4])>=2016:
                    # os.environ['MAYA_OPENCL_IGNORE_DRIVER_VERSION'] = '1'
                    # os.environ['MAYA_VP2_DEVICE_OVERRIDE'] = 'VirtualDeviceDx11'              

            # try:

                # key = win32api.RegOpenKey(win32con.HKEY_LOCAL_MACHINE,'SOFTWARE\\Autodesk\\MC3',0, win32con.KEY_ALL_ACCESS)
                # MC3Enabled = win32api.RegQueryValueEx(key,'MC3Enabled')

                # print "original  key is: %s" % str(MC3Enabled)

                # win32api.RegSetValueEx(key,'MC3Enabled',0,win32con.REG_SZ,'0')

                # MC3Enabled = win32api.RegQueryValueEx(key,'MC3Enabled')

                # print "now key is : %s" % str(MC3Enabled)
    ...
    def set_plugins_myx(self):
        ...
        if self["common"]["plugin_file"]:
            ...
            maya_plugin=MayaPlugin(self["common"]["plugin_file"],[custom_file],self["common"]["userId"],self["common"]["taskId"])
            maya_plugin.config()           
            sys.stdout.flush()
...
class MayaRender(MayaClass):
    ...
    def render(self):
        ...
        if self["common"]["userId"] in [962814,1849749,1858642,1858237,1858240,1858241,1858637,1858638,1879929,1874812,1879925,1879928,1879929,1879930,1879931,1879932,1879934,1879935,1879936,1879937,1879938,1879963,1879966,1879967,1879968,1879969,1879970,1882048,1884724,1886445]:
        ...
        if self["renderSettings"]["renderableLayer"]:
            ...
            elif self["server"]["user_id"] in [1841631,1870881]:                
        ...
        HQID = [1821228,962413,1821226,1821229,1821231,1821234,1821235,1868236,1868237,1868238,1868370,1868371,1868372,1868373,1868374,1868375,1868376,1868377,1868379,1868380,1868382,1868383,1868384]


        if self["common"]["userId"] not in [1844854,1875231,1846887,1854355,1814593,963931,1873017,1812148,1812148,1830380,1871175,1803147,1870881] and  self["common"]["userId"] not in  HQID  and "-preRender"  not in cmd:
        ...
        if self["common"]["userId"] in [1844854]:
            cmd += " -preRender \"python \\\"user_id=%s;mapping=%s;plugins=%s;taskid=%s;rendersetting=%s;start=%s;execfile(\\\\\\\"//10.60.100.101/o5/py/model/prerender_1844854.py\\\\\\\")\\\"\"" % (self["common"]["userId"],self["mappings"],self["plugins"]["plugins"],self["common"]["taskId"],self["renderSettings"],self["renderSettings"]["start"])
        ...
        if self["common"]["userId"] in [1873017]:
            #cmd += " -preRender \"python \\\"user_id=%s;plugins=%s;taskid=%s;renderableCamera=%s;maya_file=%s;execfile(\\\\\\\"//10.60.100.101/o5/py/model/1873017.py\\\\\\\")\\\"\"" % (self["common"]["userId"],self["plugins"]["plugins"],self["common"]["taskId"],self["renderSettings"]["renderableCamera"],self['common']["render_file"])
            cmd += " -preRender \"python \\\"user_id=%s;plugins=%s;taskid=%s;renderableCamera=\\\\\\\"%s\\\\\\\";maya_file=\\\\\\\"%s\\\\\\\";execfile(\\\\\\\"//10.60.100.101/o5/py/model/1873017.py\\\\\\\")\\\"\"" % (self["common"]["userId"],self["plugins"]["plugins"],self["common"]["taskId"],self["renderSettings"]["renderableCamera"],self['common']["render_file"])
                

        if self["common"]["userId"] in [1812148,1830380,1871175]:    
            cmd += " -preRender \"python \\\"user_id=%s;mapping=%s;plugins=%s;taskid=%s;maya_ver=%s;rendersetting=%s;execfile(\\\\\\\"//10.60.100.101/o5/py/model/1812148.py\\\\\\\")\\\"\"" % (self["common"]["userId"],self["mappings"],self["plugins"]["plugins"],self["common"]["taskId"],self["common"]["cgv"],self["renderSettings"])
...
#----------------------------------------------------------------------------------------------------------------------------------------------
        ...
        #----------------------redshift cmd -------------------------------------------------------------                  
            ...
            if self["common"]["userId"] in [961743,964311,1819610,1814856,1848484,1848680,1846319,1859026]:
        ...
        elif self["common"]["userId"] in [1859026]:
            cmd = cmd.replace("-logLevel 2" ," ")
        # elif self["common"]["userId"] in [1870881,1868555,1844854,1860501,962640,1814964,963287, 963492]:
        ...
        if self["common"]["userId"] in [1846778,120151,1824747,1803147]:
            if "mtoa" in self["plugins"]["plugins"]:
                cmd += " -r arnold -ai:lve 1 -ai:txmm 20480"
        if self["common"]["userId"] in [1813119,1858235,1859175]:
            if "mtoa" in self["plugins"]["plugins"]:
...
#            cmd +=" -r file "
                    ...
                    if line_str:
                        ...
                        if self["common"]["userId"] in [963287]:
                            ...
                            if "Maya exited with status -1073741819" in line_str:
                                exit(-1073741819)
                        if self["common"]["userId"] in [1849653]:
                            if "Maya exited with status -1073741819" in line_str:
                                exit(0)
        ...
        if self["common"]["userId"] in [1873017]:
            print "copy vrmap"                    
            copy_vrmap_cmd = "c:/python27/python.exe" +" "+ r"//10.60.100.101/o5/py/model/copy_vrmap.py" +" --ti " + str(self["common"]["taskId"]) +  " --ot " + self["renderSettings"]['output']+ " --rlog " + render_log
