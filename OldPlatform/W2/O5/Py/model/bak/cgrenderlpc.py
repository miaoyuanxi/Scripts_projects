    ...
    if kwargs["task_id"]:
        ...
        if options["platform"]["platform"] == 1005:
            if options["common"]["userId"] in [100001]:
                ...
                #options["platform"]["home_path"] = "%s/d/inputdata5" % (options["mount"]["vcfs"])
        ...
        # if options["common"]["userId"] in [963130, 100001]:
        if options["common"]["userId"] in [963130]:
            options["common"]["plugin_file"] = options["common"]["plugin_file"] = os.path.join(cfg_path, "plugins.cfg")
...
class MayaClass(Render):
    ...
    def set_plugins(self):
        if self["common"]["plugin_file"]:
            ...
            sys.stdout.flush()
            plginLd.RayvisionPluginsLoader(self["common"]["plugin_file"], [])
            sys.stdout.flush()
...
class MayaRender(MayaClass):
    ...
    #                    cmd += " -im \"%s\"" % image_name
            ...
            if self["common"]["userId"] in [120151, 100001]:
