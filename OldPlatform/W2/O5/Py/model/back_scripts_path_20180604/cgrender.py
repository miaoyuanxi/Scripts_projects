...
def get_platfom(platform):
    ...
    info["platform"] = str(info["platform"])
...
class MayaRender(MayaClass):
    ...
    def render(self):
        ...
        if self["common"]["userId"] not in [1844428]:
            cmd += " -preRender \"python \\\"user_id=%s;mapping=%s;plugins=%s;taskid=%s;rendersetting=%s;start=%s;execfile(\\\\\\\"%s/prerender.py\\\\\\\")\\\"\"" % (self["common"]["userId"], self["mappings"], self["plugins"]["plugins"], self["common"]["taskId"],self["renderSettings"],self["renderSettings"]["start"],self["common"]['pre_render'])
    ...
    #                    cmd += " -im \"%s\"" % image_name
        ...
        if self["common"]["userId"] in [1896034,1868555,1850239,963287, 963492, 963250, 964462,965366]:
            cmd += " -r arnold -ai:lve 1"
        if self["common"]["userId"] in [1870881,1854348]:
            cmd += " -r arnold -ai:lve 1 -ai:txmm 20480"
        if self["common"]["userId"] in [1853469,1849549,1824747,1859782,1857702,1832694,1844428]:
            if "mtoa" in self["plugins"]["plugins"]:
                cmd += " -r arnold -ai:lve 1 -ai:txmm 20480"
                
        if self["common"]["userId"] in [1163153]:
            if self["renderSettings"]["tiles"] == 1:
                cmd += " -r mr -rt 32 -aml -v 5"
